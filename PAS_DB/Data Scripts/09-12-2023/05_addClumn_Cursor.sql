...
IF NOT EXISTS (SELECT TOP 1 1 FROM FieldMaster(nolock) WHERE FieldName = 'statusValue' AND ModuleId = (SELECT TOP 1 GridModuleId FROM GridModule(nolock) WHERE ModuleName = 'SalesOrderHistory') AND MasterCompanyId=@MasterCompanyId)
BEGIN
	...
	VALUES ((SELECT TOP 1 GridModuleId FROM GridModule(nolock) WHERE ModuleName = 'SalesOrderHistory') ,'statusValue','Status','120px','string',1,'',14,0,0,null,null,0,@MasterCompanyId,'AUTO SCRIPT',GETUTCDATE(),'AUTO SCRIPT',GETUTCDATE(),1,0)
END
