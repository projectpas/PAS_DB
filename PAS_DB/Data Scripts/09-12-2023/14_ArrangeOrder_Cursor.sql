...
BEGIN   	  	   	 
	 ...
	 SELECT [FieldMasterId],[FieldSortOrder],[FieldName] FROM [dbo].[FieldMaster] WHERE MasterCompanyId = @MasterCompanyId AND ModuleId = (SELECT GridModuleId FROM GridModule WHERE ModuleName = 'WorkOrderMaterialHistory') ORDER BY [FieldSortOrder] ASC

	 SELECT @conditionNameSortOrder = [FieldSortOrder],  
	        @MinId = [rowid]+1 FROM #tmpTblorderbyfieldmaster WHERE [FieldName]='condition'
