...
--EXEC AutoCompleteDropdownsBasedoncolumn 'Asset','AssetRecordid','AssetID','',1,'IsTangible',1,'20',0 ,1     
	  ...
	  ELSE      
	   ...
	   IF(@Parameter4=1)      
		BEGIN      
		 ...
		 BEGIN  
		 SET @Sql = N'INSERT INTO #TempTable (Value, Label) SELECT DISTINCT TOP ' +@Count+ ' CAST ( '+@Parameter1+' AS BIGINT) As Value,CAST ( '+ @Parameter2+  ' AS VARCHAR) AS Label FROM  dbo.'+@TableName+       
			  ' WHERE MasterCompanyId = ' + CAST ( @MasterCompanyId AS nvarchar(50) ) + ' AND CAST ( '+@Parameter1+' AS VARCHAR ) IN (SELECT Item FROM DBO.SPLITSTRING('''+ @Idlist +''','',''))      
				...
				INSERT INTO #TempTable (Value, Label) SELECT DISTINCT TOP ' +@Count+ ' CAST ( '+@Parameter1+' AS BIGINT) As Value,CAST ( '+ @Parameter2+  ' AS VARCHAR) AS Label FROM dbo.' + @TableName+       
				' WHERE MasterCompanyId = ' + CAST ( @MasterCompanyId AS nvarchar(50) ) + ' AND IsActive=1 AND ISNULL(IsDeleted,0)=0 AND CAST ( '+@Parameter2+' AS VARCHAR) !='''' AND '+@Parameter2+'  LIKE '''+ @Parameter3 +'%''  AND CAST ( '+ @Parameter5 +' AS VARCHAR) = '+@Parameter6+'       
				ORDER BY Label'      
		END 
		ELSE 
		 BEGIN
			...
			INSERT INTO #TempTable (Value, Label) SELECT DISTINCT TOP ' +@Count+ ' CAST ( '+@Parameter1+' AS BIGINT) As Value,CAST ( '+ @Parameter2+  ' AS VARCHAR) AS Label FROM  dbo.'+@TableName+       
			  ' WHERE IsActive=1 AND ISNULL(IsDeleted,0)=0 AND CAST ( '+@Parameter2+' AS VARCHAR) !='''' AND '+@Parameter2+'  LIKE '''+ @Parameter3 +'%''         
			  ORDER BY Label'      
		 END
		END
	   ELSE      
