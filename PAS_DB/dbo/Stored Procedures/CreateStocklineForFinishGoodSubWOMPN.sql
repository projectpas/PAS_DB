...
-- EXEC sp_executesql N'EXEC dbo.CreateStocklineForFinishGoodSubWOMPN @SubWOPartNumberId, @UpdatedBy, @IsMaterialStocklineCreate',N'@SubWOPartNumberId bigint,@UpdatedBy nvarchar(11),@IsMaterialStocklineCreate bit',@SubWOPartNumberId=62,@UpdatedBy=N'ADMIN ADMIN',@IsMaterialStocklineCreate=1
**************************************************************/
			...
			BEGIN  
				...
				BEGIN
					...
					UPDATE dbo.WorkOrderMaterials 
						SET Quantity = ISNULL(WOM.Quantity,0) - ISNULL(@SubWOQuantity,0),
							UpdatedDate = GETDATE()												
					...
					-- #STEP 2 ADD STOCKLINE TO WO MATERIAL LIST
						...
						SELECT @NewWorkOrderMaterialsId = WorkOrderMaterialsId 
							...
							BEGIN
								...
								SELECT @UnitCost = UnitCost FROM [dbo].[Stockline] WHERE StockLineId = @StocklineId
							...
							SELECT @NewWorkOrderMaterialsId, @NewStocklineId, @ItemMasterId, @ProvisionId, @RevisedConditionId, @SubWOQuantity, @SubWOQuantity, 0, @UnitCost, (ISNULL(@UnitCost, 0) * ISNULL(@SubWOQuantity, 0)), @UnitCost,
