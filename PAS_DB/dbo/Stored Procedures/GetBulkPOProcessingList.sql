		...
		IF (@filterAsStatus = 'all')
			...
			-- one
			  ...
			  WHERE
						...
						-- AND CASE WHEN ISNULL(PO.IsFromBulkPO,0) = 1 THEN (SELECT TOP 1 [Status] FROM dbo.PoStatus WITH (NOLOCK)  WHERE POStatusId = PO.StatusId) ELSE (SELECT TOP 1 [Status] FROM dbo.PoStatus WITH (NOLOCK)  WHERE Status = 'Open') END  = @filterAsStatus
			 ...
			 -- six
			  ...
			  WHERE  WOP.MasterCompanyId = @MasterCompanyId AND WOStage.WorkOrderStageId = @StageId AND PO.IsFromBulkPO  = 1 
					-- AND CASE WHEN ISNULL(PO.IsFromBulkPO,0) = 1 THEN (SELECT TOP 1 [Status] FROM dbo.VendorRFQStatus  WITH (NOLOCK)  WHERE VendorRFQStatusId = PO.StatusId) ELSE (SELECT TOP 1 [Status] FROM dbo.VendorRFQStatus WITH (NOLOCK)  WHERE Status = 'Open') END  = @filterAsStatus
		...
		ELSE
			...
			SELECT * INTO #TempResult1 FROM  Result 
			 WHERE Quantity > 0 
					...
					SELECT @Count = COUNT(porfqNo) FROM #TempResult1			
