...
-- EXEC GetCustomerInvoicePaymentsByReceiptId 154,1,2,11
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			--  ,[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[IsActive],[IsDeleted],[IsDeposite],[IsTradeReceivable],[TradeReceivableORMiscReceiptGLAccnt],[CtrlNum],[InvoiceType]
			--  ,[OriginalAmount],[NewRemainingBal],[DocNum],[CurrencyCode],[FxRate],[WOSONum],[DSI],[DSO],[AmountPastDue],[ARBalance],[InvDueDate],[CreditLimit],[CreditTermName]
			--  ,[LastMSLevel],[AllMSlevels],[PageIndex],[RemainingAmount],[InvoiceDate],[Id],[GLARAccount]
		--FROM [dbo].[InvoicePayments] WITH (NOLOCK) WHERE ReceiptId = @ReceiptId AND PageIndex=@PageIndex;

			;WITH CTE AS(
					--SELECT [PaymentId],[CustomerId],[SOBillingInvoicingId],[ReceiptId],[MasterCompanyId],0 AS [IsMultiplePaymentMethod],0 AS [IsCheckPayment],0 AS [IsWireTransfer],0 AS [IsEFT],0 AS [IsCCDCPayment]
					--			  ,[PaymentAmount],[DiscAmount],[DiscType],[BankFeeAmount],[BankFeeType],[OtherAdjustAmt],[Reason],[RemainingBalance],[Status]
					--			  ,[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[IsActive],[IsDeleted],0 AS [IsDeposite],[IsTradeReceivable],[TradeReceivableORMiscReceiptGLAccnt]
					...
					--			  ,[CreditLimit],[CreditTermName],[LastMSLevel],[AllMSlevels],[PageIndex],[RemainingAmount],[InvoiceDate],[Id],[GLARAccount]
					--			  --,1 AS 'Selected'
					--			  ,CASE WHEN IsDeleted = 1 THEN 0 ELSE 1 END AS 'Selected'
					--	      FROM [dbo].[InvoicePayments] WITH (NOLOCK) WHERE ReceiptId = @ReceiptId AND PageIndex=@PageIndex
					...
					SELECT [PaymentId],INV.[CustomerId],INV.[SOBillingInvoicingId],[ReceiptId],INV.[MasterCompanyId],0 AS [IsMultiplePaymentMethod],0 AS [IsCheckPayment],0 AS [IsWireTransfer],0 AS [IsEFT],0 AS [IsCCDCPayment]
								  ...
								  ,[CtrlNum],[InvoiceType],[OriginalAmount],[NewRemainingBal],[DocNum],[CurrencyCode],[FxRate],[WOSONum],[DSI],[DSO],[AmountPastDue],[ARBalance],[InvDueDate]
								  ,[CreditLimit],[CreditTermName],[LastMSLevel],[AllMSlevels],[PageIndex],
								  --INV.[RemainingAmount],
					...
					--CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[IsDeposite]  ELSE 0 END AS IsDeposite,
					...
					CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.LastMSLevel    ELSE '' END AS 'LastMSLevel',
					CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.AllMSlevels    ELSE '' END AS 'AllMSlevels',
					--CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.PageIndex    ELSE 0 END AS 'PageIndex',
					CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN @PageIndex    ELSE @PageIndex END AS 'PageIndex',
					--CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.RemainingAmount    ELSE SOBI.RemainingAmount END AS 'RemainingAmount',
					...
					--CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN 1    ELSE 0 END AS 'Selected'
					...
					--AND IPT.IsDeleted=0
					--where SOBI.SOBillingInvoicingId=10063
					...
					--INNER JOIN CreditTerms CT ON CT.CreditTermsId=WO.cre
					...
					--where WOBI.BillingInvoicingId=10063
