  ...
  BEGIN      
   ;WITH CTE AS(      
		SELECT [PaymentId],INV.[CustomerId],INV.[SOBillingInvoicingId],[ReceiptId],INV.[MasterCompanyId],0 AS [IsMultiplePaymentMethod],0 AS [IsCheckPayment],0 AS [IsWireTransfer],0 AS [IsEFT],0 AS [IsCCDCPayment]      
              ...
              ,INV.[CreditLimit],INV.[CreditTermName], CASE WHEN INV.[LastMSLevel] IS NOT NULL THEN INV.[LastMSLevel] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[LastMSLevel] WHEN InvoiceType = 2 THEN MSD_WO.[LastMSLevel] ELSE MSD_CM.[LastMSLevel] END END LastMSLevel,      
               CASE WHEN INV.[AllMSlevels] IS NOT NULL THEN INV.[AllMSlevels] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[AllMSlevels] WHEN InvoiceType = 2 THEN MSD_WO.[AllMSlevels] ELSE MSD_CM.[AllMSlevels] END  END AllMSlevels,      
               [PageIndex],   
               CASE WHEN InvoiceType = 1 THEN SOBI.RemainingAmount     
               WHEN InvoiceType = 2 THEN WOBI.RemainingAmount ELSE 0 END AS 'RemainingAmount',      
   ...
   ;WITH CTE AS(      
     SELECT [PaymentId],INV.[CustomerId],INV.[SOBillingInvoicingId],[ReceiptId],INV.[MasterCompanyId],0 AS [IsMultiplePaymentMethod],0 AS [IsCheckPayment],0 AS [IsWireTransfer],0 AS [IsEFT],0 AS [IsCCDCPayment]      
           ...
           ,[CtrlNum],[InvoiceType],[OriginalAmount],[NewRemainingBal],[DocNum],[CurrencyCode],[FxRate],[WOSONum],[DSI],[DSO],[AmountPastDue],[ARBalance],[InvDueDate],INV.[CreditLimit],INV.[CreditTermName],     
            CASE WHEN INV.[LastMSLevel] IS NOT NULL THEN INV.[LastMSLevel] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[LastMSLevel] WHEN InvoiceType = 2 THEN MSD_WO.[LastMSLevel] ELSE MSD_CM.[LastMSLevel]  END END LastMSLevel,      
            CASE WHEN INV.[AllMSlevels] IS NOT NULL THEN INV.[AllMSlevels] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[AllMSlevels] WHEN InvoiceType = 2 THEN MSD_WO.[AllMSlevels] ELSE MSD_CM.[AllMSlevels] END  END AllMSlevels,      
            [PageIndex],  CASE WHEN InvoiceType = 1 THEN SOBI.RemainingAmount WHEN InvoiceType = 2 THEN WOBI.RemainingAmount ELSE 0 END AS 'RemainingAmount',      
            INV.[InvoiceDate],[Id],[GLARAccount]      
           ,CASE WHEN INV.IsDeleted = 1 THEN 0 ELSE 1 END AS 'Selected',   
