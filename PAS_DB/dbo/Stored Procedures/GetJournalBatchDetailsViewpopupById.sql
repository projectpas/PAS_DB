 ...
 ** --   --------     -------		--------------------------------            
 ...
 14   25/09/2023  Nainshi Joshi			 Added GLAccountClassName.

   
 EXEC GetJournalBatchDetailsViewpopupById 1085,0,'EXPS'  

 EXEC GetJournalBatchDetailsViewpopupById 1684,0,'CMDA'  
...
BEGIN  
 ...
 BEGIN TRY  
  ...
  END  
  Else IF(UPPER(@Module) = UPPER('WOP-DIRECTLABOR'))  
   BEGIN  
   ...
   WHERE JBD.JournalBatchDetailId =@JournalBatchDetailId AND JBD.IsDeleted = @IsDeleted  
  END  
  ELSE IF(UPPER(@Module) = UPPER('RPO') OR UPPER(@Module) = UPPER('RRO') OR UPPER(@Module) = UPPER('RECPO') OR UPPER(@Module) = UPPER('RECRO') OR UPPER(@Module) = UPPER('AST')) OR (UPPER(@Module) = UPPER('MSTK'))        
  BEGIN  
    ...
    SELECT JBD.CommonJournalBatchDetailId
     ...
     INNER JOIN [dbo].[BatchHeader] JBH WITH(NOLOCK) ON BD.JournalBatchHeaderId=JBH.JournalBatchHeaderId  
	  ...
	  --LEFT JOIN [dbo].[VendorRMAPaymentBatchDetails] VRMAP WITH(NOLOCK) ON JBD.CommonJournalBatchDetailId = VRMAP.CommonJournalBatchDetailId  
	  ...
	  --LEFT JOIN [dbo].[StocklineManagementStructureDetails] SMSD WITH (NOLOCK) ON SMSD.ModuleID = @ModuleID AND SMSD.ReferenceID = VRMAP.ReferenceId 
  ...
  END  
  ELSE IF((UPPER(@Module) = UPPER('VRMACS')) OR (UPPER(@Module) = UPPER('VRMAPR'))  OR (UPPER(@Module) = UPPER('VRMACA')))  
  BEGIN  
    DECLARE @STKModuleID INT = 2;    
  ...
  ELSE IF(UPPER(@Module) = UPPER('CRS'))  
   ...
   WHERE JBD.JournalBatchDetailId =@JournalBatchDetailId AND JBD.IsDeleted = @IsDeleted  
  END  
  ELSE IF(UPPER(@Module) = UPPER('CKS')  OR UPPER(@Module) = UPPER('WRT') OR UPPER(@Module) = UPPER('ACHT') OR UPPER(@Module) = UPPER('CCP'))  
   BEGIN  
    SET @CPModuleID = 63  
   ...
   WHERE JBD.JournalBatchDetailId =@JournalBatchDetailId AND JBD.IsDeleted = @IsDeleted  
  END  
  ELSE IF(UPPER(@Module) = UPPER('EXPS') OR UPPER(@Module) = UPPER('EXFB') OR UPPER(@Module) = UPPER('EXCR'))  
   BEGIN  
  ...
  END  
  ELSE IF(UPPER(@Module) = UPPER('CMDA'))  
   BEGIN  
	...
	END
  END  
  ELSE IF(UPPER(@Module) = UPPER('YEP'))
  BEGIN
	SELECT JBD.CommonJournalBatchDetailId
	      ,JBD.[JournalBatchDetailId]  
          ,JBH.[JournalBatchHeaderId]  
