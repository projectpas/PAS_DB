...
-- EXEC [GetVendorRFQPurchaseOrderParts] 42   
************************************************************************/    
...
BEGIN    
 ...
 BEGIN TRY    
  ...
  SELECT PP.[VendorRFQPOPartRecordId]    
	...
	OUTER APPLY(    
   select DISTINCT
		stuff((select ',' + t2.WorkOrderNum from VendorRFQPurchaseOrderPartReference t left join WorkOrder t2 with(nolock) on t.ReferenceId = t2.WorkOrderId
		and t.ModuleId = 1 WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
		for xml path(''),TYPE).value('.','varchar(max)') , 1,1,'') as newdata
		from VendorRFQPurchaseOrderPartReference WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
    ) AS WorkOrderRefNumber 
      OUTER APPLY(    
   select DISTINCT
		stuff((select ',' + s.SalesOrderNumber from VendorRFQPurchaseOrderPartReference t left join SalesOrder s with(nolock) on t.ReferenceId = s.SalesOrderId
		and t.ModuleId = 3 WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
		for xml path(''),TYPE).value('.','varchar(max)') , 1,1,'') as newdata
		from VendorRFQPurchaseOrderPartReference WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
      ...
      OUTER APPLY(    
   select DISTINCT
		stuff((select ',' + sw.SubWorkOrderNo from VendorRFQPurchaseOrderPartReference t left join SubWorkOrder sw with(nolock) on t.ReferenceId = sw.SubWorkOrderId
		and t.ModuleId = 5 WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
		for xml path(''),TYPE).value('.','varchar(max)') , 1,1,'') as newdata
		from VendorRFQPurchaseOrderPartReference WHERE VendorRFQPurchaseOrderId = PP.VendorRFQPurchaseOrderId 
	...
	GROUP BY PP.VendorRFQPOPartRecordId,PP.VendorRFQPurchaseOrderId,PP.ItemMasterId,PP.PartNumber,PP.PartDescription,PP.StockType,PP.ManufacturerId,PP.Manufacturer,PP.PriorityId,PP.Priority,PP.NeedByDate,PP.PromisedDate,PP.ConditionId,PP.Condition,PP.QuantityOrdered,PP.UnitCost,PP.ExtendedCost,PP.WorkOrderId,PP.SubWorkOrderId,PP.SalesOrderId,PP.ManagementStructureId,PP.Level1,PP.Level2,PP.Level3,PP.Level4,PP.Memo,PP.MasterCompanyId,PP.CreatedBy,PP.CreatedDate,PP.UpdatedBy,PP.UpdatedDate,PP.IsActive,PP.IsDeleted,PP.PurchaseOrderId,PP.PurchaseOrderNumber,PP.UOMId,PP.UnitOfMeasure,PO.CreatedDate,PO.Status,POMSD.EntityMSID,POMSD.LastMSLevel,POMSD.AllMSlevels
