 ...
 ** PARAMETERS: @StockLineDraftId bigint,@EntityStructureId bigint,@MasterCompanyId int,@CreatedBy varchar,@UpdatedBy varchar,@ModuleId bigint,@Opr int  
 ...
 ** --   --------     -------  --------------------------------            
    1    09/03/2022  Moin Bloch     Created  
 ...
 BEGIN    
  ...
  BEGIN   
    ...
    DROP TABLE #TempTable  
     ...
     INSERT INTO [dbo].[StockLineDraftManagementStructureDetails]  
        ([ModuleID],[ReferenceID],[EntityMSID],  
         ...
         [MasterCompanyId],[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[IsActive],[IsDeleted])  
           SELECT @ModuleId,@StockLineDraftId,@EntityMSID,                       
     ...
     SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[StockLineDraftManagementStructureDetails] MSD WITH(NOLOCK)   
     WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'  
  
     EXECUTE(@Query)    
       
     UPDATE [dbo].[StockLineDraftManagementStructureDetails] SET [LastMSLevel] = LastMSName,  
  ...
  BEGIN  
    ...
    UPDATE [dbo].[StockLineDraftManagementStructureDetails]  
     ...
     WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @StockLineDraftId AND MSD.EntityMSID = @EntityMSID  
  
     SELECT @MSLevel = MC.ManagementStructureLevel  
     FROM [dbo].[MasterCompany] MC WITH(NOLOCK)   
     WHERE MC.MasterCompanyId = @MasterCompanyId  
  
     SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[StockLineDraftManagementStructureDetails] MSD WITH(NOLOCK)   
     WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'  
  ...
  IF(@Opr = 3)  
  BEGIN  
    DELETE FROM [dbo].[StockLineDraftManagementStructureDetails] WHERE ModuleID = @ModuleId AND ReferenceID = @StockLineDraftId;  
    SELECT @MSDetailsId = isnull(@MSDetailsId,1);  
  END  
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------  
              , @AdhocComments     VARCHAR(150)    = 'PROCAddStockLineDraftMSData'   
              , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ CAST(ISNULL(@StockLineDraftId, '') AS varchar(100))  
