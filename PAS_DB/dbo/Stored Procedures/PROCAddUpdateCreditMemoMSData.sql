...
/*************************************************************           
 ** File:   [PROCAddUpdateCreditMemoMSData]           
 ** Author:  Moin Bloch
 ** Description: This stored procedure is used to store Credit Memo Management Structure Details
 ** Purpose:         
 ** Date:   18/04/2022      
          
 ** PARAMETERS: @CreditMemoHeaderId bigint,@EntityStructureId bigint,@MasterCompanyId int,@CreatedBy varchar,@UpdatedBy varchar,@ModuleId bigint,@Opr int
...
-- EXEC PROCAddUpdateCreditMemoMSData 47,3,2,'moin','moin',47,2,2
************************************************************************/

CREATE PROCEDURE [dbo].[PROCAddUpdateCreditMemoMSData]
...
BEGIN
	...
	BEGIN  
		...
		BEGIN	
			  ...
			  INSERT INTO [dbo].[RMACreditMemoManagementStructureDetails]
			        ...
			        SELECT @ModuleId,@CreditMemoHeaderId,@EntityMSID,													        
			  ...
			  SELECT @MSDetailsId = IDENT_CURRENT('RMACreditMemoManagementStructureDetails');
			  		      
			  UPDATE dbo.CreditMemo SET ManagementStructureId = @EntityMSID WHERE [CreditMemoHeaderId] = @CreditMemoHeaderId; 

			  SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId;
								
			  SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[RMACreditMemoManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

			  EXECUTE(@Query)  
			  
			  UPDATE [dbo].[RMACreditMemoManagementStructureDetails] SET [LastMSLevel] = LastMSName,
		...
		BEGIN
				...
				UPDATE [dbo].[RMACreditMemoManagementStructureDetails]
					...
					FROM [dbo].EntityStructureSetup EST WITH(NOLOCK)
							 ...
							 LEFT JOIN ManagementStructureLevel MSL9 WITH (NOLOCK) ON  EST.Level9Id = MSL9.ID
							 LEFT JOIN ManagementStructureLevel MSL10 WITH (NOLOCK) ON EST.Level10Id = MSL10.ID													   
					WHERE ModuleID = @ModuleId AND ReferenceID = @CreditMemoHeaderId AND EST.EntityStructureId=@EntityMSID;

					SELECT @MasterCompanyId = [MasterCompanyId], @MSDetailsId = MSDetailsId
					FROM [dbo].[RMACreditMemoManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @CreditMemoHeaderId AND MSD.EntityMSID = @EntityMSID

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[RMACreditMemoManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'PROCAddUpdateCreditMemoMSData' 
