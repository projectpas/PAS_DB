            ...
            ELSE BEGIN
                     IF(@Parameter4=1)BEGIN
                         IF(@TableName='ItemMaster' AND ISNULL(@IsFromUpload,0) = 0)BEGIN
                             ...
                             WHERE Im.MasterCompanyId=@MasterCompanyId AND ISNULL(IsActive, 1)=1 AND ISNULL(IsDeleted, 0)=0 AND (Im.PartNumber like '%'+@Parameter3+'%' OR REPLACE(REPLACE(Im.PartNumber, '-', ''), '/', '') like '%'+REPLACE(REPLACE(@Parameter3, '-', ''), '/', '')+'%')
                         ...
                         ELSE IF(@TableName='ItemMasterNonStock')BEGIN
                                  ...
                                  FROM dbo.ItemMaster IM
                                  WHERE IM.MasterCompanyId=@MasterCompanyId AND ISNULL(IM.IsActive, 1)=1 AND ISNULL(IM.IsDeleted, 0)=0 AND (IM.PartNumber like '%'+@Parameter3+'%' OR REPLACE(REPLACE(IM.PartNumber, '-', ''), '/', '') like '%'+REPLACE(REPLACE(@Parameter3, '-', ''), '/', '')+'%')
