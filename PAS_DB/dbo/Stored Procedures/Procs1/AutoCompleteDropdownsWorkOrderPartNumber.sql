		...
		BEGIN		
					...
					FROM dbo.ReceivingCustomerWork RCW WITH(NOLOCK) 
					 ...
					 AND ISNULL(OIM.IsNonStock,0) = 0
					  ...
					  AND ISNULL(RCW.IsPiecePart,0) = 0 AND RCW.MasterCompanyId = @MasterCompanyId AND (Im.partnumber LIKE @StartWith + '%' OR Im.partnumber  LIKE '%' + @StartWith + '%' OR REPLACE(REPLACE(Im.partnumber, '-', ''), '/', '') LIKE '%' + REPLACE(REPLACE(@StartWith, '-', ''), '/', '') + '%') 
			...
			BEGIN
					...
					WHERE SL.IsActive = 1 AND SL.IsDeleted = 0 AND ISNULL(SL.WorkOrderId, 0) = 0 AND ISNULL(SL.IsParent,0) = 1 AND ISNULL(SL.IsCustomerStock, 0) = 0 AND ISNULL(SL.QuantityAvailable, 0) > 0
						AND ISNULL(RCW.IsPiecePart,0) = 0 AND SL.MasterCompanyId = @MasterCompanyId AND (Im.partnumber LIKE @StartWith + '%' OR Im.partnumber  LIKE '%' + @StartWith + '%' OR REPLACE(REPLACE(Im.partnumber, '-', ''), '/', '') LIKE '%' + REPLACE(REPLACE(@StartWith, '-', ''), '/', '') + '%')
