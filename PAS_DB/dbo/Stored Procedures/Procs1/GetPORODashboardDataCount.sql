 ...
 ** --   --------     -------		-------------------------------            
	...
	5    19 JUN 2026  Abhishek Jirawla	Adding IsPiecePart condition in RepairOrderPart table 
...
SET NOCOUNT ON  
	...
	BEGIN  
		...
		--INNER JOIN dbo.EmployeeUserRole EUR WITH (NOLOCK) ON EUR.RoleId = RMS.RoleId AND EUR.EmployeeId = @EmployeeId  
		WHERE ISNULL(RO.IsDeleted, 0) = 0 AND ISNULL(ROP.isParent, 0) = 1 AND ISNULL(ROP.IsDeleted, 0) = 0 AND (RO.StatusId = @ROOpenStatusId)  AND ISNULL(ROP.[IsPiecePart], 0) = 0
		AND RO.MasterCompanyId = @MasterCompanyId  
		GROUP BY RO.StatusId  
		...
		--INNER JOIN dbo.RoleManagementStructure RMS WITH (NOLOCK) ON ROP.ManagementStructureId = RMS.EntityStructureId  
		--INNER JOIN dbo.EmployeeUserRole EUR WITH (NOLOCK) ON EUR.RoleId = RMS.RoleId AND EUR.EmployeeId = @EmployeeId  
		WHERE ISNULL(RO.IsDeleted, 0) = 0 AND ISNULL(ROP.isParent, 0) = 1 AND ISNULL(ROP.IsDeleted, 0) = 0 AND (RO.StatusId = @ROApprovedStatusId)   AND ISNULL(ROP.[IsPiecePart], 0) = 0
		AND RO.MasterCompanyId = @MasterCompanyId  
		GROUP BY RO.StatusId  
		...
		--INNER JOIN dbo.RoleManagementStructure RMS WITH (NOLOCK) ON ROP.ManagementStructureId = RMS.EntityStructureId  
		--INNER JOIN dbo.EmployeeUserRole EUR WITH (NOLOCK) ON EUR.RoleId = RMS.RoleId AND EUR.EmployeeId = @EmployeeId  
		WHERE ISNULL(RO.IsDeleted, 0) = 0 AND ISNULL(ROP.isParent, 0) = 1 AND ISNULL(ROP.IsDeleted, 0) = 0 AND (RO.StatusId = @ROFulfillingStatusId)   AND ISNULL(ROP.[IsPiecePart], 0) = 0
		AND RO.MasterCompanyId = @MasterCompanyId  
		GROUP BY RO.StatusId  
