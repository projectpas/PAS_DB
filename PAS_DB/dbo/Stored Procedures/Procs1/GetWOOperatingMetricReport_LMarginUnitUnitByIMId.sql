...
-- ---------------------------------------------------------------------------------------------------
-- Stored Procedure: dbo.GetWOOperatingMetricReport_LMarginUnitUnitByIMId   (source: PAS_DB/dbo/Stored Procedures/Procs1/GetWOOperatingMetricReport_LMarginUnitUnitByIMId.sql)
-- ---------------------------------------------------------------------------------------------------
/********************************************************************             
 ** File:   [dbo.GetWOOperatingMetricReport_LMarginUnitUnitByIMId]             
 ** Author:  Rajesh Gami    
 ** Description: Get Data for Workorder Operating Metric Report Low Margin To Highest Margin  By ItemMaster Id
 ** Purpose:           
 ** Date:   19-Mar-2024         
...
***********************************************************************/  
CREATE     PROCEDURE [dbo].[GetWOOperatingMetricReport_LMarginUnitUnitByIMId] 
@PageNumber int = 1,
@PageSize int = NULL,
@mastercompanyid int,
@xmlFilter XML
...
BEGIN  
  ...
  BEGIN TRY  
    --BEGIN TRANSACTION  
      ...
      (SELECT 
			...
			(ISNULL(WOBIT.GrandTotal, 0) - (ISNULL(WBI.SalesTax, 0) + ISNULL(WBI.OtherTax, 0))) AS totalRevenues,
			...
			((ISNULL(WOBIT.GrandTotal, 0) - (ISNULL(WBI.SalesTax, 0) + ISNULL(WBI.OtherTax, 0))) - (ISNULL(CST.PartsCost,0) + ISNULL(CST.LaborCost,0) + ISNULL(CST.ChargesCost,0) + ISNULL(CST.FreightCost,0) + ISNULL(WOBIT.SalesTax,0)+ ISNULL(WOBIT.OtherTax,0)) ) as marginAmounts,
       ...
       FROM 
			...
			INNER JOIN DBO.BillingInvoicing AS WBI WITH (NOLOCK) ON WOBIT.BillingInvoicingId = WBI.BillingInvoicingId AND ISNULL(WBI.IsVersionIncrease,0)=0 AND ISNULL(WBI.IsPerformaInvoice, 0) = 0  
			...
			INNER JOIN DBO.WorkOrderMPNCostDetails CST WITH (NOLOCK) ON WOBIT.SubReferenceId = CST.WOPartNoId  
  ...
  BEGIN CATCH  
    ...
    DECLARE @ErrorLogID int,  
            ...
            @AdhocComments varchar(150) = '[dbo.GetWOOperatingMetricReport_LMarginUnitUnitByIMId]',  
