...
-- ---------------------------------------------------------------------------------------------------
-- Stored Procedure: dbo.GetWorkOrderLaborTrackingList   (source: PAS_DB/dbo/Stored Procedures/Procs1/GetWorkOrderLaborTrackingList.sql)
-- ---------------------------------------------------------------------------------------------------
/*************************************************************           
...
CREATE     PROCEDURE [dbo].[GetWorkOrderLaborTrackingList]  
 ...
 BEGIN TRY  
  BEGIN TRANSACTION  
   BEGIN   
    ...
    BEGIN  
	set @SortOrder = 1
     Set @SortColumn=Upper('ISCOMPLETED')  
    END   
    Else  
    BEGIN   
    ...
    IF(@IsFromEmployee = 1)
	BEGIN
	;WITH Result(WorkOrderLaborTrackingId,WorkOrderLaborId,WorkOrderNum,WorkOrderId,PartNumber,PartDescription,ManufacturerName,OpenDate,StartTime,EndTime,TotalHours,TotalMinutes,IsCompleted,CreatedDate
	,UpdatedDate,CreatedBy,UpdatedBy,IsActive,IsDeleted,EmployeeName,TaskStatusGr,TaskStatusId,StatusChangedDate,TaskName,IsAnyTaskStarted)
	AS (
	SELECT   
		WOT.WorkOrderLaborTrackingId,
		WOT.WorkOrderLaborId,
        WO.WorkOrderNum,   
        WO.WorkOrderId,  
		...
		ISNULL(WOT.IsCompleted,0)IsCompleted,
	 ...
	 AND ISNULL(IM.IsNonStock,0) = 0 ),ResultCount AS(Select COUNT(WorkOrderId) AS totalItems FROM Result)	
       ...
       SELECT *, @Count As NumberOfItems FROM #TempResult  
		...
		CASE WHEN (@SortOrder=1 and @SortColumn='ISCOMPLETED')  THEN IsCompleted END ASC, 
		...
		CASE WHEN (@SortOrder=-1 and @SortColumn='ISCOMPLETED')  THEN IsCompleted END DESC, 
