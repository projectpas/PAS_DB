/*************************************************************           
 ** File:   [PROCAddCustomerMSData]           
 ** Author:  Deep Patel
 ** Description: This stored procedure is used to store Customer Payement Management Structure Details
 ** Purpose:         
 ** Date:   07/03/2022        
 ** PARAMETERS: @RepairOrderId bigint,@EntityStructureId bigint,@MasterCompanyId int,@CreatedBy varchar,@UpdatedBy varchar,@ModuleId bigint,@Opr int  
 ** RETURN VALUE:           
 **************************************************************           
 ** Change History           
 **************************************************************           
 ...
 ** --   --------     -------		--------------------------------          
    1    08/03/2022  Deep Patel     Created
-- EXEC PROCAddCustomerMSData 47,3,5,2,'moin','moin',47,2,2
************************************************************************/
CREATE PROCEDURE [dbo].[PROCAddCustomerMSData]
...
BEGIN
	...
	BEGIN  
		...
		BEGIN	
			  ...
			  INSERT INTO [dbo].[CustomerManagementStructureDetails]
			        ...
			        SELECT @ModuleId,@CustomerPaymentId,@EntityMSID,													        
			  ...
			  SELECT @MSDetailsId = IDENT_CURRENT('CustomerManagementStructureDetails');
			  		      
			  --UPDATE dbo.RepairOrder SET ManagementStructureId = @EntityMSID WHERE [RepairOrderId] = @RepairOrderId; 

			  SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId;
								
			  SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[CustomerManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

			  EXECUTE(@Query)  
			  
			  UPDATE [dbo].[CustomerManagementStructureDetails]
		...
		BEGIN
				...
				UPDATE [dbo].[CustomerManagementStructureDetails]
					...
					FROM [dbo].EntityStructureSetup EST WITH(NOLOCK)
							 ...
							 LEFT JOIN ManagementStructureLevel MSL9 WITH (NOLOCK) ON  EST.Level9Id = MSL9.ID
							 LEFT JOIN ManagementStructureLevel MSL10 WITH (NOLOCK) ON EST.Level10Id = MSL10.ID													   
					WHERE ModuleID = @ModuleId AND ReferenceID = @CustomerPaymentId AND EST.EntityStructureId=@EntityMSID;

					SELECT @MasterCompanyId = [MasterCompanyId], @MSDetailsId = MSDetailsId
					FROM [dbo].[CustomerManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @CustomerPaymentId AND MSD.EntityMSID = @EntityMSID

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[CustomerManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              ...
              , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ CAST(ISNULL(@CustomerPaymentId, '') AS varchar(100))
