...
/*************************************************************           
 ** File:   [PROCAddUpdatePublicationMSData]           
...
-- EXEC PROCAddUpdatePublicationMSData 47,3,2,'subhash','subhash',47,2,2
************************************************************************/

CREATE PROCEDURE [dbo].[PROCAddUpdatePublicationMSData]
@ReferenceId bigint,
@EntityMSID bigint,
@MasterCompanyId int,
...
BEGIN
	...
	BEGIN  
		...
		BEGIN	
			  ...
			  INSERT INTO [dbo].[PublicationManagementStructureDetails]
						  ([ModuleID],[ReferenceID],[EntityMSID],
						   ...
						   [MasterCompanyId],[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[IsActive],[IsDeleted],PublicationRecordId)
			        SELECT @ModuleId,@ReferenceId,@EntityMSID,													        
							...
							@MasterCompanyId,@CreatedBy,@UpdatedBy,GETUTCDATE(),GETUTCDATE(),1,0,@PublicationRecordId
			  ...
			  SELECT @MSDetailsId = IDENT_CURRENT('PublicationManagementStructureDetails');
			  		      
			  UPDATE dbo.PublicationManagementStructureMapping SET ManagementStructureId = @EntityMSID WHERE PublicationManagementStructureMappingId = @ReferenceId; 

			  SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId;
								
			  SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PublicationManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

			  EXECUTE(@Query)  
			  
			  UPDATE [dbo].[PublicationManagementStructureDetails] SET [LastMSLevel] = LastMSName,
		...
		BEGIN
				...
				UPDATE [dbo].[PublicationManagementStructureDetails]
					...
					FROM [dbo].[PublicationManagementStructureDetails] MSD WITH(NOLOCK) 
					...
					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PublicationManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'PROCAddUpdatePublicationMSData' 
