 ...
 ** --   --------     -------		--------------------------------          
    1    10/03/2022  Deep Patel     Created    
-- EXEC PROCAddVendorRFQROMSData 47,3,2,'moin','moin',47,2,2
************************************************************************/
CREATE PROCEDURE [dbo].[PROCAddVendorRFQROMSData]
...
BEGIN
	...
	BEGIN  
		...
		DECLARE @VendorRFQROPartModuleID VARCHAR(MAX) = 23;
		IF(@Opr = 1)
		BEGIN
			IF NOT EXISTS(SELECT ReferenceID FROM RepairOrderManagementStructureDetails WHERE ReferenceID = @RepairOrderId AND ModuleID = @VendorRFQROPartModuleID)
			BEGIN
				...
				INSERT INTO [dbo].[RepairOrderManagementStructureDetails]
				      ...
				      SELECT @ModuleId,@RepairOrderId,@EntityMSID,													        
				...
				SELECT @MSDetailsId = IDENT_CURRENT('RepairOrderManagementStructureDetails');
				...
				SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[RepairOrderManagementStructureDetails] MSD WITH(NOLOCK) 
						WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

				EXECUTE(@Query)  
				
				UPDATE [dbo].[RepairOrderManagementStructureDetails] 
			...
			BEGIN
				...
				UPDATE [dbo].[RepairOrderManagementStructureDetails]
					...
					FROM [dbo].EntityStructureSetup EST WITH(NOLOCK)
							 ...
							 LEFT JOIN ManagementStructureLevel MSL9 WITH (NOLOCK) ON  EST.Level9Id = MSL9.ID
							 LEFT JOIN ManagementStructureLevel MSL10 WITH (NOLOCK) ON EST.Level10Id = MSL10.ID													   
					WHERE ModuleID = @ModuleId AND ReferenceID = @RepairOrderId AND EST.EntityStructureId=@EntityMSID;
					...
					WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @RepairOrderId AND MSD.EntityMSID = @EntityMSID

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[RepairOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
