			...
			BEGIN
				...
				SELECT  sop.SalesOrderId as RefId,
						...
						LEFT JOIN DBO.ItemMaster imt WITH (NOLOCK) ON imt.ItemMasterId = sop.ItemMasterId
						 AND ISNULL(imt.IsNonStock,0) = 0 LEFT JOIN DBO.Priority P WITH (NOLOCK)  ON P.PriorityId = sop.PriorityId
					...
					from DBO.ExchangeSalesOrderPart sop WITH (NOLOCK)
						...
						LEFT JOIN DBO.ItemMaster imt WITH (NOLOCK) on imt.ItemMasterId = rop.ItemMasterId
						 AND ISNULL(imt.IsNonStock,0) = 0 LEFT JOIN DBO.Priority P WITH (NOLOCK) ON P.PriorityId = rop.PriorityId
						...
						LEFT JOIN DBO.ItemMaster imt WITH (NOLOCK) ON imt.ItemMasterId = rmad.ItemMasterId
						 AND ISNULL(imt.IsNonStock,0) = 0 LEFT JOIN DBO.VendorShipping VS WITH (NOLOCK) ON VS.VendorId = rma.VendorId and VS.IsPrimary=1
