 ...
 ** --   --------     -------				--------------------------------          
   ...
   11    07-07-2025   Moin Bloch            Changed Old To New Billing Table
	...
	BEGIN TRY
		BEGIN TRANSACTION
			BEGIN
				...
				BEGIN 
					INSERT INTO #SalesOrderBillingInvoiceList(SalesOrderNumber,partnumber,ItemMasterId,PartDescription,ConditionId,Condition,SalesOrderId,SalesOrderPartId, SOPartId,Status,ItemNo,CustomerId)
					(
					SELECT DISTINCT so.SalesOrderNumber, imt.partnumber,imt.ItemMasterId, imt.PartDescription, sop.ConditionId, cond.Description as 'Condition', sop.SalesOrderId, imt.ItemMasterId As SalesOrderPartId, sop.SalesOrderPartId, '' as [Status],	0 AS ItemNo,so.CustomerId
					...
					LEFT JOIN DBO.BillingInvoicing sobi WITH (NOLOCK) on sobi.ReferenceId = sos.SalesOrderId AND sobi.IsPerformaInvoice = 0 AND sobi.[ModuleId] = @SOModuleId
					...
					WHERE sop.SalesOrderId = @SalesOrderId AND ISNULL(stk.StockLineId, 0) > 0
				...
				BEGIN
					...
					BEGIN 
						...
						sop.SalesOrderId, imt.ItemMasterId AS SalesOrderPartId, sop.SalesOrderPartId AS SOPartId,	'' as [Status], 0 AS ItemNo,so.CustomerId
						FROM DBO.SalesOrderPartV1 sop WITH (NOLOCK)
						LEFT JOIN DBO.SalesOrderStocklineV1 stk WITH (NOLOCK) ON stk.SalesOrderPartId = sop.SalesOrderPartId
						LEFT JOIN DBO.SalesOrder so WITH (NOLOCK) on so.SalesOrderId = sop.SalesOrderId
						LEFT JOIN DBO.ItemMaster imt WITH (NOLOCK) on imt.ItemMasterId = sop.ItemMasterId
						 AND ISNULL(imt.IsNonStock,0) = 0
						 LEFT JOIN DBO.Stockline sl WITH (NOLOCK) on sl.StockLineId = stk.StockLineId AND ISNULL(sl.IsNonStock,0) = 0
						LEFT JOIN DBO.BillingInvoicing sobi WITH (NOLOCK) on sobi.ReferenceId = sop.SalesOrderId AND sobi.[ModuleId] = @SOModuleId  AND ISNULL(sobi.IsPerformaInvoice,0) = 0--AND sobi.IsVersionIncrease = 0
						LEFT JOIN DBO.BillingInvoicingItems sobii WITH (NOLOCK) on sobii.BillingInvoicingId = sobi.BillingInvoicingId
