					...
					BEGIN
						...
						FROM [dbo].[Work_ReleaseFrom_8130] wro WITH(NOLOCK)
							  ...
							  LEFT JOIN [dbo].[ItemMaster] im  WITH(NOLOCK) ON im.ItemMasterId = wop.ItemMasterId  
							   ...
							   AND ISNULL(ims.IsNonStock,0) = 0 LEFT JOIN [dbo].[WorkOrderSettlementDetails] wosc WITH(NOLOCK) ON wop.WorkOrderId = wosc.WorkOrderId AND wop.ID = wosc.workOrderPartNoId AND wosc.WorkOrderSettlementId = @WorkOrderSettlementId
