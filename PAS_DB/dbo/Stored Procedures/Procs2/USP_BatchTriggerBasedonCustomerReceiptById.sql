...
************************************************************************/
CREATE PROCEDURE [dbo].[USP_BatchTriggerBasedonCustomerReceiptById]
...
BEGIN
	...
	BEGIN
			 ...
			 SELECT @MasterCompanyId=MasterCompanyId FROM DBO.CustomerPayments WHERE ReceiptId=@ReceiptId;
			 ...
			 BEGIN
					  select @ReceiptNo = ReceiptNo,@CurrentManagementStructureId =ManagementStructureId from DBO.CustomerPayments WITH(NOLOCK)  where ReceiptId=@ReceiptId
	                  select @LastMSLevel=LastMSLevel,@AllMSlevels=AllMSlevels from DBO.CustomerManagementStructureDetails  where ReferenceID=@ReceiptId and ModuleID=@CRMSModuleId
				...
				END
                 ...
                 END
	              ...
	              BEGIN
					 ...
					 --SELECT @AapliedAmount=AppliedAmount,@InvoiceAmount=InvoiceAmount,@IsDeposit=IsDeposite from CustomerPaymentDetails WHERE ReceiptId=@ReceiptId AND PageIndex=@PageIndex;
					 --SET @InvoiceAmountDiffeence = @AapliedAmount - @InvoiceAmount;

					 DECLARE @CustomerPaymentDetailsId BIGINT;
					 DECLARE @CustomerPaymentDetailsCursor AS CURSOR;
					 SET @CustomerPaymentDetailsCursor = CURSOR FAST_FORWARD FOR	
				     SELECT CustomerPaymentDetailsId from DBO.CustomerPaymentDetails WHERE ReceiptId=@ReceiptId
					 							   		 	  	  
					 OPEN @CustomerPaymentDetailsCursor;
					 FETCH NEXT FROM @CustomerPaymentDetailsCursor INTO @CustomerPaymentDetailsId;
					 WHILE @@FETCH_STATUS = 0
					 BEGIN
					     PRINT @CustomerPaymentDetailsId;
						 Declare @PaymentAmount decimal(18,2)=0;
						 Declare @AccountsReceivablesAmount decimal(18,2)=0;
						 --Declare @DiscAmount decimal(18,2)=0;
						 --Declare @BankFeesAmount decimal(18,2)=0;
						 ...
						 SELECT @AapliedAmount=AppliedAmount,@InvoiceAmount=InvoiceAmount,@IsDeposit=IsDeposite,@PageIndex=PageIndex,@CustomerId=CustomerId,@UpdatedBy=UpdatedBy from DBO.CustomerPaymentDetails WHERE CustomerPaymentDetailsId=@CustomerPaymentDetailsId;
						 SET @InvoiceAmountDiffeence = @AapliedAmount - @InvoiceAmount;

						 SELECT @Ismiscellaneous=Ismiscellaneous from DBO.Customer WHERE CustomerId=@CustomerId;
						 ...
						 IF(@IsDeposit = 1)
						 BEGIN
						 	SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName from DBO.DistributionSetup WITH(NOLOCK)  where UPPER(Name) =UPPER('Deposit/Unearned Revenue') And DistributionMasterId=@DistributionMasterId
							...
							VALUES
								...
								GETUTCDATE(),GETUTCDATE(),@JournalTypeId ,@JournalTypename ,0,0,@AapliedAmount,@ManagementStructureId ,@ModuleName,@LastMSLevel,@AllMSlevels ,@MasterCompanyId,@UpdatedBy,@UpdatedBy,GETUTCDATE(),GETUTCDATE(),1,0)
							...
							SET @CommonJournalBatchDetailId=SCOPE_IDENTITY()
						             ...
						             VALUES
						              (@JournalBatchDetailId,@JournalBatchHeaderId,@CustomerTypeId ,@CustomerTypeName ,@CustomerId,@CustomerName,0,@ReceiptId,@ReceiptNo ,@SOBillingInvoicingId,@InvoiceNo,@SOBillingInvoicingId,@InvoiceNo,NULL,NULL,@CommonJournalBatchDetailId)
						 END

						 -----Suspense------
					     IF(@InvoiceAmountDiffeence > 0 AND @Ismiscellaneous=0)
					     BEGIN
					  	  SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName from DBO.DistributionSetup WITH(NOLOCK)  where UPPER(Name) =UPPER('Suspense') And DistributionMasterId=@DistributionMasterId
						  ...
						  END
							...
							VALUES
								(@JournalBatchDetailId,@JournalTypeNumber,@currentNo,@DistributionSetupId,@DistributionName,@JournalBatchHeaderId,1 ,@GlAccountId ,@GlAccountNumber ,@GlAccountName,
								GETUTCDATE(),GETUTCDATE(),@JournalTypeId ,@JournalTypename ,0,0,@InvoiceAmountDiffeence,@ManagementStructureId ,@ModuleName,@LastMSLevel,@AllMSlevels ,@MasterCompanyId,@UpdatedBy,@UpdatedBy,GETUTCDATE(),GETUTCDATE(),1,0)
						 ...
						 BEGIN
						 	SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName from DBO.DistributionSetup WITH(NOLOCK)  where UPPER(Name) =UPPER('Revenue - Misc Charge') And DistributionMasterId=@DistributionMasterId
						 ...
						 END
						 -----Account Receivables------
						 SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName from DBO.DistributionSetup WITH(NOLOCK)  where UPPER(Name) =UPPER('Account Receivables') And DistributionMasterId=@DistributionMasterId
						 ...
						 FETCH NEXT FROM @CustomerPaymentDetailsCursor INTO @CustomerPaymentDetailsId 
					 END
					 CLOSE @CustomerPaymentDetailsCursor  
					 DEALLOCATE @CustomerPaymentDetailsCursor
