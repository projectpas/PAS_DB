...
BEGIN
	...
	BEGIN
		...
		SELECT @CurrentManagementStructureId = ISNULL(ManagementStructureId,0) FROM [dbo].[Employee] WITH(NOLOCK)  WHERE CONCAT(TRIM(REPLACE([FirstName], ' ', '')),'',TRIM(REPLACE([LastName], ' ', ''))) IN (replace(@UpdateBy, ' ', '')) and MasterCompanyId=@MasterCompanyId
		...
		BEGIN 
			...
			BEGIN				
				...
				LEFT JOIN [dbo].[Stockline] STL  WITH(NOLOCK)  ON STL.[StockLineId] = WOM.[StockLineId]
				LEFT JOIN [dbo].[Lot] LO  WITH(NOLOCK) ON LO.[LotId] = STL.[LotId]		
			...
			BEGIN
				...
				BEGIN
					...
					SET @OverheadCost=0
					SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName,@CrDrType = CRDRType,@IsAutoPost = ISNULL(IsAutoPost,0) 
					FROM [dbo].[DistributionSetup] WITH(NOLOCK)  WHERE UPPER(DistributionSetupCode) =UPPER('WIPDIRECTLABOR') and DistributionMasterId =@DistributionMasterId AND MasterCompanyId=@MasterCompanyId
				END
			...
			BEGIN
				SELECT @MaterialCost=PartsCost,@LaborCost=(Isnull(WOPN.LaborCost,0)-Isnull(WOPN.OverHeadCost,0)),@LaborOverHeadCost=(Isnull(WOPN.OverHeadCost,0)) 
				...
				BEGIN
					...
					BEGIN
						SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName,@CrDrType = CRDRType,@IsAutoPost = ISNULL(IsAutoPost,0)
						FROM [dbo].[DistributionSetup] WITH(NOLOCK)  WHERE UPPER(DistributionSetupCode) =UPPER('FGINVENTROY') and DistributionMasterId =@DistributionMasterId AND MasterCompanyId=@MasterCompanyId
						...
						BEGIN							
							SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName,@CrDrType = CRDRType 
							FROM [dbo].[DistributionSetup] WITH(NOLOCK)  WHERE UPPER(DistributionSetupCode) =UPPER('FG-WIP-PARTS') and DistributionMasterId =@DistributionMasterId AND MasterCompanyId=@MasterCompanyId
					...
					BEGIN
						SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName,@CrDrType = CRDRType
						FROM [dbo].[DistributionSetup] WITH(NOLOCK)  WHERE UPPER(DistributionSetupCode) =UPPER('FG-WIP-LABOR') and DistributionMasterId =@DistributionMasterId AND MasterCompanyId=@MasterCompanyId
					...
					BEGIN
						SELECT top 1 @DistributionSetupId=ID,@DistributionName=Name,@JournalTypeId =JournalTypeId,@GlAccountId=GlAccountId,@GlAccountNumber=GlAccountNumber,@GlAccountName=GlAccountName,@CrDrType = CRDRType
						FROM [dbo].[DistributionSetup] WITH(NOLOCK)  WHERE UPPER(DistributionSetupCode) =UPPER('FG-WIP-OVERHEAD') and DistributionMasterId =@DistributionMasterId AND MasterCompanyId=@MasterCompanyId
				...
				BEGIN
					...
					BEGIN
						...
						ELSE
						BEGIN
							SELECT @JournalBatchHeaderId=JournalBatchHeaderId,@CurrentPeriodId=isnull(AccountingPeriodId,0) 
	        ...
	        BEGIN
				...
				SELECT @LaborCost = (ISNULL(SUM(WOPN.[LaborCost]),0) - ISNULL(SUM(WOPN.[OverHeadCost]),0)),
				       @LaborOverHeadCost = ISNULL(SUM(WOPN.[OverHeadCost]),0),
					   @MaterialCost = ISNULL(SUM(WOPN.[PartsCost]),0)
				  FROM [dbo].[WorkOrderMPNCostDetails]  WOPN WITH(NOLOCK)
                INNER JOIN [dbo].[BillingInvoicingItems] WOBIT WITH(NOLOCK) ON WOPN.[WOPartNoId] = WOBIT.[SubReferenceId]
                WHERE WOBIT.[BillingInvoicingId] = @InvoiceId AND ISNULL(WOBIT.[IsVersionIncrease],0) = 0 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
            , @AdhocComments     VARCHAR(150)    = 'USP_BatchTriggerBasedonDistributionForInternalWONew' 
            , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ ISNULL(@InvoiceId, '') + ''
