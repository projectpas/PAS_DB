...
-- ---------------------------------------------------------------------------------------------------
-- Stored Procedure: dbo.USP_BatchTriggerBasedonDistributionForSubWorkOrder   (source: PAS_DB/dbo/Stored Procedures/Procs2/USP_BatchTriggerBasedonDistributionForSubWorkOrder.sql)
-- ---------------------------------------------------------------------------------------------------
/*************************************************************           
 ...
 ** --   --------     -------		--------------------------------              
	...
	9    22/Sep/2026			 RAJESH GAMI						[PN-17782] Removed the ISNULL(...IsNonStock,0)=0 restriction on the SubWorkOrderMaterialStockLine/ItemMaster lookup so Non-Stock parts are included in the Sub Work Order material accounting batch trigger
************************************************************************/
						...
						INSERT INTO [dbo].[CommonBatchDetails]
								 ...
								 VALUES
										(@JournalBatchDetailId,
										 ...
										 1,
										 0
										 ,@SubWorkOrderNumber,@CustomerName,@CurrencyCode,@FXRate,@CurrencyCode,@ReferenceId,@ReferenceModule)
				...
				BEGIN
					 ...
					 BEGIN
						...
						VALUES
							...
							CASE WHEN @CrDrType = 1 THEN @Amount ELSE 0 END,
							CASE WHEN @CrDrType = 1 THEN 0 ELSE @Amount END
							,@ManagementStructureId ,@ModuleName,@LastMSLevel,@AllMSlevels ,@MasterCompanyId,@UpdateBy,@UpdateBy,GETUTCDATE(),GETUTCDATE(),1,0,@SubWorkOrderNumber,@CustomerName,@CurrencyCode,@FXRate,@CurrencyCode,@ReferenceId,@ReferenceModule)
