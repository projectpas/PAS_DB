...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WHERE BillingInvoicingId=@InvoiceId AND ISNULL(IsPerformaInvoice,0) = 0 AND ISNULL(IsVersionIncrease,0) = 0
			...
			BEGIN 
				...
				BEGIN
					...
					SELECT @SalesTotal = SUM(ISNULL(PartCost,0))
					 ...
					 SELECT
					       @InoiceGrandTotal = ISNULL(SOBI.SubTotal,0),
					 ...
					 WHERE SOBI.BillingInvoicingId=@InvoiceId 
					 AND ISNULL(SOBI.IsPerformaInvoice,0) = 0 AND ISNULL(SOBI.[IsVersionIncrease],0) = 0
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
        , @AdhocComments     VARCHAR(150)    = 'USP_BatchTriggerSOInvoiceNewCommon' 
