 ...
 ** --   --------     -------  ------ --------------------------                  
    ...
    3    19/06/2026   Kishor Makwana added ClientId, TenantId and ClientSecret 
 ...
 BEGIN TRANSACTION;  
  ...
  BEGIN  
   ...
   (  
    ...
    [IsActive],[IsDeleted],[AuthTypeId],[ClientId],[TenantId],[ClientSecret]  
   )  
   VALUES  
   (  
    @SmtpUserEmail, @SmtpServer, @SmtpEmailPassword, @SmtpPort, @UseSsl,  
    @MasterCompanyId, @CreatedBy, @CreatedBy, GETUTCDATE(), GETUTCDATE(),  
    1, 0, @AuthTypeId, @ClientId,@TenantId,@ClientSecret  
   );  
