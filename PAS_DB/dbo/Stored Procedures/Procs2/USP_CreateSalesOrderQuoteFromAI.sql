...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SELECT TOP 1 @SalesOrderQuotePrefix = CodeTypeId
            FROM [dbo].[CodeTypes] WITH (NOLOCK)
            WHERE [CodeType] = 'Sales Order Quote';

            -- Get Module for Update in CustomerRFQ table.
            SELECT @SOQModuleId = [ModuleId]
            FROM [dbo].[Module] WITH (NOLOCK)
            WHERE [ModuleName] = 'SalesQuote';
            ...
            WHERE [IsActive] = 1
              AND [IsDeleted] = 0
              AND [CodeTypeId] = @SalesOrderQuotePrefix
              AND [MasterCompanyId] = @MasterCompanyId;
            ...
            -- Fetch Employee Name
            SELECT TOP 1 @EmployeeName = FirstName + ' ' + LastName
            FROM [dbo].[Employee] WITH (NOLOCK)
            WHERE EmployeeId = ISNULL(@EmployeeId, 0);

            -- Fetch Customer Warning Name
            SELECT TOP 1 @CustomerWarningName = WarningMessage
            FROM [dbo].[CustomerWarning] WITH (NOLOCK)
            WHERE CustomerWarningId = ISNULL(@CustomerWarningId, 0);

            SELECT TOP 1 @ManagementStructureName = LastMSLevel
            FROM [dbo].[EmployeeManagementStructureDetails] WITH (NOLOCK)
            WHERE ReferenceID = ISNULL(@EmployeeId, 0);

            DECLARE @StatusId BIGINT;
            SELECT @StatusId = Id FROM [dbo].[MasterSalesOrderStatus] WITH (NOLOCK) WHERE Name = 'Open';

            -- Fetch StatusName
            SELECT TOP 1 @StatusName = Name
            ...
            WHERE ModuleName = 'SalesOrderQuote';
            ...
            BEGIN
                ...
                DECLARE @ConditionId        BIGINT,
                        ...
                        @NetSaleAmount       DECIMAL(18, 2) = 500;  -- Currently fix amount
                ...
                BEGIN
                    ...
                    SELECT @SalesOrderQuotePartId      = SalesOrderQuotePartId,
                           ...
                           @SalesOrderQuoteStocklineId = SalesOrderQuoteStocklineId,
                    ...
                    BEGIN
                        ...
                        SELECT @SOQPartStatus = SOPartStatusId
                        FROM [dbo].[SOPartStatus] WITH (NOLOCK)
                        WHERE [PartStatus] = 'Open';
                    ...
                    BEGIN
                        ...
                        SELECT @ExistingQtyReq = SOP.QtyRequested
