...
@TotalFreight DECIMAL(18,2) OUTPUT,
@TotalCharges DECIMAL(18,2) OUTPUT,
@SubTotal DECIMAL(18,2) OUTPUT,
@WOQGrandTotal DECIMAL(18,2) OUTPUT,
@FinalSalesTaxes DECIMAL(18,2) OUTPUT,
@FinalOtherTaxes DECIMAL(18,2) OUTPUT
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN TRY
   ...
   ---------------------------------Freight--------------------------------------------------------
	...
	FROM #tmprWorkorderquoteDetails	
			...
			SET   @TotalFreight = ISNULL(@TotalFreight,0)
			SET   @TotalCharges =ISNULL(@TotalCharges,0)
	        SET   @SubTotal =ISNULL((@SubTotal),0)
	        SET   @WOQGrandTotal =ISNULL((@SubTotal + @FinalSalesTaxes + @FinalOtherTaxes),0)
			SET   @FinalSalesTaxes =ISNULL(@FinalSalesTaxes,0)
			SET   @FinalOtherTaxes =ISNULL(@FinalOtherTaxes,0)
  ...
  BEGIN CATCH
    DECLARE @ErrorLogID int,
            ...
            @AdhocComments varchar(150) = '[USP_GetCustomerTax_Information_Repair_WOQ_Output]',
