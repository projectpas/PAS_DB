/*************************************************************           
 ** File:		 [USP_LegalEntityInternationalWirePaymentV2]           
 ** Author:		 Rajesh Gami
 ** Description: This Stored Procedure Is Used To Add or Update LegalEntity International Wire Payment.
 ** Purpose:         
 ** Date:   08 OCT 2025 
 **************************************************************           
 ** Change History           
 ...
 ** --   -------------		----------------		--------------------------------          
 ...
 -- EXEC [USP_LegalEntityInternationalWirePaymentV2] 
**************************************************************/
CREATE     PROCEDURE [dbo].[USP_LegalEntityInternationalWirePaymentV2]
...
/***************Start Save LegalEntity International Wire Payment Details***************/
	...
	BEGIN
		...
		INSERT INTO [DBO].[InternationalWirePaymentV2](
        ...
        BEGIN
			IF EXISTS (SELECT 1 FROM [DBO].[LegalEntityInternationalWireBankingV2] WITH(NOLOCK) WHERE [LegalEntityId] = @LegalEntityId AND [IsPrimay] = 1)
			BEGIN
        ...
        BEGIN
            ...
            IF NOT EXISTS (SELECT 1 FROM [DBO].[LegalEntityInternationalWireBankingV2] WITH(NOLOCK) WHERE [LegalEntityId] = @LegalEntityId AND [IsPrimay] = 1)
            BEGIN
                SET @IsPrimay = 1;
            END
        END
		...
		INSERT INTO [DBO].[LegalEntityInternationalWireBankingV2](
...
/***************Start Update LegalEntity International Wire Payment Details***************/
	ELSE
	BEGIN	
		SELECT @InternationalWirePaymentId = [InternationalWirePaymentId] FROM [DBO].[LegalEntityInternationalWireBankingV2] WITH(NOLOCK) WHERE [LegalEntityInternationalWireBankingId] = @LegalEntityInternationalWireBankingId;

        UPDATE [DBO].[InternationalWirePaymentV2]
        SET 
			[SwiftCode] = @SwiftCode,
			[ABA] = @ABA,
		...
		IF (ISNULL(@IsPrimay, 0) = 1)
        BEGIN
			IF EXISTS (SELECT 1 FROM [DBO].[LegalEntityInternationalWireBankingV2] WITH(NOLOCK) WHERE [LegalEntityId] = @LegalEntityId AND [LegalEntityInternationalWireBankingId] != @LegalEntityInternationalWireBankingId AND [IsPrimay] = 1)
			BEGIN
				UPDATE [DBO].[LegalEntityInternationalWireBankingV2]
				SET [IsPrimay] = 0,
					[UpdatedBy] = @UpdatedBy,
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_LegalEntityInternationalWirePaymentV2'
