...
/*************************************************************           
 ** File:   [USP_Lot_AddUpdateMSDetails]           
 ** Author:  Moin Bloch
...
-- EXEC USP_Lot_AddUpdateMSDetails 1,3,1,'moin','moin',42,1,1
************************************************************************/

CREATE   PROCEDURE [dbo].[USP_Lot_AddUpdateMSDetails]
@ReferenceID bigint,
@EntityMSID bigint,
@ModuleId int,
@MasterCompanyId int,
...
BEGIN
	...
	BEGIN  
		...
		IF(@Opr = 1)
		BEGIN
			IF NOT EXISTS(SELECT [ReferenceID] FROM [dbo].[LotManagementStructureDetails] WHERE [ReferenceID] = @ReferenceID AND [ModuleID] = @ModuleId)
			BEGIN
				...
				INSERT INTO [dbo].[LotManagementStructureDetails]
				...
				SELECT @MSDetailsId = IDENT_CURRENT('LotManagementStructureDetails');
				...
				SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[LotManagementStructureDetails] MSD WITH(NOLOCK) 
						WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

				EXECUTE(@Query)  
				
				UPDATE [dbo].[LotManagementStructureDetails] 
			...
			BEGIN
				...
				UPDATE [dbo].[LotManagementStructureDetails]
					...
					FROM [dbo].[LotManagementStructureDetails] MSD WITH(NOLOCK) 
