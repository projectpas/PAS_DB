/*************************************************************               
 ** File:   [USP_SaveSuspenseAndUnAppliedPaymenttMSDetails]               
 ** Author:   Devendra Shekh      
 ** Description: to add managementstructuredetails for SUSPENSE AND UNAPPLIED PAYMENT
 ** Purpose:             
...
CREATE   PROCEDURE [dbo].[USP_SaveSuspenseAndUnAppliedPaymenttMSDetails]  
@ModuleID INT,
@ReferenceID BIGINT,
@EntityMSID BIGINT, 
@MasterCompanyId INT,
		...
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					...
					INSERT INTO [dbo].[SuspenseAndUnAppliedPaymentMSDetails]
					...
					SELECT @MSDetailsId = IDENT_CURRENT('SuspenseAndUnAppliedPaymentMSDetails');

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[SuspenseAndUnAppliedPaymentMSDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

					EXECUTE(@Query)  

					UPDATE [dbo].[SuspenseAndUnAppliedPaymentMSDetails] 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_SaveSuspenseAndUnAppliedPaymenttMSDetails' 
