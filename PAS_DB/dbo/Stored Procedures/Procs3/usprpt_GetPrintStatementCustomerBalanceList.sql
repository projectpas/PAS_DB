 ...
 ** --   --------		-------				--------------------------------          
	...
	8    15-Jan-2025    Moin Bloch          Modify No need to check RemainingAmount when Invoice Status Is ALL PBI  PN-15177
  ...
  SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
    ...
    BEGIN TRY
		...
		BEGIN
			...
			--WHERE WOBI.ModuleId = @workOrderModuleId AND ISNULL(WOBI.isPerformaInvoice, 0) = 1
			...
			INSERT INTO #TEMPInvoiceRecords([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CurrencyCode],
			       [BalanceAmount],[RemainingAmount],[CurrentAmount],[PaymentAmount],[Amountlessthan0days],[Amountlessthan30days],
				   ...
				   [MasterCompanyId],[StatusId],[IsCreditMemo],[InvoicePaidAmount],[ModuleTypeId],[LegalEntityName],[ReceivedAmount], [LegalEntityId])
			...
			FROM [dbo].[BillingInvoicing] WOBI WITH (NOLOCK) 
				...
				LEFT JOIN [dbo].[EntityStructureSetup] ES ON ES.EntityStructureId = WOBI.ManagementStructureId
				LEFT JOIN [dbo].[ManagementStructureLevel] MSL ON ES.Level1Id = MSL.ID
			...
			WHERE ISNULL(WOBI.[IsVersionIncrease],0) = 0 AND WO.[MasterCompanyId] = @MasterCompanyid 
				...
				AND ((@ViewType) = 'ALL' OR ((ISNULL(WOBI.[IsPerformaInvoice],0) = 0 AND ISNULL(WOBI.[RemainingAmount],0) > 0) 
			...
			UPDATE #TEMPInvoiceRecords SET CreditMemoAmount = ISNULL(tmpcm.CMAmount, 0)
					FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
						...
						WHERE CMD.IsWorkOrder = 1 AND CM.CustomerId = TmpInv.CustomerId AND CM.StatusId = @CMPostedStatusId AND ISNULL(CM.IsClosed, 0) = 0
			...
			--	INNER JOIN [dbo].[BillingInvoicingItems] nsobii WITH(NOLOCK) on nsop.SalesOrderPartId = nsobii.SubReferenceId AND ISNULL(nsobii.IsPerformaInvoice, 0) = 1 AND nsobii.ModuleId = @salesOrderModuleId
			...
			--	AND nsobii.SubReferenceId = nsop.SalesOrderPartId 
			--WHERE nsobi.ModuleId = @salesOrderModuleId
			...
			SELECT DISTINCT SOBI.[BillingInvoicingId],
				...
				FROM [dbo].[BillingInvoicing] SOBI WITH (NOLOCK)       
					...
					JOIN [dbo].[EntityStructureSetup] ES ON ES.EntityStructureId = SO.ManagementStructureId
					JOIN [dbo].[ManagementStructureLevel] MSL ON ES.Level1Id = MSL.ID
					JOIN [dbo].[LegalEntity] LE ON MSL.LegalEntityId = LE.LegalEntityId
				WHERE SO.[MasterCompanyId] = @Mastercompanyid
					AND SOBI.[InvoiceStatus] = @InvoiceStatus 
					AND ((@ViewType) = 'ALL' OR ((ISNULL(SOBI.IsPerformaInvoice,0) = 0 AND ISNULL(SOBI.[RemainingAmount],0) > 0) OR (ISNULL(SOBI.IsPerformaInvoice,0) = 1 AND ISNULL(SOBI.DepositAmount, 0) > 0)))
			...
			UPDATE #TEMPInvoiceRecords SET [CreditMemoAmount] = ISNULL(tmpcm.[CMAmount], 0)
				FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
					FROM [dbo].[CreditMemoDetails] CMD WITH (NOLOCK)   
						...
						JOIN #TEMPInvoiceRecords TmpInv ON TmpInv.BillingInvoicingId = SOBII.BillingInvoicingId AND TmpInv.ModuleTypeId = @SOModuleTypeId
		    ...
		    SELECT DISTINCT ESOBI.SOBillingInvoicingId,
							...
							(ESOBI.[RemainingAmount]), -- 'BalanceAmount',
							(ESOBI.[RemainingAmount]), -- '[RemainingAmount]',
				...
				FROM [dbo].[ExchangeSalesOrderBillingInvoicing] ESOBI WITH (NOLOCK)    
							...
							JOIN [dbo].[EntityStructureSetup] ES ON ES.EntityStructureId = ESO.ManagementStructureId
							JOIN [dbo].[ManagementStructureLevel] MSL ON ES.Level1Id = MSL.ID
							JOIN [dbo].[LegalEntity] LE ON MSL.LegalEntityId = LE.LegalEntityId  
				WHERE ESO.[MasterCompanyId] = @Mastercompanyid 
					--AND ISNULL(ESOBI.[RemainingAmount],0) > 0 
					AND ((@ViewType) = 'ALL' OR ISNULL(ESOBI.[RemainingAmount],0) > 0 )
			...
			UPDATE #TEMPInvoiceRecords SET [CreditMemoAmount] = ISNULL(tmpcm.[CMAmount], 0)
				FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
					...
					WHERE CM.InvoiceTypeId = @EXInvoiceTypeId AND CM.[CustomerId] = TmpInv.CustomerId AND CM.[StatusId] = @CMPostedStatusId AND ISNULL(CM.IsClosed, 0) = 0
					GROUP BY CMD.BillingInvoicingItemId, TmpInv.BillingInvoicingId  
			) tmpcm WHERE tmpcm.BillingInvoicingId = #TEMPInvoiceRecords.BillingInvoicingId
			...
			SELECT DISTINCT CM.[CreditMemoHeaderId],
					        ...
					        UPPER(C.[CustomerCode]),	
							CR.Code,
							CM.Amount, --  '[BalanceAmount]',
							CM.[Amount], --  'RemainingAmount',
							0,
							0,
							CM.Amount,
							0,
			...
			WHERE CM.[MasterCompanyId] = @Mastercompanyid      
				AND CM.[IsStandAloneCM] = 1 AND ISNULL(CM.IsClosed, 0) = 0           
				AND CM.[StatusId] = @CMPostedStatusId
				AND CM.[CustomerId] = ISNULL(@CustomerId, CM.CustomerId)
			...
			SELECT DISTINCT MJH.[ManualJournalHeaderId],
							...
							CR.Code,
							ISNULL(SUM(MJD.[Debit]),0) - ISNULL(SUM(MJD.[Credit]),0), --[BalanceAmount]
							ISNULL(SUM(MJD.[Debit]),0) - ISNULL(SUM(MJD.[Credit]),0), --[RemainingAmount]
							0,
							0,
			...
			SELECT DISTINCT CCP.[CustomerCreditPaymentDetailId],
							...
							0,
							0,
							CCP.[RemainingAmount] * (-1), --  'InvoiceAmount', 
							0, -- 'CMAmount', 
							0, -- 'CreditMemoAmount',
							0, --'CreditMemoUsed'	
							CCP.[MasterCompanyId],
			...
			SELECT  [CustomerId],
					...
					ISNULL(SUM([RemainingAmount]),0) [RemainingAmount],
					ISNULL(SUM([BalanceAmount]),0) [BalanceAmount],
					ISNULL(SUM([BalanceAmount]),0) - ISNULL(SUM([CreditMemoAmount]),0) [CurrentAmount],
			...
			GROUP BY [CustomerId],[CustomerName],[CustomerCode],[LegalEntityName],[StatusId],[CurrencyCode] ,[IsCreditMemo],[CurrencyCode],[LegalEntityId]
			...
			INSERT INTO #TEMPFinalResilts(CustomerId, CustName, CustomerCode, CurrencyCode, BalanceAmount, RemainingAmount, CurrentAmount, CreditMemoAmount,
					Amountpaidbylessthen0days, Amountpaidby30days, Amountpaidby60days, Amountpaidby90days, Amountpaidby120days,
					Amountpaidbymorethan120days, LegelEntity, [LegalEntityId])
			SELECT DISTINCT  CustomerId, CustName, CustomerCode, CurrencyCode, 
					...
					ISNULL(SUM([Amountpaidby90days]),0), 
					ISNULL(SUM([Amountpaidby120days]),0),
					ISNULL(SUM([Amountpaidbymorethan120days]),0), 
					LegelEntity,
					[LegalEntityId]
			FROM #TempResult1
			GROUP BY CustomerId, CustName, CustomerCode, CurrencyCode, LegelEntity, [LegalEntityId]
					 
			--Select * from #TEMPFinalResilts
			...
			SELECT @Count = COUNT(CustomerId),
				   ...
				   @TotalAmountmorethan120days = ISNULL(SUM([Amountpaidbymorethan120days]),0) FROM #TEMPFinalResilts;
