...
BEGIN  
      ...
      (SELECT 
       ...
       FROM 
		...
		SELECT * INTO #TempWOOperatingFinal FROM
		 (SELECT (CASE WHEN (SELECT TOP 1 Row_Number FROM #TempWOOperating tm WHERE tm.ItemMasterId = main.ItemMasterId ORDER BY Row_Number DESC) > 1 THEN (SELECT TOP 1 workscopes FROM #TempWOOperating tm WHERE tm.ItemMasterId = main.ItemMasterId ORDER BY Row_Number DESC) ELSE workscopes END) AS 'workscope',* FROM #TempWOOperating main) as res
  ...
  BEGIN CATCH  
    ...
    DECLARE @ErrorLogID int,  
            ...
            @AdhocComments varchar(150) = '[dbo.usprpt_GetWOOperatingMetricReport_LMarginUnitUnit]',  
