-- ===== PROCEDURE: [dbo].[usprpt_GetWorkOrderQuotesReport]   (file: _PAS_DB/PAS_DB/dbo/Stored Procedures/Procs3/usprpt_GetWorkOrderQuotesReport.sql) =====
/*************************************************************               
...
BEGIN    
  ...
  then filterby.value('(FieldValue/text())[1]','VARCHAR(100)') else @level10 end  
   ...
   ;WITH rptCTE (TotalRecordsCount, pn, pndescription, sernum, workscope, qtenum, vernum, quotestatus, qtedate, revenue, directcost, marginamt, marginperc, customer, custcode,   
     contact, email, mobilephone, level1, level2, level3, level4, level5, level6, level7, level8, level9, level10, salesperson, csr, masterCompanyId,OrderByDate) AS (  
   ...
   ISNULL(ISNULL(WOQD.MaterialFlatBillingAmount + WOQD.LaborFlatBillingAmount + WOQD.ChargesFlatBillingAmount,0) ,0) END 'revenue',  
  
   CASE WHEN ISNULL(WOQD.QuoteMethod, 0) = 1 THEN 0 ELSE  
   ISNULL(ISNULL(WOQD.MaterialCost + WOQD.laborcost + WOQD.ChargesCost,0),0) END 'directcost',  
  
   CASE WHEN ISNULL(WOQD.QuoteMethod, 0) = 1 THEN 0 ELSE  
   ISNULL(ISNULL(WOQD.MaterialFlatBillingAmount + WOQD.LaborFlatBillingAmount + WOQD.ChargesFlatBillingAmount,0) -   
   ISNULL(WOQD.MaterialCost + WOQD.laborcost + WOQD.ChargesCost,0),0) END 'marginamt',  
  
   CASE WHEN ISNULL(WOQD.QuoteMethod, 0) = 1 THEN 0 ELSE  
   ISNULL(ISNULL((ISNULL(WOQD.MaterialFlatBillingAmount + WOQD.LaborFlatBillingAmount + WOQD.ChargesFlatBillingAmount, 0) -   
   ...
   WOQD.ChargesFlatBillingAmount, 0),0),0)  END 'marginperc',  
   ...
   WO.MasterCompanyId  ,
   FORMAT(WOQ.OpenDate, 'MM/dd/yyyy') AS 'OrderByDate'
      FROM DBO.WorkOrder WO WITH (NOLOCK)  
        INNER JOIN DBO.WorkOrderQuote WOQ WITH (NOLOCK) ON WO.WorkOrderId = WOQ.WorkOrderId  
  INNER JOIN DBO.WorkOrderPartNumber WOPN WITH (NOLOCK) ON WO.WorkOrderId = WOPN.WorkOrderId  
  INNER JOIN dbo.WorkOrderManagementStructureDetails MSD WITH (NOLOCK) ON MSD.ModuleID = 12 AND MSD.ReferenceID = WOPN.ID  
  LEFT JOIN DBO.EntityStructureSetup ES ON ES.EntityStructureId=MSD.EntityMSID  
   ...
   (ISNULL(@tagtype,'')='' OR ES.OrganizationTagTypeId IN(SELECT value FROM String_split(ISNULL(@tagtype,''), ',')))  
    ...
    ,FinalCTE(TotalRecordsCount, pn, pndescription, sernum, workscope, qtenum, vernum, quotestatus, qtedate, revenue, directcost, marginamt, marginperc, customer, custcode,   
     contact, email, mobilephone, level1, level2, level3, level4, level5, level6, level7, level8, level9, level10, salesperson, csr, masterCompanyId ,OrderByDate)   
     AS (SELECT DISTINCT TotalRecordsCount, pn, pndescription, sernum, workscope, qtenum, vernum, quotestatus, qtedate, revenue, directcost, marginamt, marginperc, customer, custcode,   
     contact, email, mobilephone, level1, level2, level3, level4, level5, level6, level7, level8, level9, level10, salesperson, csr, masterCompanyId ,OrderByDate FROM rptCTE)  
  
   ,WithTotal (masterCompanyId, TotalRevenue, TotalDirectCost, TotalMarginAmt, TotalMarginAmtPerc)   
     AS (SELECT masterCompanyId,   
    FORMAT(SUM(revenue), 'N', 'en-us') TotalRevenue,  
    FORMAT(SUM(directcost), 'N', 'en-us') TotalDirectCost,  
    FORMAT(SUM(marginamt), 'N', 'en-us') TotalMarginAmt,  
    FORMAT(SUM(marginperc), 'N', 'en-us') TotalMarginAmtPerc  
    FROM FinalCTE  
    GROUP BY masterCompanyId)  
  
  
   ,ForPerc(masterCompanyId, TotalRevenue, TotalDirectCost, TotalMarginAmt, TotalMarginAmtPerc)  
    AS (SELECT masterCompanyId,   
    TotalRevenue,  
    TotalDirectCost,  
    TotalMarginAmt,  
    ...
    GROUP BY masterCompanyId, TotalRevenue, TotalDirectCost, TotalMarginAmt, TotalMarginAmtPerc)  
     ...
     SELECT COUNT(2) OVER () AS TotalRecordsCount,  pn, pndescription, sernum, workscope, qtenum, vernum, quotestatus, qtedate,  
     ...
     customer, custcode, contact, email, mobilephone, level1, level2, level3, level4, level5, level6, level7, level8, level9, level10, salesperson, csr,  
     WC.TotalRevenue,  
     WC.TotalDirectCost,  
     WC.TotalMarginAmt,  
