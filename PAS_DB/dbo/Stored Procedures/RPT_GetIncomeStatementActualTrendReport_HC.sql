...
EXEC [RPT_GetIncomeStatementActualTrendReport] 1, 8, 1, 128,128
************************************************************************/
CREATE PROCEDURE [dbo].[RPT_GetIncomeStatementActualTrendReport_HC] (@masterCompanyId bigint,
  ...
  BEGIN TRY
    ...
    SELECT
      ...
      @LegalEntityId = LegalEntityId
			...
			WHERE AccountingCalendarId = @StartAccountingPeriodId
			AND IsDeleted = 0
    SELECT
      @TODATE = ToDate
			FROM dbo.AccountingCalendar WITH (NOLOCK)
			...
			AND IsDeleted = 0
    SELECT
		  @PostedBatchStatusId = Id
		  FROM dbo.BatchStatus WITH (NOLOCK)
		  WHERE [Name] = 'Posted' -- For Posted Batch Details Only

    SELECT
          @ManualJournalStatusId = ManualJournalStatusId
		  FROM dbo.ManualJournalStatus WITH (NOLOCK)
          WHERE [Name] = 'Posted' -- For Posted Manual Batch Details Only
    ...
    CREATE TABLE ##AccTrendTablePivot (
      ...
      IsBlankHeader bit DEFAULT 0,
      IsTotlaLine bit DEFAULT 0, ' + STUFF((SELECT
      ',' + QUOTENAME(PeriodName) + ' DECIMAL(18,2) NULL'
							FROM #AccPeriodTable
    ...
    BEGIN
      ...
      BEGIN
			...
			IF EXISTS (SELECT TOP 1 ID FROM #TempTable 
											WHERE IsProcess = 0
											AND AccountingPeriodId = @AccountcalID)
			BEGIN
				SELECT TOP 1
					   ...
					   @CLID = LeafNodeId
							  FROM #TempTable
							  WHERE IsProcess = 0
							  AND AccountingPeriodId = @AccountcalID ORDER BY ID
        END
      END
      ...
      UPDATE #TempTable
					  ...
					  FROM dbo.LeafNode L WITH (NOLOCK)
								  ...
								  JOIN dbo.ManualJournalHeader MJH WITH (NOLOCK)
									ON MJH.ManualJournalHeaderId = MJD.ManualJournalHeaderId
								  JOIN dbo.AccountingCalendar AC WITH (NOLOCK)
									ON MJH.AccountingPeriodId = AC.AccountingCalendarId
								  ...
								  GROUP BY MJD.GlAccountId,
										   MJH.AccountingPeriodId,
										   AC.PeriodName) UN
								  GROUP BY UN.GlAccountId,
										   UN.AccountingPeriodId,
										   UN.PeriodName,
										   UN.AccountingPeriod) CBD
								  WHERE L.ReportingStructureId = @ReportingStructureId
								  AND L.IsDeleted = 0
								  AND L.MasterCompanyId = @masterCompanyId

      UPDATE #TempTable
				  SET ChildCount = ISNULL((SELECT
						COUNT(ISNULL(T.Amount, 0))
					  FROM #TempTable T
					  WHERE T.ParentId = T1.LeafNodeId
					  AND T.AccountingPeriodId = @AccountcalID), 0),
					  Amount =
							  CASE
								WHEN T1.IsPositive = 1 THEN Amount
      ...
      UPDATE #TempTable
			  SET IsProcess = 0
			  WHERE AccountingPeriodId = @AccountcalID
      ...
      BEGIN
		...
		UPDATE #TempTable
					SET Amount =    CASE WHEN IsPositive = 1 THEN (SELECT  SUM(ISNULL(T.Amount, 0))
																		FROM #TempTable T
																		WHERE T.ParentId = @CLID
																		AND T.AccountingPeriodId = @AccountcalID)
										  ELSE ISNULL((SELECT SUM(ISNULL(T.Amount, 0))
															FROM #TempTable T
															WHERE T.ParentId = @CLID
															AND T.AccountingPeriodId = @AccountcalID)
															, 0) * -1
      ...
      BEGIN
        ...
        BEGIN
				INSERT INTO #AccTrendTable (LeafNodeId, NodeName, Amount, AccountingPeriodId, IsBlankHeader)
							SELECT TOP 1 LeafNodeId,  Name + ' :', NULL,@AccountcalID, 1
										FROM #TempTable
											WHERE LeafNodeId = @CLID												
											      AND AccountingPeriodId = @AccountcalID
				IF (@IsFristRow = 1)
				  BEGIN
					INSERT INTO ##AccTrendTablePivot (Name, IsBlankHeader, LeafNodeId, ParentId)
					  SELECT TOP 1	Name + ' :',1,LeafNodeId,ParentId
								  FROM #TempTable
								  WHERE LeafNodeId = @CLID								  
								  AND AccountingPeriodId = @AccountcalID
				  END
        ...
        INSERT INTO #AccTrendTable (LeafNodeId, NodeName, Amount, AccountingPeriodId, IsBlankHeader)
          SELECT LeafNodeId, Name, Amount,  @AccountcalID, 0
				       FROM #TempTable
						  WHERE ID = @CID
						     AND AccountingPeriodId = @AccountcalID  --                    AND ChildCount = 0
      
		IF (@IsFristRow = 1)
        BEGIN
          INSERT INTO ##AccTrendTablePivot (Name, IsBlankHeader, LeafNodeId, ParentId)
            SELECT Name,  0, LeafNodeId,    ParentId
						FROM #TempTable  WHERE ID = @CID   AND AccountingPeriodId = @AccountcalID  --                    AND ChildCount = 0
        END
        ...
        IF NOT EXISTS (SELECT TOP 1 ID  
		                     FROM #TempTable WHERE ParentId = @CLID AND IsProcess = 0
														AND AccountingPeriodId = @AccountcalID)
        BEGIN
			IF NOT EXISTS (SELECT TOP 1 ID FROM #AccTrendTable  WHERE LeafNodeId = @CLID
																	 AND IsTotlaLine = 1
												AND AccountingPeriodId = @AccountcalID)
			BEGIN
            INSERT INTO #AccTrendTable (LeafNodeId, NodeName, Amount, AccountingPeriodId, IsBlankHeader, IsTotlaLine)
              SELECT   LeafNodeId,  'Total - ' + Name,  TotalAmount,   @AccountcalID, 0, 1  FROM #TempTable
										  WHERE LeafNodeId = @CLID										  
													 AND AccountingPeriodId = @AccountcalID
            IF (@IsFristRow = 1)
            BEGIN
        ...
        BEGIN
          SELECT TOP 1  @CID = ID  FROM #TempTable  WHERE IsProcess = 0
								  AND AccountingPeriodId = @AccountcalID
								  ORDER BY ID 
        END
    ...
    UPDATE #TempTable
    SET AccountingPeriodName = REPLACE(AP.PeriodName, ' - ', ' ')
    FROM #TempTable tmp
    JOIN dbo.AccountingCalendar AP WITH (NOLOCK)
      ON tmp.AccountingPeriodId = AP.AccountingCalendarId

    UPDATE #AccTrendTable
    SET AccountingPeriod =
                          CASE
                            WHEN ISNULL(AP.PeriodName, '') != '' THEN REPLACE(AP.PeriodName, ' - ', ' ')
                            ELSE 'Total'
                          END
    FROM #AccTrendTable tmp
    LEFT JOIN dbo.AccountingCalendar AP WITH (NOLOCK)
      ON tmp.AccountingPeriodId = AP.AccountingCalendarId
    ...
    FROM (SELECT
      ...
      NodeName
    FROM #AccTrendTable act
    WHERE AccountingPeriod != 'Total'
    GROUP BY NodeName) Groptoal
    WHERE Groptoal.NodeName = #AccTrendTable.NodeName
    AND AccountingPeriod = 'Total'
    ...
    INSERT INTO ##tmpFinalReturnTable (leafNodeId, [name], parentId, parentNodeName, isLeafNode, masterCompanyId, reportingStructureId, isPositive, sequenceNumber)
      SELECT
        ...
        LF.IsLeafNode,
        LF.MasterCompanyId,
        LF.ReportingStructureId,
        LF.IsPositive,
        LF.SequenceNumber
      FROM dbo.LeafNode LF WITH (NOLOCK)
      LEFT JOIN dbo.GLAccountLeafNodeMapping GLM WITH (NOLOCK)
        ON LF.LeafNodeId = GLM.LeafNodeId
      LEFT JOIN dbo.GLAccount GL WITH (NOLOCK)
        ON GL.GLAccountId = GLM.GLAccountId
      LEFT JOIN dbo.LeafNode LFP WITH (NOLOCK)
        ON LF.ParentId = LFP.LeafNodeId
      WHERE LF.IsActive = 1
      AND LF.IsDeleted = 0
      AND LF.MasterCompanyId = @masterCompanyId
      AND LF.ReportingStructureId = @ReportingStructureId
      GROUP BY LF.LeafNodeId,
               LF.Name,
               LF.ParentId,
               LFP.Name,
               LF.IsLeafNode,
    ...
    UPDATE T1
    SET T1.IsLeafNode = T2.IsLeafNode,
        T1.MasterCompanyId = T2.MasterCompanyId,
        T1.ReportingStructureId = T2.ReportingStructureId,
        T1.sequenceNumber = T2.sequenceNumber,
        T1.IsPositive = T2.IsPositive,
        T1.parentNodeName = T2.parentNodeName
    FROM ##AccTrendTablePivot T1
    JOIN ##tmpFinalReturnTable T2
      ON T1.leafNodeId = T2.leafNodeId
    ...
    JOIN ##tmpFinalReturnTable T2
      ON T1.leafNodeId = T2.leafNodeId
      AND T1.parentId = T2.parentId --AND T1.[name] = T2.[name]
    WHERE T1.parentNodeName IS NULL

    UPDATE ##AccTrendTablePivot
    SET leafNodeId = NULL
    WHERE IsBlankHeader = 1
    OR IsTotlaLine = 1
    ...
    BEGIN
      ...
      UPDATE ##AccTrendTablePivot
      SET leafNodeId = @MAXLeafNodeId
      WHERE ID = @COUNT2
      AND leafNodeId IS NULL
      AND (IsBlankHeader = 1
      OR IsTotlaLine = 1)
    ...
    BEGIN
      ...
      IF ((SELECT
          COUNT(1)
        FROM ##AccTrendTablePivot
        WHERE ISNULL(parentId, 0) IN (ISNULL(@LeafNodeId3, 0)))
        > 0)
      BEGIN
