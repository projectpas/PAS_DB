...
BEGIN
	...
	BEGIN TRY		
	  ...
	  BEGIN
			SET @StatusID = NULL			
      END 	  	      
     ;WITH Result AS(SELECT CM.[CreditMemoHeaderId]
                           ,CM.[CreditMemoNumber]
                           ,CM.[RMAHeaderId]
                           ,CM.[RMANumber]
                           ,CM.[InvoiceId]
        ...
        WHERE ((CM.MasterCompanyId = @MasterCompanyId) AND (CM.IsDeleted = @IsDeleted) AND (@StatusID IS NULL OR CM.StatusId = @StatusID))		
			...
			PartCTE AS(  
			 ...
			 PartDescCTE AS(  
			  ...
			  Results AS( SELECT M.[CreditMemoHeaderId],M.[CreditMemoNumber],M.[RMAHeaderId],M.[RMANumber],M.[InvoiceId],M.[InvoiceNumber],
