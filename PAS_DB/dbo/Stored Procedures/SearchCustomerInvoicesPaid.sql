...
BEGIN
	...
	BEGIN TRY
			 SELECT [IP].PaymentId, 
					...
					DATEDIFF(DAY, [IP].InvoiceDate, GETDATE()) AS 'DSI',					
					(CT.NetDays - DATEDIFF(DAY, CASt([IP].InvoiceDate as date), GETDATE())) AS 'DSO',
		...
		WHERE [IP].CustomerId = @customerId AND [IP].ReceiptId = @receiptId
