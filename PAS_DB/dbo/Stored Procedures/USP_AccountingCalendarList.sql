	...
	BEGIN
		...
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					;With Result AS(
							SELECT Name,FiscalYear, max(AccountingCalendarId) as AccountingCalendarId							
							from AccountingCalendar WITH(NOLOCK)
							WHERE IsDeleted =0 AND IsActive=1 AND MasterCompanyId=@MasterCompanyId
							GROUP BY Name,FiscalYear
					),
						FinalResult AS(
					Select RS.AccountingCalendarId ,RS.Name,Description,FiscalName,RS.FiscalYear,
					Quater,Period,FromDate,ToDate,PeriodName,Notes,MasterCompanyId,CreatedBy,UpdatedBy,
					CreatedDate,UpdatedDate,IsActive,IsDeleted,Status,LegalEntityId,isUpdate,IsAdjustPeriod,
					NoOfPeriods,PeriodType,LegalEntityIds,
					...
					inner join AccountingCalendar as AC WITH(NOLOCK) on  AC.AccountingCalendarId=RS.AccountingCalendarId
					WHERE AC.IsDeleted =0 AND AC.IsActive=1 AND AC.MasterCompanyId=@MasterCompanyId
					),
