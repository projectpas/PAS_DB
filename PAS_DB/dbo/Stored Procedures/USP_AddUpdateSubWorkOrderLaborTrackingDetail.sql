...
**************************************************************/
Create           PROCEDURE [dbo].[USP_AddUpdateSubWorkOrderLaborTrackingDetail]
 @WorkFlowWOId bigint,
 @TaskId bigint,
 @EmployeeId bigint,
	...
	SET NOCOUNT ON;
		...
		BEGIN TRANSACTION
			BEGIN  
				...
				DECLARE @totalHours int = 0 , @totalMinutes decimal(10,2), @laborTaskStatus varchar(50)
				...
				SELECT top 1  @AdjustedHours=Isnull(Adjustments,0),@woLaborId = ISNULL(SubWorkOrderLaborId,0) , @laborTaskStatus = (SELECT Top 1 [Description] FROM dbo.TaskStatus TS WHERE TS.TaskStatusId = WL.TaskStatusId)
				...
				WHERE WL.TaskId = @TaskId AND WL.EmployeeId = @EmployeeId AND WLH.SubWOPartNoId = @WorkFlowWOId

				SELECT Top 1 @woLaborTrackingId = ISNULL(SubWorkOrderLaborTrackingId,0) ,@scanStartTime = StartTime FROM dbo.SubWorkOrderLaborTracking WITH(NOLOCK) where TaskId = @TaskId AND EmployeeId = @EmployeeId AND ISNULL(IsCompleted,0) = 0 AND SubWorkOrderLaborId = @woLaborId
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								Update [dbo].SubWorkOrderLaborTracking 
								 ...
								 WHERE SubWorkOrderLaborTrackingId = @woLaborTrackingId
								 		print 'as'
						...
						BEGIN
							...
							BEGIN
								...
								BEGIN
									Declare @TotalRunnigTask int = (SELECT COUNT(SubWorkOrderLaborTrackingId) FROM dbo.SubWorkOrderLaborTracking WHERE EmployeeId = @EmployeeId AND ISNULL(IsCompleted,0) = 0)
									
									WHILE (@TotalRunnigTask >0)
									BEGIN
										SELECT Top 1 @runningLaborId=SubWorkOrderLaborId, @woLaborTrackingId = ISNULL(SubWorkOrderLaborTrackingId,0),@runnigTaskId= TaskId ,@scanStartTime = StartTime FROM dbo.SubWorkOrderLaborTracking WITH(NOLOCK) where EmployeeId = @EmployeeId AND ISNULL(IsCompleted,0) = 0
										...
										FROM dbo.SubWorkOrderLabor WL WITH(NOLOCK) WHERE SubWorkOrderLaborId = @runningLaborId;
										...
										Update [dbo].SubWorkOrderLaborTracking 
										 ...
										 SELECT @totalHours =  SUM(ISNULL(TotalHours,0)), @totalMinutes = SUM(ISNULL(TotalMinutes,0)) FROM [dbo].[WorkOrderLaborTracking] WHERE WorkOrderLaborId =@runningLaborId AND TaskId = @runnigTaskId AND EmployeeId = @EmployeeId 
											...
											UPDATE WL set [Hours]= @totalMainHours , AdjustedHours = @FinalAdjustedHours,
											  ...
											  LEFT JOIN dbo.[Percent] P WITH(NOLOCK) 
													on Wl.BurdaenRatePercentageId = P.PercentId 
											  WHERE  Wl.SubWorkOrderLaborId = @runningLaborId
							...
							VALUES
							  ...
							  Declare @CurrentTaskStatusId int =0
							   ...
							   select @CurrentTaskStatusId=TaskStatusId from SubWorkOrderLabor  WL  WITH(NOLOCK)  WHERE  Wl.SubWorkOrderLaborId = @woLaborId
