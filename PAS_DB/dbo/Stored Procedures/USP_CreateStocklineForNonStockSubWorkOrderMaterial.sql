/*************************************************************
 ** File:   [USP_CreateStocklineForNonStockSubWorkOrderMaterial]
 ** Author:   Rajesh Gami
 ** Description: This stored procedure is used to create a Stockline for a Non-Stock Sub Work Order Material.
                  Modeled directly on [USP_CreateStocklineForNosStockSalesOrderPart] (the Sales Order
                  Non-Stock equivalent) - same target column list/order, same defaults - only the source
                  of a handful of values differs (Work Order Material instead of Sales Order Part):
                    - Quantity/QuantityOnHand/QuantityAvailable come from @Quantity (the WO Material's own
                      Quantity, passed in by the caller - WorkOrderMaterials.Quantity), not a re-queried value.
                    - ConditionId is sourced from @ConditionCodeId (WorkOrderMaterials.ConditionCodeId).
                    - ManagementStructureId/LegalEntityId are derived via WorkOrder.CustomerId ->
                      Customer.ManagementStructureId -> ManagementStructure.LegalEntityId, since
                      dbo.WorkOrder itself has no ManagementStructureId column (unlike dbo.SalesOrder).
                    - WorkOrderId/SubWorkOrderId/SubWorkOrderMaterialsId are populated instead of
                      SalesOrderPartId (left NULL). Stockline.WorkOrderMaterialsId is also left NULL for this
                      SP (this row belongs to a SubWorkOrderMaterials, not a WorkOrderMaterials row) - the
                      @WorkOrderMaterialsId parameter below is unused/vestigial from the WO-version template
                      and intentionally not referenced in the body; kept only for signature parity, safe to
                      ignore/omit when calling.
 ** Purpose:
 ** Date: 25-Aug-2026
 ...
 ** --   --------     -------		--------------------------------
    1    25-Aug-2026	  RAJESH GAMI	[PN-17782] Created - allows Non-Stock parts added as Sub Work
									  Order Materials to get a real dbo.Stockline row (mirroring how
									  USP_CreateStocklineForNosStockSalesOrderPart already does this for
									  Sales Order Non-Stock parts), so they can flow through Reserve/Issue/
									  Pick Ticket/Unreserve/Unissue like Stock materials do. Called from
									  USP_CreateSubWorkOrderMaterials when a material's ItemMaster.IsNonStock = 1
									  and no existing @WOMStockLineId was supplied by the caller.

************************************************************************/
CREATE PROCEDURE [dbo].[USP_CreateStocklineForNonStockSubWorkOrderMaterial]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @PartNumber VARCHAR(50),@ManufacturerId BIGINT, @ConditionId BIGINT,@SerialNumber VARCHAR(30) = ''
		...
		DECLARE @Memo NVARCHAR(MAX) ='Stockline Created For Sub Work Order Non-Stock Item'
		...
		SET @ConditionId = @ConditionCodeId;

		SELECT @CustomerId = WO.[CustomerId] FROM [dbo].[WorkOrder] WO WITH(NOLOCK) WHERE [WorkOrderId] = @WorkOrderId

		SELECT @ManagementStructureId = C.[ManagementStructureId] FROM [dbo].[Customer] C WITH(NOLOCK) WHERE C.[CustomerId] = @CustomerId
		...
		WHERE ITM.[ItemMasterId] = @ItemMasterId;
	...
	BEGIN CATCH
		...
		DECLARE @ErrorLogID INT,
				...
				,@AdhocComments VARCHAR(150) = 'USP_CreateStocklineForNonStockSubWorkOrderMaterial'
				,@ProcedureParameters VARCHAR(3000) = '@WorkOrderId = ''' + ISNULL(CAST(@WorkOrderId AS VARCHAR(20)), '') + ''', @SubWorkOrderId = ''' + ISNULL(CAST(@SubWorkOrderId AS VARCHAR(20)), '') + ''', @SubWorkOrderMaterialsId = ''' + ISNULL(CAST(@SubWorkOrderMaterialsId AS VARCHAR(20)), '') + ''', @ItemMasterId = ''' + ISNULL(CAST(@ItemMasterId AS VARCHAR(20)), '') + ''''
