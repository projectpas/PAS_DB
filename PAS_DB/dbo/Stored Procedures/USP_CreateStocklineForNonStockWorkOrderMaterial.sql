/*************************************************************
 ** File:   [USP_CreateStocklineForNonStockWorkOrderMaterial]
 ** Author:   Rajesh Gami
 ** Description: This stored procedure is used to create a Stockline for a Non-Stock Work Order Material.
                  ...
                  of a handful of values differs (Work Order Material instead of Sales Order Part):
                    ...
                    - WorkOrderId/WorkOrderMaterialsId are populated instead of SalesOrderPartId (left NULL).
 ** Purpose:
 ** Date: 25-Aug-2026
 ...
 ** --   --------     -------		--------------------------------
    1    25-Aug-2026	  RAJESH GAMI	[PN-17782] Created - allows Non-Stock parts added as Work
									  ...
									  USP_CreateWorkOrderMaterials when a material's ItemMaster.IsNonStock = 1
									  and no existing @WOMStockLineId was supplied by the caller.

************************************************************************/
CREATE PROCEDURE [dbo].[USP_CreateStocklineForNonStockWorkOrderMaterial]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @Memo NVARCHAR(MAX) ='Stockline Created For Work Order Non-Stock Item'
	...
	BEGIN CATCH
		...
		DECLARE @ErrorLogID INT,
				...
				,@AdhocComments VARCHAR(150) = 'USP_CreateStocklineForNonStockWorkOrderMaterial'
				,@ProcedureParameters VARCHAR(3000) = '@WorkOrderId = ''' + ISNULL(CAST(@WorkOrderId AS VARCHAR(20)), '') + ''', @WorkOrderMaterialsId = ''' + ISNULL(CAST(@WorkOrderMaterialsId AS VARCHAR(20)), '') + ''', @ItemMasterId = ''' + ISNULL(CAST(@ItemMasterId AS VARCHAR(20)), '') + ''''
