...
BEGIN
	...
	BEGIN TRY
	  ...
	  SELECT COUNT(1) OVER () AS NumberOfItems  
		...
		(CAST(TotalDebit AS NVARCHAR(10)) LIKE '%' +@GlobalFilter+'%') OR	
		(CAST(TotalCredit AS NVARCHAR(10)) LIKE '%' +@GlobalFilter+'%') OR	
		(CAST(TotalBalance AS NVARCHAR(10)) LIKE '%' +@GlobalFilter+'%') OR	
		...
		(IsNull(@TotalDebit,'') ='' OR TotalDebit= @TotalDebit) AND 
		(IsNull(@TotalCredit,'') ='' OR TotalCredit = @TotalCredit) AND  
		(IsNull(@TotalBalance,'') ='' OR TotalBalance = @TotalBalance) AND 
		(IsNull(@CreatedBy,'') ='' OR CreatedBy like '%' + @CreatedBy+'%')  
        )
		)  
        ORDER BY    
        ...
        CASE WHEN (@SortOrder=1 and @SortColumn='TotalDebit')  THEN TotalDebit END ASC,  
        CASE WHEN (@SortOrder=1 and @SortColumn='TotalCredit')  THEN TotalCredit END ASC,  
        CASE WHEN (@SortOrder=1 and @SortColumn='TotalBalance')  THEN TotalBalance END ASC,  
        ...
        CASE WHEN (@SortOrder=-1 and @SortColumn='TotalDebit')  THEN TotalDebit END Desc,  
        CASE WHEN (@SortOrder=-1 and @SortColumn='TotalCredit')  THEN TotalCredit END Desc,  
        CASE WHEN (@SortOrder=-1 and @SortColumn='TotalBalance')  THEN TotalBalance END Desc,  
