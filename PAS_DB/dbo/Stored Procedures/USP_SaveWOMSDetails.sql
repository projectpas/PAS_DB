...
CREATE PROCEDURE [dbo].[USP_SaveWOMSDetails]    
(    
@ModuleID INT,
@ReferenceID BIGINT,
@EntityMSID BIGINT, 
		...
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					...
					INSERT INTO [dbo].[WorkOrderManagementStructureDetails]
					...
					SELECT @MSDetailsId = IDENT_CURRENT('WorkOrderManagementStructureDetails');

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[WorkOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

					EXECUTE(@Query)  

					UPDATE [dbo].[WorkOrderManagementStructureDetails] 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_SaveWOMSDetails' 
