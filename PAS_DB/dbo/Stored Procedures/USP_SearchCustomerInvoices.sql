...
BEGIN
	...
	BEGIN
				...
				WQ.VersionNo [VersionNo],WQ.QuoteNumber,
				IsWorkOrder=1,
				MSD.LastMSLevel,
				MSD.AllMSlevels,
			...
			PartCTE AS(  
				...
				PT.PartNumberType,PD.PartDescriptionType,SC.StockType,SC.StocktypeType,
				M.VersionNo,M.QuoteNumber,M.LastMSLevel,M.AllMSlevels	,
				CR.CustomerReference,CR.CustomerReferenceType,SRC.SerialNumber,SRC.SerialNumberType,M.IsWorkOrder, M.ReferenceId)
			,SOResult AS(
			SELECT SOBI.SOBillingInvoicingId [InvoicingId],SOBI.InvoiceNo [InvoiceNo],
				...
				SQ.VersionNumber [VersionNo],SQ.SalesOrderQuoteNumber [QuoteNumber],
				IsWorkOrder=0,
				SMS.LastMSLevel,SMS.AllMSlevels, SOBI.SalesOrderId AS [ReferenceId]
			...
			SOPartCTE AS(  
				...
				M.CustomerName,M.CustomerType,M.InvoiceAmt,PT.PN [PN],PD.PNDescription [PNDescription],
				PT.PartNumberType,PD.PartDescriptionType,SC.StockType,SC.StocktypeType,
				M.VersionNo,M.QuoteNumber,M.LastMSLevel,M.AllMSlevels, M.ReferenceId, 
				CR.CustomerReference,ISNULL(SRC.SerialNumber,'') [SerialNumber],M.IsWorkOrder,CR.CustomerReferenceType,SRC.SerialNumberType
				from SOResult M   
				...
				PT.PartNumberType,PD.PartDescriptionType,SC.StockType,SC.StocktypeType,
				M.VersionNo,M.QuoteNumber,M.LastMSLevel,M.AllMSlevels, M.ReferenceId, 
				CR.CustomerReference,ISNULL(SRC.SerialNumber,''),M.IsWorkOrder,CR.CustomerReferenceType,SRC.SerialNumberType
					)
				...
				VersionNo,QuoteNumber,LastMSLevel,AllMSlevels,
				CustomerReference,SerialNumber,IsWorkOrder,CustomerReferenceType,SerialNumberType, ReferenceId
				from Results
				GROUP BY 
