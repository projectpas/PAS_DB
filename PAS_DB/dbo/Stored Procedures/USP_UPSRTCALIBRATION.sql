...
BEGIN
		if @Status=0
		BEGIN
		IF EXISTS (SELECT * FROM AssetInventory WHERE AssetInventoryId=@AssetInventoryId  and CalibrationRequired=1)
		BEGIN
		IF EXISTS (select * from CalibrationManagment where AssetInventoryId=@AssetInventoryId and CertifyType='Calibration' and IsActive=1)
		BEGIN
		...
		ELSE IF @Status=1
		BEGIN
		IF EXISTS (SELECT * FROM CalibrationManagment where AssetInventoryId=@AssetInventoryId and CertifyType='Calibration')
			BEGIN
