 ...
 ** --   --------     -------		--------------------------------          
 ...
 EXECUTE USP_UpdateMSDetails 2

**************************************************************/ 
    
CREATE PROCEDURE [dbo].[USP_UpdateMSDetails]    
(    
@MSDetailsId BIGINT,
@EntityMSID BIGINT, 
				...
				BEGIN
					...
					UPDATE [dbo].[ManagementStructureDetails]
						SET [EntityMSID] = @EntityMSID, 
							...
							[Level1Name] = CAST(MSL1.Code AS VARCHAR(250)) + ' - ' + MSL1.[Description],
							[Level2Id] = EST.Level2Id,
							[Level2Name] = CAST(MSL2.Code AS VARCHAR(250)) + ' - ' + MSL2.[Description],
							[Level3Id] = EST.Level3Id,
							[Level3Name] = CAST(MSL3.Code AS VARCHAR(250)) + ' - ' + MSL3.[Description],
							[Level4Id] = EST.Level4Id,
							[Level4Name] = CAST(MSL4.Code AS VARCHAR(250)) + ' - ' + MSL4.[Description],
							[Level5Id] = EST.Level5Id,
							[Level5Name] = CAST(MSL5.Code AS VARCHAR(250)) + ' - ' + MSL6.[Description],
							[Level6Id] = EST.Level6Id,
							[Level6Name] = CAST(MSL6.Code AS VARCHAR(250)) + ' - ' + MSL6.[Description],
							[Level7Id] = EST.Level7Id,
							[Level7Name] = CAST(MSL7.Code AS VARCHAR(250)) + ' - ' + MSL7.[Description],
							[Level8Id] = EST.Level8Id,
							[Level8Name] = CAST(MSL8.Code AS VARCHAR(250)) + ' - ' + MSL8.[Description],
							[Level9Id] = EST.Level9Id,
							[Level9Name] = CAST(MSL9.Code AS VARCHAR(250)) + ' - ' + MSL9.[Description],
							[Level10Id] = EST.Level10Id,
							[Level10Name] = CAST(MSL10.Code AS VARCHAR(250)) + ' - ' + MSL10.[Description]
					...
					WHERE EST.EntityStructureId=@EntityMSID AND MSDetailsId = @MSDetailsId; 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_UpdateMSDetails' 
