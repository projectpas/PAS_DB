...
BEGIN 
	BEGIN TRY
		...
		BEGIN 
			...
			SELECT @VendorRMADetailId = VendorRMADetailId, @VendorRMAId = VendorRMAId,@ExtAmount = ISNULL(ApplierdAmt,0) FROM [DBO].[VendorCreditMemoDetail] WITH(NOLOCK) WHERE VendorCreditMemoId = @VendorRMADetailId;
		...
		BEGIN 
			...
			BEGIN 
				SELECT @MasterCompanyId=MasterCompanyId,@UpdateBy=CreatedBy FROM [DBO].[VendorCreditMemoDetail] WITH(NOLOCK) WHERE VendorCreditMemoId = @tmpVendorRMADetailId;
			END
			...
			BEGIN 
				SELECT @stklineId= StockLineId FROM [DBO].[VendorCreditMemoDetail] WITH(NOLOCK) WHERE VendorCreditMemoId = @tmpVendorRMADetailId;
			END
			...
			BEGIN
				 ...
				 INSERT INTO [dbo].[CommonBatchDetails]
					...
					VALUES(@JournalBatchHeaderId,@JournalBatchDetailId,@VendorRMAId,@ExtNumber,@VendorId,@ExtDate,@CommonBatchDetailId,@stklineId)
