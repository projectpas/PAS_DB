...
--  EXEC [dbo].[UpdateSalesOrderQuoteApprovalNameColumnsWithId] 5
CREATE PROCEDURE [dbo].[UpdateSalesOrderQuoteApprovalNameColumnsWithId]
	@SalesOrderQuoteApprovalId int
AS
BEGIN
	...
	BEGIN
		...
		FROM [dbo].[SalesOrderQuoteApproval] SOQA WITH (NOLOCK)
		...
		Where SOQA.SalesOrderQuoteApprovalId = @SalesOrderQuoteApprovalId
	END
	COMMIT  TRANSACTION
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
            , @AdhocComments     VARCHAR(150)    = 'UpdateSalesOrderQuoteApprovalNameColumnsWithId' 
            , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ ISNULL(@SalesOrderQuoteApprovalId, '') + ''
