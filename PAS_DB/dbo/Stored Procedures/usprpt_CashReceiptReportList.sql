		...
		BEGIN
			...
			;WITH  Result AS (
				SELECT DISTINCT
					...
					--CPD.Amount AS 'baseCurrencyAmount',
					CP.CreatedDate,				
					S.Name AS 'Status',
					CASE 
			...
			FinalResult AS (
				...
				WHERE (
					(@GlobalFilter <> '' AND (
						...
						(bankAccount LIKE '%' + @GlobalFilter + '%') OR    
						(PaymentReference LIKE '%' + @GlobalFilter + '%') OR    
						(C.Code LIKE '%' + @GlobalFilter + '%') OR    
						(CustomerName LIKE '%' + @GlobalFilter + '%') OR    
						(PaymentMethod LIKE '%' + @GlobalFilter + '%')     
						OR TRY_CONVERT(VARCHAR(50), baseCurrencyAmount) LIKE '%' + @GlobalFilter + '%'
					))    
					OR       
					(@GlobalFilter = '' AND (
			...
			ORDER BY    
				...
				CASE WHEN (@SortOrder=1 and @SortColumn='CreditTermName') THEN CreditTermName END ASC,   
				CASE WHEN (@SortOrder=-1 and @SortColumn='CreditTermName') THEN CreditTermName END DESC,
...
/***************************************||||||%%%%%%%%%%%%%%%%%%%%% OLD CHANEGS  :11FEB %%%%%%%%%%%%%%%%%%%%%%%%%%%|||||||*****************************************/
			...
			--INSERT INTO #invoiceTmpTable([ReceiptId], InvoiceNum, InvoiceDate,Amount,InvoicePaymentId,[PaymentType],CreditTermName)
			--SELECT DISTINCT
			--	[IP].[ReceiptId],
			--	 UPPER([IP].[DocNum]) AS InvoiceNum,   
			--	 [IP].[InvoiceDate] as InvoiceDate,  
			...
			--	    ELSE (CAST(CP.PostedDate AS DATETIME)) END) < DATEADD(DAY, 1, @ToDate)))
			--	  GROUP BY [IP].[ReceiptId],[IP].[DocNum],[IP].[InvoiceDate],[IP].PaymentAmount,Ip.PaymentId,
			--	  [IP].IsCheckPayment,[IP].IsWireTransfer,[IP].IsCCDCPayment,IP.CreditTermName
			--	  --CPD.IsCheckPayment,CPD.IsWireTransfer,CPD.IsCCDCPayment
  ...
  BEGIN CATCH
    ...
    DECLARE @ErrorLogID int,
        ...
        @AdhocComments varchar(150) = '[usprpt_CashReceiptReportList]',
