...
CREATE   PROCEDURE [dbo].[usprpt_GetSalesOrderBillingReportnew] 
@PageNumber int = 1,
@PageSize int = NULL,
@mastercompanyid int,
@xmlFilter XML
...
@name=case when filterby.value('(FieldName/text())[1]','VARCHAR(100)')='Cutsomer(Optional)' 
  ...
  IF ISNULL(@PageSize,0)=0
	  BEGIN 
		  ...
		  WHERE (C.Name like '%'+@name+'%' OR isnull(@name,'') = '' ) AND 
      ...
      SELECT COUNT(1) OVER () AS TotalRecordsCount,    
       ...
       UPPER(WO.workordernum) 'wonum',  
        FORMAT (STL.receiveddate, 'MM/dd/yyyy hh:mm tt')  AS 'receiveddate',  
       FORMAT (SO.opendate, 'MM/dd/yyyy hh:mm tt')  AS  'opendate',  
        UPPER(SOBI.invoiceno) 'invoicenum',  
       FORMAT (SOBI.invoicedate, 'MM/dd/yyyy hh:mm tt')  AS  'invoicedate',  
       (isnull(SOP.SalesPriceExtended,0) + (select isnull(sum(sc.billingamount),0) from SalesOrderCharges sc WITH (NOLOCK)
       where sc.SalesOrderId =SO.SalesOrderId and sop.SalesOrderPartId=sc.SalesOrderPartId and sc.isdeleted=0 and sc.isactive =1 ) )as  'revenue',  
         FORMAT (SOQ.OpenDate, 'MM/dd/yyyy hh:mm tt') 'quotedate',  
  FORMAT (soq.ApprovedDate, 'MM/dd/yyyy hh:mm tt')  AS 'quoteapprovaldate',  
     FORMAT (SOBI.shipdate, 'MM/dd/yyyy hh:mm tt')  AS     'shipdate',  
      ...
      FROM dbo.salesorder SO WITH (NOLOCK)  
        ...
        LEFT JOIN dbo.workorder WO WITH (NOLOCK)  ON STL.workorderid = WO.workorderid  
        LEFT JOIN dbo.condition CDTN WITH (NOLOCK) ON SOP.conditionid = CDTN.conditionid  
        --LEFT JOIN dbo.employee AS E WITH (NOLOCK)  ON SO.salespersonid = E.employeeid  
        --LEFT JOIN dbo.employee AS E1 WITH (NOLOCK) ON SO.customersevicerepid = E1.employeeid  
        --LEFT OUTER JOIN dbo.mastercompany MC WITH (NOLOCK) ON SO.mastercompanyid = MC.mastercompanyid  
      WHERE (C.Name like '%'+@name+'%' OR isnull(@name,'') = '' ) AND 
	  CAST(SOBI.invoicedate AS DATETIME) BETWEEN CAST(@FromDate AS DATETIME) AND CAST(@ToDate AS DATETIME)  AND
	  SO.mastercompanyid = @mastercompanyid  
	    AND  
