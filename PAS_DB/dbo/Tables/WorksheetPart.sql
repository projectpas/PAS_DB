...
BEGIN
    ...
    paired AS (
        SELECT
            ...
            END AS Action,
            (SELECT COALESCE(i.WorksheetPartId, d.WorksheetPartId) AS WorksheetPartId FOR JSON PATH, WITHOUT_ARRAY_WRAPPER) AS PKJson
        FROM d
        FULL OUTER JOIN i
    ...
    WHERE
        (m.Action = 'U' AND ( (m.OldValue IS NULL AND m.NewValue IS NOT NULL)
