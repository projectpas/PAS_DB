 ...
 ** PARAMETERS: @SerachText VARCHAR(50), @MasterCompanyId BIGINT
...
AS BEGIN
	...
	BEGIN TRY
		...
		IM.partnumber + (CASE WHEN (SELECT COUNT(ISNULL(SD.[ManufacturerId], 0)) FROM [dbo].[ItemMaster]  SD WITH(NOLOCK)  WHERE im.partnumber = SD.partnumber AND SD.MasterCompanyId = 1 ) > 1 then ' - '+ M.[Name] ELSE '' END) AS partnumber
		...
		WHERE IM.MasterCompanyId = 1 AND IM.IsActive = 1 AND IM.IsDeleted = 0 AND
