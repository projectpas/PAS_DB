	...
	BEGIN TRY
		...
		BEGIN
			SELECT @CustomerId = SO.[CustomerId],@MasterCompanyId = SO.[MasterCompanyId],
				  ...
				  @SoTotalCharges = ISNULL(TotalCharges,0), @SoTotalFreight = ISNULL(TotalFreight,0), @AllowInvoiceBeforeShipping = ISNULL(AllowInvoiceBeforeShipping,0)
		...
		BEGIN
			UPDATE #TempWithRowNum SET MiscCharges = 0, FreightCost = 0, TotalCost = CASE WHEN (ISNULL(TotalCost,0) - (ISNULL(MiscCharges,0) + ISNULL(FreightCost,0))) >= 0 THEN (ISNULL(TotalCost,0) - (ISNULL(MiscCharges,0) + ISNULL(FreightCost,0))) ELSE 0 END  WHERE RowNum > 1
