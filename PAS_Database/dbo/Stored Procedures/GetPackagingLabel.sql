...
BEGIN
    ...
    FROM SOPickTicket sopkt WITH(NOLOCK)
    JOIN SalesOrder soq WITH(NOLOCK) ON sopkt.SalesOrderId = soq.SalesOrderId
    LEFT JOIN SalesOrderPartV1 part WITH(NOLOCK) ON soq.SalesOrderId = part.SalesOrderId
    LEFT JOIN SalesOrderStockLineV1 stk WITH(NOLOCK) ON part.SalesOrderPartId = stk.SalesOrderPartId
    LEFT JOIN Customer cust WITH(NOLOCK) ON soq.CustomerId = cust.CustomerId
    LEFT JOIN Address cuad WITH(NOLOCK) ON cust.AddressId = cuad.AddressId
    LEFT JOIN Countries ccnty WITH(NOLOCK) ON cuad.CountryId = ccnty.countries_id
    ...
    LEFT JOIN AllAddress posadd WITH(NOLOCK) ON soq.SalesOrderId = posadd.ReffranceId 
        AND posadd.IsShippingAdd = 1 
        AND posadd.ModuleId = @SalesOrderModuleId -- assuming SalesOrder module
    LEFT JOIN AllShipVia posv WITH(NOLOCK) ON soq.SalesOrderId = posv.ReferenceId 
        AND posv.ModuleId = 1 -- assuming SalesOrder module
    LEFT JOIN SalesOrderPackaginSlipItems spi WITH(NOLOCK) ON sopkt.SOPickTicketId = spi.SOPickTicketId
    LEFT JOIN SalesOrderPackaginSlipHeader spb WITH(NOLOCK) ON spi.PackagingSlipId = spb.PackagingSlipId
    LEFT JOIN SalesOrderShippingItem sosi WITH(NOLOCK) ON sopkt.SOPickTicketId = sosi.SOPickTicketId
    LEFT JOIN SalesOrderShipping sos WITH(NOLOCK) ON sosi.SalesOrderShippingId = sos.SalesOrderShippingId
    LEFT JOIN SalesOrderBillingInvoicing sobi WITH(NOLOCK) ON sos.SalesOrderId = sobi.SalesOrderId
    LEFT JOIN Employee saemp WITH(NOLOCK) ON soq.SalesPersonId = saemp.EmployeeId
    ...
    LEFT JOIN ShippingVia sh WITH(NOLOCK) ON sos.ShipviaId = sh.ShippingViaId
