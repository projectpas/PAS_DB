...
BEGIN
    ...
    SET NOCOUNT ON
		...
		SELECT DISTINCT
			...
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.DiscountAmount, 0)
			ELSE ISNULL(SOPC.DiscountAmount, 0) END discountAmount,
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.NetSaleAmount, 0)
			ELSE ISNULL(SOPC.NetSaleAmount, 0) END netSales,
			...
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.UnitSalesPriceExtended, 0)
			ELSE ISNULL(SOPC.UnitSalesPriceExtended, 0) END salesPriceExtended,
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.MarkUpAmount, 0)
			ELSE ISNULL(SOPC.MarkUpAmount, 0) END markupExtended,
			0 salesDiscountExtended,
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.NetSaleAmount, 0)
			ELSE ISNULL(SOPC.NetSaleAmount, 0) END netSalePriceExtended,
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.UnitCostExtended, 0)
			ELSE ISNULL(SOPC.UnitCostExtended, 0) END AS unitCostExtended,
			...
			ISNULL((
				CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL
					THEN (CASE WHEN itemMaster.[StockUnitOfMeasure] = itemMaster.[ConsumeUnitOfMeasure] THEN ISNULL(SOSC.MarginAmount,0) ELSE [dbo].[fn_ConvertUOM](ISNULL(SOSC.MarginAmount,0), itemMaster.[StockUnitOfMeasure], itemMaster.[ConsumeUnitOfMeasure], 1, part.[MasterCompanyId]) END)
					ELSE (CASE WHEN itemMaster.[StockUnitOfMeasure] = itemMaster.[ConsumeUnitOfMeasure] THEN ISNULL(SOPC.MarginAmount,0) ELSE [dbo].[fn_ConvertUOM](ISNULL(SOPC.MarginAmount,0), itemMaster.[StockUnitOfMeasure], itemMaster.[ConsumeUnitOfMeasure], 1, part.[MasterCompanyId]) END)
				END
				*
				CASE WHEN stk.SalesOrderStocklineId IS NOT NULL
					THEN (CASE WHEN itemMaster.[StockUnitOfMeasure] = itemMaster.[ConsumeUnitOfMeasure] THEN ISNULL(stk.QtyOrder,0) ELSE [dbo].[fn_ConvertUOM](ISNULL(stk.QtyOrder,0), itemMaster.[StockUnitOfMeasure], itemMaster.[ConsumeUnitOfMeasure], 0, part.[MasterCompanyId]) END)
					ELSE (CASE WHEN itemMaster.[StockUnitOfMeasure] = itemMaster.[ConsumeUnitOfMeasure] THEN ISNULL(part.QtyOrder,0) ELSE [dbo].[fn_ConvertUOM](ISNULL(part.QtyOrder,0), itemMaster.[StockUnitOfMeasure], itemMaster.[ConsumeUnitOfMeasure], 0, part.[MasterCompanyId]) END)
				END
			...
			CASE WHEN SOSC.SalesOrderStocklineId IS NOT NULL THEN ISNULL(SOSC.NetSaleAmount, 0)
			ELSE ISNULL(SOPC.NetSaleAmount, 0) END AS totalSales,
