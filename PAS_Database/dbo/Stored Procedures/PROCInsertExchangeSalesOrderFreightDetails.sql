/*************************************************************           
 ** File:   [PROCInsertExchangeSalesOrderFreightDetails]           
...
-- EXEC [PROCInsertExchangeSalesOrderFreightDetails] 15
************************************************************************/
CREATE   PROCEDURE [dbo].[PROCInsertExchangeSalesOrderFreightDetails](@TableExchangeSalesOrderFreightType ExchangeSalesOrderFreightType READONLY)  
AS  
BEGIN  
	SET NOCOUNT ON;
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED		
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					IF((SELECT COUNT(ExchangeSalesOrderId) FROM @TableExchangeSalesOrderFreightType) > 0 )
					BEGIN
						DECLARE @ExchangeSalesOrderId AS bigint
						SET @ExchangeSalesOrderId = (SELECT TOP 1 ExchangeSalesOrderId FROM @TableExchangeSalesOrderFreightType);
						MERGE dbo.ExchangeSalesOrderFreight AS TARGET
						USING @TableExchangeSalesOrderFreightType AS SOURCE ON (TARGET.ExchangeSalesOrderId = SOURCE.ExchangeSalesOrderId AND 
					  													     TARGET.ExchangeSalesOrderFreightId = SOURCE.ExchangeSalesOrderFreightId) 
						WHEN MATCHED 
						THEN UPDATE 
						SET
						...
						THEN
							INSERT([ExchangeSalesOrderId],
									--[ExchangeSalesOrderPartRecordId],[ItemMasterId],[PartNumber],
									[ShipViaId],
					         ...
					         VALUES(SOURCE.ExchangeSalesOrderId,
									--SOURCE.ExchangeSalesOrderPartRecordId,SOURCE.ItemMasterId,SOURCE.PartNumber,
									SOURCE.ShipViaId,
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'PROCInsertExchangeSalesOrderFreightDetails' 
