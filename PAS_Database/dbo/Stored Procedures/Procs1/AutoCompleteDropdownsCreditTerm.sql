...
CREATE PROCEDURE [dbo].[AutoCompleteDropdownsCreditTerm]
...
BEGIN	
			...
			BEGIN		
			   ...
			   UNION     
					SELECT DISTINCT  
						...
						C.Code AS StatusCode
					FROM dbo.CreditTerms C WITH(NOLOCK)
					WHERE C.MasterCompanyId = @MasterCompanyId AND C.CreditTermsId in (SELECT Item FROM DBO.SPLITSTRING(@Idlist,','))    
				ORDER BY Label				
			End
			ELSE
			BEGIN
	...
	BEGIN CATCH	
			...
			,@AdhocComments VARCHAR(150) = 'AutoCompleteDropdownsCreditTerm'
