/***************************************************************************
-- EXEC [dbo].[AutoCompleteDropdownsOpenExchangeSalesOrder] '',1,25,'0',1
****************************************************************************/
CREATE PROCEDURE [dbo].[AutoCompleteDropdownsOpenExchangeSalesOrder]
		...
		--BEGIN TRANSACTION
			--BEGIN
				...
				END	
					...
					WHERE (c.IsActive = 1 AND ISNULL(c.IsDeleted, 0) = 0 AND c.MasterCompanyId = @MasterCompanyId AND (c.ExchangeSalesOrderNumber LIKE @StartWith + '%') AND c.StatusId=1)
					UNION
					SELECT DISTINCT TOP 20 
						c.ExchangeSalesOrderId,
						c.ExchangeSalesOrderNumber
