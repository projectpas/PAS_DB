...
** 4    29/10/2024      RAJESH GAMI         Un Mapped Sub WO if there is no other material link with the same Sub workorder in the PurchaseOrder Part
...
BEGIN
	...
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
		...
		BEGIN TRY
			...
			IF((SELECT COUNT(1) FROM dbo.PurchaseOrderPart WITH(NOLOCK) WHERE SubWorkOrderId = @SubWorkOrderId) = 1)
			BEGIN
				UPDATE P    
					SET	   SubWorkOrderId = NULL, SubWorkOrderNo = NULL,
							...
							UpdatedDate = GETUTCDATE()
					FROM DBO.PurchaseOrderPart P WHERE P.SubWorkOrderId = @SubWorkOrderId
			END
			IF OBJECT_ID(N'tempdb..##TempSubWOtbl') IS NOT NULL
				BEGIN
					DROP TABLE #TempSubWOtbl 
