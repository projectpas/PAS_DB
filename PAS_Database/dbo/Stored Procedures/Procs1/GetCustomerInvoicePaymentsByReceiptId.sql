 ...
 BEGIN TRY      
	...
	BEGIN      
		...
		SELECT INV.[PaymentId],INV.[CustomerId],INV.[SOBillingInvoicingId],INV.[ReceiptId],INV.[MasterCompanyId],0 AS [IsMultiplePaymentMethod],0 AS [IsCheckPayment],0 AS [IsWireTransfer],0 AS [IsEFT],0 AS [IsCCDCPayment]      
              ...
              ,INV.[CreditLimit],INV.[CreditTermName], 
			   CASE WHEN INV.[LastMSLevel] IS NOT NULL THEN INV.[LastMSLevel] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[LastMSLevel] WHEN InvoiceType = 2 THEN MSD_WO.[LastMSLevel] WHEN InvoiceType = 6 THEN EMSD.[LastMSLevel] WHEN InvoiceType = 7 THEN MSD_UC.[LastMSLevel] ELSE MSD_CM.[LastMSLevel] END END LastMSLevel,      
               CASE WHEN INV.[AllMSlevels] IS NOT NULL THEN INV.[AllMSlevels] ELSE CASE WHEN InvoiceType = 1 THEN MSD.[AllMSlevels] WHEN InvoiceType = 2 THEN MSD_WO.[AllMSlevels] WHEN InvoiceType = 6 THEN EMSD.[AllMSlevels] WHEN InvoiceType = 7 THEN MSD_UC.[LastMSLevel] ELSE MSD_CM.[AllMSlevels] END END AllMSlevels,      
               [PageIndex],   
		...
		SELECT CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.PaymentId    ELSE 0 END AS 'PaymentId',      
			  ...
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.CreditTermName    ELSE SO.CreditTermName END AS 'CreditTermName',      
		...
		SELECT CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.PaymentId ELSE 0 END AS 'PaymentId',      
			  ...
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.Reason ELSE 0 END AS Reason,      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.RemainingBalance  ELSE 0 END AS RemainingBalance,      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[Status]  ELSE 'Open' END AS [Status],      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[CreatedBy]  ELSE CCP.CreatedBy END AS 'CreatedBy',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[UpdatedBy]  ELSE CCP.UpdatedBy END AS 'UpdatedBy',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[CreatedDate]  ELSE CCP.CreatedDate END AS 'CreatedDate',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[UpdatedDate]  ELSE CCP.UpdatedDate END AS 'UpdatedDate',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[IsActive]  ELSE CCP.IsActive END AS 'IsActive',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[IsDeleted]  ELSE CCP.IsDeleted END AS 'IsDeleted',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN ISNULL(IPT.[IsDeposite],0)  ELSE 0 END AS 'IsDeposite',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[IsTradeReceivable]  ELSE 0 END AS IsTradeReceivable,      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.[TradeReceivableORMiscReceiptGLAccnt]  ELSE 0 END AS TradeReceivableORMiscReceiptGLAccnt,      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.CtrlNum    ELSE '' END AS 'CtrlNum',    
			  7 AS 'InvoiceType',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.OriginalAmount ELSE CCP.RemainingAmount END AS 'OriginalAmount',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.NewRemainingBal ELSE 0 END AS 'NewRemainingBal',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.DocNum ELSE CCP.SuspenseUnappliedNumber END AS DocNum,      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.CurrencyCode ELSE Curr.Code END AS 'CurrencyCode',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.FxRate ELSE 0 END AS 'FxRate',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.WOSONum ELSE CCP.SuspenseUnappliedNumber END AS 'WOSONum',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.DSI ELSE 0 END AS 'DSI',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.DSO ELSE 0 END AS 'DSO',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.AmountPastDue ELSE 0 END AS 'AmountPastDue',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.ARBalance ELSE 0 END AS 'ARBalance',      
			  ...
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN @PageIndex ELSE @PageIndex END AS 'PageIndex',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.RemainingAmount ELSE 0 END AS 'RemainingAmount',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.InvoiceDate  ELSE CCP.ReceiveDate END AS 'InvoiceDate',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.Id ELSE CCP.CustomerCreditPaymentDetailId END AS 'Id',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.GLARAccount ELSE '' END AS 'GLARAccount',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN CASE WHEN IPT.IsDeleted = 1 THEN 0 ELSE 1 END ELSE 0 END AS 'Selected' ,  
			  NULL AS DiscountDate,        
			  0 AS DiscountAvailable   
		 ...
		 FROM [dbo].[CustomerCreditPaymentDetail] CCP WITH (NOLOCK)      
			  ...
			  LEFT JOIN [dbo].[InvoicePayments] IPT WITH (NOLOCK) ON IPT.SOBillingInvoicingId = CCP.CustomerCreditPaymentDetailId AND IPT.InvoiceType=7 AND IPT.ReceiptId = @ReceiptId AND IPT.PageIndex = @PageIndex      
			  INNER JOIN [dbo].[SuspenseAndUnAppliedPaymentMSDetails] MSD WITH (NOLOCK) ON MSD.ModuleID = @SuspenseModuleID AND MSD.ReferenceID = CCP.CustomerCreditPaymentDetailId AND MSD.Level1Id = @legalEntityId   
		...
		SELECT CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.PaymentId    ELSE 0 END AS 'PaymentId',      
			  ...
			  NULL AS 'InvDueDate',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.CreditLimit    ELSE CF.CreditLimit END AS 'CreditLimit',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.CreditTermName    ELSE ESO.[CreditTermName] END AS 'CreditTermName',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.LastMSLevel    ELSE MSD.LastMSLevel END AS 'LastMSLevel',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN IPT.AllMSlevels    ELSE MSD.AllMSlevels END AS 'AllMSlevels',      
			  CASE WHEN IPT.SOBillingInvoicingId IS NOT NULL THEN @PageIndex    ELSE @PageIndex END AS 'PageIndex',      
		...
		FROM [dbo].[ExchangeSalesOrderBillingInvoicing] ESOBI WITH (NOLOCK)      
			  ...
			  LEFT JOIN [dbo].[Percent] p WITH(NOLOCK) ON CAST(ESO.PercentId as INT) = p.PercentId        
     ...
     GROUP BY [PaymentId],[CustomerId],[SOBillingInvoicingId],[ReceiptId],[IsMultiplePaymentMethod],[IsCheckPayment],[IsWireTransfer],[IsEFT],[IsCCDCPayment]      
          ...
          ,[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[IsActive],[IsDeleted],[IsDeposite],[IsTradeReceivable],[TradeReceivableORMiscReceiptGLAccnt]      
          ,[CtrlNum],[InvoiceType],[OriginalAmount],[NewRemainingBal],[DocNum],[CurrencyCode],[FxRate],[WOSONum],[DSI],[DSO],[AmountPastDue],[ARBalance],[InvDueDate]      
          ,[CreditLimit],[CreditTermName],[LastMSLevel],[AllMSlevels],[PageIndex],[RemainingAmount],[InvoiceDate],[Id],[GLARAccount],[Selected],[DiscountDate],[DiscountAvailable] ,[CustomerPaymentDetailsId]
