	...
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED	
		...
		BEGIN			
			;WITH Result AS(									
				...
				CAST(ISNULL([dbo].[fn_ConvertUOM](ISNULL(SPC.[UnitSalesPrice], 0),IM.[StockUnitOfMeasure], IM.[ConsumeUnitOfMeasure],1),0) AS VARCHAR(50)) AS [UnitSalesPrice],
				--ISNULL(SPC.[UnitCost], 0) AS [UnitCost],
				ISNULL([dbo].[fn_ConvertUOM](ISNULL(SPC.[UnitCost], 0),IM.[StockUnitOfMeasure], IM.[ConsumeUnitOfMeasure],1),0) AS [UnitCost],				
				--ISNULL(SP.[QtyOrder], 0) AS [Qty],
				ISNULL([dbo].[fn_ConvertUOM](ISNULL(SP.[QtyOrder],0),IM.[StockUnitOfMeasure], IM.[ConsumeUnitOfMeasure],0),0) AS [Qty],
				--ISNULL(SPC.[UnitCostExtended], 0) AS [UnitCostExtended],
				ISNULL([dbo].[fn_ConvertUOM](ISNULL(SPC.[UnitCostExtended], 0),IM.[StockUnitOfMeasure], IM.[ConsumeUnitOfMeasure],1),0) AS [UnitCostExtended],
