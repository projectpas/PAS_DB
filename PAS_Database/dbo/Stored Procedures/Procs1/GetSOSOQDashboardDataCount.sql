...
BEGIN
	...
	BEGIN
		...
		SELECT @SOQReceivedAmount = SUM(SOQPC.NetSaleAmount) FROM 
				DBO.SalesOrderQuotePartV1 POP WITH (NOLOCK)
				INNER JOIN DBO.SalesOrderQuote PO WITH (NOLOCK) ON PO.SalesOrderQuoteId = POP.SalesOrderQuoteId
				LEFT JOIN DBO.SalesOrderQuotePartCost SOQPC WITH (NOLOCK) ON SOQPC.SalesOrderQuotePartId=POP.SalesOrderQuotePartId and ISNULL(SOQPC.IsDeleted, 0)=0
			...
			WHERE  (ISNULL(PO.IsDeleted, 0) = 0) and (ISNULL(POP.IsDeleted, 0) = 0) and (PO.StatusId =@SOQReceivedId) AND C.CustomerAffiliationId IN (SELECT Item FROM DBO.SPLITSTRING(@CustomerAffiliation, ','))
