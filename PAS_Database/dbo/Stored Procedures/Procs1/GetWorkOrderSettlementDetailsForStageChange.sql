...
BEGIN
	...
	BEGIN  
		...
		WHERE WorkFlowWorkOrderId = @workflowWorkorderId AND (WOMS.ProvisionId = @ProvisionId OR WOMS.ProvisionId = @RepairProvisionId)

		SELECT @OtherProvisionQty = SUM(ISNULL(WOMS.Quantity, 0)) 
		FROM dbo.WorkOrderMaterialStockLine WOMS WITH(NOLOCK) 
			JOIN dbo.WorkOrderMaterials WOM WITH(NOLOCK) ON WOM.WorkOrderMaterialsId = WOMS.WorkOrderMaterialsId	  
		WHERE WorkFlowWorkOrderId = @workflowWorkorderId AND (WOMS.ProvisionId != @ProvisionId AND WOMS.ProvisionId != @RepairProvisionId) -- REPLACE
		...
		WHERE WorkFlowWorkOrderId = @workflowWorkorderId AND (WOMS.ProvisionId = @ProvisionId OR WOMS.ProvisionId = @RepairProvisionId) -- REPLACE
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
            , @AdhocComments     VARCHAR(150)    = 'GetWorkOrderSettlementDetailsForStageChange' 
