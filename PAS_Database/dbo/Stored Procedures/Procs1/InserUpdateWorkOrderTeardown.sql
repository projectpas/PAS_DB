...
CREATE PROCEDURE [dbo].[InserUpdateWorkOrderTeardown]
	 ...
	 BEGIN TRY
		BEGIN TRANSACTION
			BEGIN
				...
				SELECT @WorkOrderTeardownId = SCOPE_IDENTITY()
						...
						insert into  [dbo].[WorkOrderAdditionalComments] (WorkOrderTeardownId,MasterCompanyId,CreatedBy,UpdatedBy,CreatedDate,UpdatedDate,IsActive,IsDeleted,SubWorkOrderTeardownId)
																  values (@WorkOrderTeardownId,@MasterCompanyId,@CreatedBy,@UpdatedBy,@CreatedDate,@UpdatedDate,1,0,null)
				...
				if(@SubWorkOrderTeardownId =0)
				  begin
				       set @SubWorkOrderTeardownId= null
				  end
				if(@ReasonId =0)
				  begin
				     set @ReasonId= null
				...
				if(LOWER(@TableName) ='workorderadditionalcomments')
				 BEGIN
							 ...
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsAdditionalComments=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end

							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderAdditionalCommentsId  from [WorkOrderAdditionalComments] where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN   
							 ...
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsBulletinsModification=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end

							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderBulletinsModificationId  from WorkOrderBulletinsModification where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsDiscovery=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderDiscoveryId  from [WorkOrderDiscovery] where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 ...
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsFinalInspection=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderFinalInspectionId  from [WorkOrderFinalInspection] where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsFinalTest=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderFinalTestId  from [WorkOrderFinalTest] where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 ...
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsPmaDerBulletins=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderPmaDerBulletinsId  from WorkOrderPmaDerBulletins where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsPreAssemblyInspection=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderPreAssemblyInspectionId  from WorkOrderPreAssemblyInspection where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsPreAssmentResults=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderPreAssmentResultsId  from WorkOrderPreAssmentResults where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsPreliinaryReview=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderPreliinaryReviewId  from WorkOrderPreliinaryReview where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
				 ...
				 BEGIN
						 if(isnull(@WorkOrderTeardownId,0) > 0)
						 begin 
						  update WorkOrderTeardown set IsRemovalReasons=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
						 end

						 if(isnull(@TableprimaryId,0) = 0)
						 begin
						  select @TableprimaryId =WorkOrderRemovalReasonsId  from WorkOrderRemovalReasons where WorkOrderTeardownId = @WorkOrderTeardownId
						 end
				 ...
				 BEGIN
						 if(isnull(@WorkOrderTeardownId,0) > 0)
						 begin 
						  update WorkOrderTeardown set IsTestDataUsed=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
						 end

						 if(isnull(@TableprimaryId,0) = 0)
						 begin
						  select @TableprimaryId =WorkOrderTestDataUsedId  from WorkOrderTestDataUsed where WorkOrderTeardownId = @WorkOrderTeardownId
						 end
				 ...
				 BEGIN
							 if(isnull(@WorkOrderTeardownId,0) > 0)
							 begin 
							      update WorkOrderTeardown set IsWorkPerformed=@ischeckvalue where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
							 
							 if(isnull(@TableprimaryId,0) = 0)
							 begin
							      select @TableprimaryId =WorkOrderWorkPerformedId  from WorkOrderWorkPerformed where WorkOrderTeardownId = @WorkOrderTeardownId
							 end
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'InserUpdateWorkOrderTeardown' 
              , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = ''' + CAST(ISNULL(@TableName, '') AS varchar(100))												   
			   ...
			   + '@Parameter26 = ''' + CAST(ISNULL(@inspectorId  , '') AS varchar(100))
			   + '@Parameter27 = ''' + CAST(ISNULL(@inspectorDate , '') AS varchar(100))
