 ...
 ** Description: This stored procedure is used to store Asset Management Structure Details
 ** Purpose:         
 ** Date:   11/03/2022        
 ...
 ** --   --------     -------		--------------------------------          
    1    11/03/2022  Subhash saliya     Created
     
-- EXEC PROCAddAssetMSData 47,3,5,2,'moin','moin',47,2,2
************************************************************************/

CREATE PROCEDURE [dbo].[PROCAddAssetMSData]
@ReferenceID bigint,
@EntityMSID bigint,
@MasterCompanyId int,
...
BEGIN
	...
	BEGIN  
		...
		BEGIN	
			  ...
			  INSERT INTO [dbo].[AssetManagementStructureDetails]
			  ...
			  SELECT @MSDetailsId = IDENT_CURRENT('AssetManagementStructureDetails');
			  ...
			  SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[AssetManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

			  EXECUTE(@Query)  
			  
			  UPDATE [dbo].[AssetManagementStructureDetails]
		...
		BEGIN
				...
				UPDATE [dbo].[AssetManagementStructureDetails]
					...
					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[AssetManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'PROCAddAssetMSData' 
