...
-- EXEC PROCAddExchangeMSData 47,3,2,'moin','moin',47,2,2
************************************************************************/
CREATE PROCEDURE [dbo].[PROCAddExchangeMSData]
@ExchangeId bigint,
@EntityMSID bigint,
@MasterCompanyId int,
...
BEGIN
	...
	BEGIN  
		...
		BEGIN
				...
				INSERT INTO [dbo].[ExchangeManagementStructureDetails]
				      ...
				      SELECT @ModuleId,@ExchangeId,@EntityMSID,													        
				...
				SELECT @MSDetailsId = IDENT_CURRENT('ExchangeManagementStructureDetails');
				...
				SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[ExchangeManagementStructureDetails] MSD WITH(NOLOCK) 
						WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

				EXECUTE(@Query)  
				
				UPDATE [dbo].[ExchangeManagementStructureDetails] 
		...
		BEGIN
				...
				UPDATE [dbo].[ExchangeManagementStructureDetails]
					...
					FROM [dbo].EntityStructureSetup EST WITH(NOLOCK)
							 ...
							 LEFT JOIN ManagementStructureLevel MSL9 WITH (NOLOCK) ON  EST.Level9Id = MSL9.ID
							 LEFT JOIN ManagementStructureLevel MSL10 WITH (NOLOCK) ON EST.Level10Id = MSL10.ID													   
					WHERE ModuleID = @ModuleId AND ReferenceID = @ExchangeId AND EST.EntityStructureId=@EntityMSID;

					SELECT @MasterCompanyId = [MasterCompanyId], @MSDetailsId = MSDetailsId
					FROM [dbo].[ExchangeManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @ExchangeId AND MSD.EntityMSID = @EntityMSID

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[ExchangeManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              ...
              , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ CAST(ISNULL(@ExchangeId, '') AS varchar(100))
