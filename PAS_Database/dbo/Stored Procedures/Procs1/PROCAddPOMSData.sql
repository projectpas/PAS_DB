 ...
 ** Author:  Moin Bloch
 ** Description: This stored procedure is used to store purchase order Management Structure Details
 ** Purpose:         
 ** Date:   07/03/2022        
          
 ** PARAMETERS: @PurchaseOrderId bigint,@EntityStructureId bigint,@MasterCompanyId int,@CreatedBy varchar,@UpdatedBy varchar,@ModuleId bigint,@Opr int
 ...
 ** --   --------     -------		--------------------------------          
    1    07/03/2022  Moin Bloch     Created
...
BEGIN
	...
	BEGIN  
		...
		BEGIN	
			  ...
			  INSERT INTO [dbo].[PurchaseOrderManagementStructureDetails]
			        ...
			        SELECT @ModuleId,@PurchaseOrderId,@EntityMSID,													        
			  ...
			  SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PurchaseOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

			  EXECUTE(@Query)  
			  
			  UPDATE [dbo].[PurchaseOrderManagementStructureDetails] SET [LastMSLevel] = LastMSName,
		...
		BEGIN
				...
				UPDATE [dbo].[PurchaseOrderManagementStructureDetails]
					...
					FROM [dbo].EntityStructureSetup EST WITH(NOLOCK)
							 ...
							 LEFT JOIN ManagementStructureLevel MSL9 WITH (NOLOCK) ON  EST.Level9Id = MSL9.ID
							 LEFT JOIN ManagementStructureLevel MSL10 WITH (NOLOCK) ON EST.Level10Id = MSL10.ID													   
					WHERE ModuleID = @ModuleId AND ReferenceID = @PurchaseOrderId AND EST.EntityStructureId=@EntityMSID;
					...
					WHERE MSD.ModuleID = @ModuleId AND MSD.ReferenceID = @PurchaseOrderId AND MSD.EntityMSID = @EntityMSID

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable2 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PurchaseOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
		...
		BEGIN	
			  ...
			  END
		      CREATE TABLE #TempTable3(LastMSName VARCHAR(MAX)) 
			  INSERT INTO [dbo].[PurchaseOrderManagementStructureDetails]
						  ([ModuleID],[ReferenceID],[EntityMSID],
						   [Level1Id],[Level1Name],
						   [Level2Id],[Level2Name],
			  ...
			  SET @Query = N'INSERT INTO #TempTable3 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PurchaseOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
			  ...
			  UPDATE [dbo].[PurchaseOrderManagementStructureDetails] SET [LastMSLevel] = LastMSName,
							[AllMSlevels] = (SELECT AllMSlevels 
					  FROM DBO.udfGetAllEntityMSLevelString(@EntityMSID))
					FROM #TempTable3 WHERE MSDetailsId = @MSDetailsId;										
		...
		IF(@Opr = 4)
		BEGIN
				IF OBJECT_ID(N'tempdb..#TempTable4') IS NOT NULL
				BEGIN
					DROP TABLE #TempTable4
				END
				CREATE TABLE #TempTable4(LastMSName VARCHAR(MAX)) 
				UPDATE [dbo].[PurchaseOrderManagementStructureDetails]
						SET [EntityMSID] = @EntityMSID, 
							[UpdatedBy] = @UpdatedBy,
							[Level1Id] = EST.Level1Id,
					...
					SET @Query = N'INSERT INTO #TempTable4 (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[PurchaseOrderManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'
					...
					UPDATE [dbo].[PurchaseOrderManagementStructureDetails] 
						SET [LastMSLevel] = LastMSName,
							[AllMSlevels] = (SELECT AllMSlevels FROM DBO.udfGetAllEntityMSLevelString(@EntityMSID))
					FROM #TempTable4 WHERE MSDetailsId = @MSDetailsId
