...
-- EXEC PROCAddVendorRFQPOMSData 47,3,2,'moin','moin',47,2,2
************************************************************************/

CREATE PROCEDURE [dbo].[PROCAddVendorRFQPOMSData]
...
BEGIN
	...
	BEGIN  
		...
		DECLARE @VendorRFQPOPartModuleID VARCHAR(MAX) = 21;
		IF(@Opr = 1)
		BEGIN
			IF NOT EXISTS(SELECT ReferenceID FROM PurchaseOrderManagementStructureDetails WHERE ReferenceID = @PurchaseOrderId AND ModuleID = @VendorRFQPOPartModuleID)
			BEGIN
