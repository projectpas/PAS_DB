...
BEGIN  
	...
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED		
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					IF((SELECT COUNT(PurchaseOrderId) FROM @TablePurchaseOrderFreightType) > 0 )
					BEGIN
						DECLARE @PurchaseOrderId AS bigint
						SET @PurchaseOrderId = (SELECT TOP 1 PurchaseOrderId FROM @TablePurchaseOrderFreightType);
						MERGE dbo.PurchaseOrderFreight AS TARGET
						USING @TablePurchaseOrderFreightType AS SOURCE ON (TARGET.PurchaseOrderId = SOURCE.PurchaseOrderId AND 
					  													     TARGET.PurchaseOrderFreightId = SOURCE.PurchaseOrderFreightId) 
						WHEN MATCHED 
						THEN UPDATE 
						SET
						...
						THEN
							INSERT([PurchaseOrderId],[PurchaseOrderPartRecordId],[ItemMasterId],[PartNumber],[ShipViaId],
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'PROCInsertPurchaseOrderFreightDetails' 
