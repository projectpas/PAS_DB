 ...
 ** --   --------         -------		     ----------------------------       
	...
	9	 21-Aug-2025	Bhargav saliya		 added integrationPortal
**********************/
...
BEGIN	
		...
		;WITH CTE_IntegrationPortal AS (
			SELECT
				iM.ItemMasterId,
				STRING_AGG(CAST(R.[Description] AS NVARCHAR(MAX)), ',') AS integrationPortal,
				STRING_AGG(mp.IntegrationPortalId, ',') AS IntegrationPortalStringIds
			FROM dbo.ItemMaster iM WITH(NOLOCK)
			INNER JOIN dbo.ItemMasterIntegrationPortal mp WITH(NOLOCK) ON iM.ItemMasterId = mp.ItemMasterId
			INNER JOIN dbo.Ranking R WITH(NOLOCK) ON mp.IntegrationPortalId = R.RankingId
		 ...
		 Result AS(
				SELECT DISTINCT im.ItemMasterId,
                       ...
                       im.UpdatedBy,	
					   im.IsDeleted,
					   itp.integrationPortal as RankingsName
			   FROM dbo.ItemMaster im WITH (NOLOCK)	
