 ...
 ** --   --------      -------			-------------------------------            
    ...
    7    27/06/2025    Moin Bloch		Modify(Changed TO New Table)
 ...
 BEGIN TRY  
    ...
    SELECT [IP].PaymentId,   
     ...
     CASE WHEN ([IP].[InvoiceType] = @CREDITMEMO OR [IP].[InvoiceType] = @STANDALONECREDITMEMO OR [IP].[InvoiceType] = @MANUALJOURNAL)
	      THEN 0 
		  ELSE CASE WHEN [IP].[InvoiceType] = @SOInvoiceType AND SOBI.[IsPerformaInvoice] = 1 
				    THEN 0 
				    WHEN [IP].[InvoiceType] = @SOInvoiceType AND ISNULL(SOBI.[IsPerformaInvoice],0) = 0
				    THEN DATEDIFF(DAY, SOBI.[InvoiceDate], GETUTCDATE())
				    ...
				    THEN DATEDIFF(DAY, WOBI.[InvoiceDate], GETUTCDATE())
				    WHEN [IP].[InvoiceType] = @ESOInvoiceType
				    THEN DATEDIFF(DAY, ESOBI.[InvoiceDate], GETUTCDATE())
			END
	  END AS 'DSI', 
	  CASE WHEN (IP.InvoiceType = @CREDITMEMO OR [IP].[InvoiceType] = @STANDALONECREDITMEMO OR [IP].[InvoiceType] = @MANUALJOURNAL)
	      THEN 0 
...
WHERE [IP].[CustomerId] = @customerId AND [IP].[ReceiptId] = @receiptId AND [IP].[IsDeleted]=0  
     ...
     SOBI.[IsPerformaInvoice],WOBI.[IsPerformaInvoice],SOBI.[InvoiceDate],WOBI.[InvoiceDate],ESOBI.[InvoiceDate],WO.[NetDays],ES.[NetDays]
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------  
        ...
        , @ProcedureParameters VARCHAR(3000)  = '@Parameter1 = '''+ ISNULL(CAST(@customerId AS VARCHAR(100)), '') + ''  
