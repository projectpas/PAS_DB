...
**************************************************************/
CREATE   PROCEDURE [dbo].[USP_AddUpdateWorkOrderLaborTrackingDetailScheduler]
	...
	SET NOCOUNT ON;
		...
		BEGIN TRANSACTION
			BEGIN  
				...
				SELECT TOP 1 @woLaborTrackingId = ISNULL(WorkOrderLaborTrackingId,0) ,@scanStartTime = StartTime 
				FROM dbo.WorkOrderLaborTracking WITH(NOLOCK) WHERE TaskId = @TaskId AND EmployeeId = @EmployeeId AND ISNULL(IsCompleted,0) = 0 AND WorkOrderLaborId = @woLaborId
				...
				BEGIN
						...
						BEGIN
							...
							BEGIN
							   ...
							   SELECT @LaborlogoffHours=ISNULL(LaborlogoffHours,0) from WorkOrderSettings wos WITH(NOLOCK)  WHERE wos.MasterCompanyId=@MasterCompanyId AND wos.WorkOrderTypeId = @WorkOrderTypeId
								...
								FROM dbo.WorkOrderLabor WL  WITH(NOLOCK) LEFT JOIN dbo.[Percent] P WITH(NOLOCK) on Wl.BurdaenRatePercentageId = P.PercentId 
