...
BEGIN    
  ...
  BEGIN  
   ...
   SELECT @ItemMasterId_Part = POP.ItemMasterId, @MainPOPartBackOrderQty = POP.QuantityBackOrdered, @ItemTypeId = ItemTypeId FROM DBO.PurchaseOrderPart POP WITH (NOLOCK) WHERE POP.PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId;  
   ...
   BEGIN  
    ...
    INSERT INTO #tmpStocklineDraft SELECT StockLineDraftId,PartNumber,StockLineNumber,StocklineMatchKey,ControlNumber,ItemMasterId,Quantity,ConditionId,SerialNumber,ShelfLife,  
    ShelfLifeExpirationDate,WarehouseId,LocationId,ObtainFrom,Owner,TraceableTo,ManufacturerId,Manufacturer,ManufacturerLotNumber,ManufacturingDate,ManufacturingBatchNumber,  
    PartCertificationNumber,CertifiedBy,CertifiedDate,TagDate,TagTypeIds,TagType,CertifiedDueDate,CalibrationMemo,OrderDate,PurchaseOrderId,PurchaseOrderUnitCost,InventoryUnitCost,  
    RepairOrderId,RepairOrderUnitCost,ReceivedDate,ReceiverNumber,ReconciliationNumber,UnitSalesPrice,CoreUnitCost,GLAccountId,AssetId,IsHazardousMaterial,IsPMA,IsDER,OEM,Memo,  
    ManagementStructureEntityId,LegalEntityId,MasterCompanyId,CreatedBy,UpdatedBy,CreatedDate,UpdatedDate,isSerialized,ShelfId,BinId,SiteId,ObtainFromType,OwnerType,TraceableToType,  
    UnitCostAdjustmentReasonTypeId,UnitSalePriceAdjustmentReasonTypeId,IdNumber,QuantityToReceive,PurchaseOrderExtendedCost,ManufacturingTrace,ExpirationDate,AircraftTailNumber,  
    ShippingViaId,EngineSerialNumber,QuantityRejected,PurchaseOrderPartRecordId,ShippingAccount,ShippingReference,TimeLifeCyclesId,TimeLifeDetailsNotProvided,WorkOrderId,  
    WorkOrderMaterialsId,QuantityReserved,QuantityTurnIn,QuantityIssued,QuantityOnHand,QuantityAvailable,QuantityOnOrder,QtyReserved,QtyIssued,BlackListed,BlackListedReason,  
    Incident,IncidentReason,Accident,AccidentReason,RepairOrderPartRecordId,isActive,isDeleted,WorkOrderExtendedCost,RepairOrderExtendedCost,NHAItemMasterId,TLAItemMasterId,  
    IsParent,ParentId,IsSameDetailsForAllParts,Level1,Level2,Level3,Level4,Condition,Warehouse,Location,ObtainFromName,OwnerName,TraceableToName,GLAccount,AssetName,LegalEntityName,  
    ShelfName,BinName,SiteName,ObtainFromTypeName,OwnerTypeName,TraceableToTypeName,UnitCostAdjustmentReasonType,UnitSalePriceAdjustmentReasonType,ShippingVia,WorkOrder,  
    WorkOrderMaterialsName,TagTypeId,StockLineDraftNumber,StockLineId,TaggedBy,TaggedByName,UnitOfMeasureId,UnitOfMeasure,RevisedPartId,RevisedPartNumber,TaggedByType,TaggedByTypeName,  
    CertifiedById,CertifiedTypeId,CertifiedType,CertTypeId,CertType,IsCustomerStock,isCustomerstockType,CustomerId,CalibrationVendorId,PerformedById,LastCalibrationDate,NextCalibrationDate,  
    LotId,SalesOrderId,SubWorkOrderId,ExchangeSalesOrderId,WOQty,SOQty,ForStockQty,IsLotAssigned,LOTQty,LOTQtyReserve,OriginalCost,POOriginalCost,ROOriginalCost,VendorRMAId,  
    VendorRMADetailId,LotMainStocklineId,IsFromInitialPO,LotSourceId,Adjustment,SerialNumberNotProvided,ShippingReferenceNumberNotProvided,IsStkTimeLife   
    FROM DBO.StocklineDraft StkDraft WITH(NOLOCK) WHERE StkDraft.PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId AND IsParent = 1 AND StockLineNumber IS NULL ORDER BY CreatedDate;  
    ...
    BEGIN  
     ...
     INSERT INTO #InsertedStkForLot(StockLineId) SELECT @NewStocklineId  
     IF(@IsTimeLIfe = 1)  
     BEGIN  
     ...
     BEGIN  
      ...
      BEGIN  
       ...
       SELECT DISTINCT @ItemMasterId, POP.PartNumber, CASE WHEN IM.ItemMasterId IS NOT NULL THEN IM.PurchaseUnitOfMeasureId ELSE 0 END, CASE WHEN IM.ItemMasterId IS NOT NULL THEN IM.PurchaseCurrencyId ELSE 0 END, POP.ForeignExchangeRate, @PP_VendorListPrice, GETUTCDATE(), @PP_PurchaseDiscPerc,  
   ...
   BEGIN  
    ...
    SELECT @IsSerializedPart = Ass.isSerialized FROM DBO.Asset Ass WITH (NOLOCK) WHERE Ass.AssetRecordId = @ItemMasterId_Part;  
    SELECT @CurrentIdNumber_Asset = ISNULL(CP.CurrentNummber, 0) FROM dbo.CodePrefixes CP WITH(NOLOCK) WHERE CP.CodeTypeId = 63 AND CP.MasterCompanyId = @MasterCompanyId AND CP.IsActive = 1 AND CP.IsDeleted = 0;  
    ...
    BEGIN  
     ...
     SELECT CodePrefixId, CP.CodeTypeId, CurrentNummber, CodePrefix, CodeSufix, StartsFrom  
     FROM dbo.CodePrefixes CP WITH(NOLOCK) JOIN dbo.CodeTypes CT WITH (NOLOCK) ON CP.CodeTypeId = CT.CodeTypeId  
     WHERE CT.CodeTypeId = @IdCodeTypeId_Asset AND CP.MasterCompanyId = @MasterCompanyId AND CP.IsActive = 1 AND CP.IsDeleted = 0;  
  
     --IF (@CurrentIndex = 0)  
     --BEGIN  
     -- SELECT @CurrentIdNumber_Asset = CASE WHEN CurrentNumber > 0 THEN CAST(CurrentNumber AS BIGINT) ELSE CAST(StartsFrom AS BIGINT) END     
     -- FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = @IdCodeTypeId_Asset  
     --END  
     --ELSE  
     --BEGIN  
     -- SELECT @CurrentIdNumber_Asset = CASE WHEN CurrentNumber > 0 THEN CAST(CurrentNumber AS BIGINT) + 1 ELSE CAST(StartsFrom AS BIGINT) + 1 END  
     -- FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = @IdCodeTypeId_Asset  
     --END  
  
     SET @ReceiverNumber_Asset = (SELECT * FROM dbo.udfGenerateCodeNumberWithOutDash(@CurrentIdNumber_Asset, 'RecNo', (SELECT CodeSufix FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = @IdCodeTypeId_Asset)))    
     ...
     SELECT @ItemMasterId_Asset = AssetRecordId, @AssetId_Asset = AssetId, @StkPurchaseOrderUnitCost_Asset = UnitCost, @ManufacturerId_Asset = ManufacturerId, @PreviousStockLineNumber_Asset = StklineNumber, @PartNumber_Asset = PartNumber FROM dbo.AssetInventoryDraft WITH(NOLOCK) WHERE AssetInventoryDraftId = @SelectedStockLineDraftId_Asset;  
     ...
     BEGIN  
      ...
      SELECT @CntrlNumber = ASST.CntrlNumber FROM DBO.Asset ASST WITH (NOLOCK) WHERE ASST.AssetId = @AssetId_Asset AND ASST.ManufacturerPN = @PartNumber_Asset AND ASST.MasterCompanyId = @MasterCompanyId;  
      ...
      WHERE AssetId = @AssetId_Asset AND ManufacturerPN = @PartNumber_Asset AND MasterCompanyId = @MasterCompanyId;  
     END  
     ELSE  
     BEGIN  
     ...
     BEGIN  
      PRINT 'Inside'  
      SET @StockLineNumber = (SELECT * FROM dbo.udfGenerateCodeNumberWithOutDash(@stockLineCurrentNo_Asset,(SELECT CodePrefix FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = 63), (SELECT CodeSufix FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = 63)))    
     ...
     BEGIN  
      SELECT @CNCurrentNumber_Asset = CASE WHEN CurrentNumber > 0 THEN CAST(CurrentNumber AS BIGINT) + 1 ELSE CAST(StartsFrom AS BIGINT) + 1 END FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = 64;  
      SET @ControlNumber_Asset = (SELECT * FROM dbo.udfGenerateCodeNumberWithOutDash(@CNCurrentNumber_Asset,(SELECT CodePrefix FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = 64), (SELECT CodeSufix FROM #tmpCodePrefixes_Asset WHERE CodeTypeId = 64)))  
  
      UPDATE DBO.CodePrefixes  
      SET CurrentNummber = @CNCurrentNumber_Asset  
      WHERE CodeTypeId = 64 AND MasterCompanyId = @MasterCompanyId;  
     END  
  
     INSERT INTO DBO.AssetInventory ([AssetRecordId],[AssetId],[AlternateAssetRecordId],[Name],[Description],[ManagementStructureId],[CalibrationRequired],[CertificationRequired],[InspectionRequired],  
     [VerificationRequired],[IsTangible],[IsIntangible],[AssetAcquisitionTypeId],[ManufacturerId],[ManufacturedDate],[Model],[IsSerialized],[UnitOfMeasureId],[CurrencyId],[UnitCost],[ExpirationDate],[Memo],  
     [AssetParentRecordId],[TangibleClassId],[AssetIntangibleTypeId],[AssetCalibrationMin],[AssetCalibrationMinTolerance],[AssetCalibratonMax],[AssetCalibrationMaxTolerance],[AssetCalibrationExpected],[AssetCalibrationExpectedTolerance],  
     ...
     [MaintenanceGLAccountId],[MaintenanceMemo],[IsWarrantyRequired],[WarrantyCompany],[WarrantyStartDate],[WarrantyEndDate],[WarrantyStatusId],[UnexpiredTime],[MasterCompanyId],[AssetLocationId],[IsDeleted],  
     [Warranty],[IsActive],[CalibrationDefaultVendorId],[CertificationDefaultVendorId],[InspectionDefaultVendorId],[VerificationDefaultVendorId],[CertificationFrequencyMonths],[CertificationFrequencyDays],  
     [CertificationDefaultCost],[CertificationGlAccountId],[CertificationMemo],[InspectionMemo],[InspectionGlaAccountId],[InspectionDefaultCost],[InspectionFrequencyMonths],[InspectionFrequencyDays],[VerificationFrequencyDays],  
     [VerificationFrequencyMonths],[VerificationDefaultCost],[CalibrationDefaultCost],[CalibrationFrequencyMonths],[CalibrationFrequencyDays],[CalibrationGlAccountId],[CalibrationMemo],[VerificationMemo],  
     [VerificationGlAccountId],[CalibrationCurrencyId],[CertificationCurrencyId],[InspectionCurrencyId],[VerificationCurrencyId],[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[AssetMaintenanceContractFileExt],  
     [WarrantyFile],[WarrantyFileExt],[MasterPartId],[EntryDate],[InstallationCost],[Freight],[Insurance],[Taxes],[TotalCost],[WarrantyDefaultVendorId],[WarrantyGLAccountId],[IsDepreciable],[IsNonDepreciable],  
     [IsAmortizable],[IsNonAmortizable],[SerialNo],[IsInsurance],[AssetLife],[WarrantyCompanyId],[WarrantyCompanyName],[WarrantyCompanySelectId],[WarrantyMemo],[IsQtyReserved],[InventoryStatusId],[InventoryNumber],  
     [AssetStatusId],[Level1],[Level2],[Level3],[Level4],[ManufactureName],[LocationName],[Qty],[StklineNumber],[AvailStatus],[PartNumber],[ControlNumber],[RepairOrderId],[RepairOrderPartRecordId],[PurchaseOrderId],  
     [PurchaseOrderPartRecordId],[ReceiverNumber],[ReceivedDate],[SiteId],[SiteName],[WarehouseId],[Warehouse],[LocationId],[Location],[ShelfId],[ShelfName],[BinId],[BinName],[StatusNote],[RRQty],[DepreciationMethodId],  
     [DepreciationMethodName],[ResidualPercentageId],[ResidualPercentage],[DepreciationFrequencyId],[DepreciationFrequencyName],[AcquiredGLAccountId],[AcquiredGLAccountName],[DeprExpenseGLAccountId],[DeprExpenseGLAccountName],       [AdDepsGLAccountId],[AdDepsGLAccountName],[AssetSaleGLAccountId],[AssetSaleGLAccountName],[AssetWriteOffGLAccountId],[AssetWriteOffGLAccountName],[AssetWriteDownGLAccountId],[AssetWriteDownGLAccountName],  
     [IntangibleGLAccountId],[IntangibleGLAccountName],[AmortExpenseGLAccountId],[AmortExpenseGLAccountName],[AccAmortDeprGLAccountId],[AccAmortDeprGLAccountName],[IntangibleWriteDownGLAccountId],[IntangibleWriteDownGLAccountName],  
     ...
     [VerificationGlAccountId],[CalibrationCurrencyId],[CertificationCurrencyId],[InspectionCurrencyId],[VerificationCurrencyId],[CreatedBy],[UpdatedBy],[CreatedDate],[UpdatedDate],[AssetMaintenanceContractFileExt],  
     [WarrantyFile],[WarrantyFileExt],[MasterPartId],[EntryDate],[InstallationCost],[Freight],[Insurance],[Taxes],[TotalCost],[WarrantyDefaultVendorId],[WarrantyGLAccountId],[IsDepreciable],[IsNonDepreciable],  
     [IsAmortizable],[IsNonAmortizable],[SerialNo],[IsInsurance],[AssetLife],[WarrantyCompanyId],[WarrantyCompanyName],[WarrantyCompanySelectId],[WarrantyMemo],[IsQtyReserved],1,@InventoryNumber_Asset,  
     [AssetStatusId],[Level1],[Level2],[Level3],[Level4],[ManufactureName],[LocationName],[Qty],@StockLineNumber,[AvailStatus],[PartNumber],@ControlNumber_Asset,[RepairOrderId],[RepairOrderPartRecordId],[PurchaseOrderId],  
     [PurchaseOrderPartRecordId],@ReceiverNumber_Asset,GETUTCDATE(),[SiteId],[SiteName],[WarehouseId],[Warehouse],[LocationId],[Location],[ShelfId],[ShelfName],[BinId],[BinName],'',0,NULL,  
     ...
     @SelectedIsSameDetailsForAllParts_Asset = IsSameDetailsForAllParts, @PurchaseOrderUnitCostAdded_Asset = UnitCost   
     FROM #tmpAssetInventoryDraft   
     WHERE AssetInventoryDraftId = @SelectedStockLineDraftId_Asset;  
     ...
     INSERT INTO @p3 VALUES(@NewStocklineId_Asset, @QtyAdded_Asset, @PurchaseOrderUnitCostAdded_Asset, 'ReceivingPO', @UpdatedBy, @MasterCompanyId, 'STOCK')  
     ...
     SELECT @ManagementStructureEntityId_Asset = [ManagementStructureId] FROM DBO.Stockline WHERE StocklineId = @NewStocklineId_Asset;  
  
     --EXEC dbo.[USP_SaveSLMSDetails] @StkManagementStructureModuleId_Asset, @NewStocklineId_Asset, @ManagementStructureEntityId_Asset, @MasterCompanyId, @UpdatedBy;  
     ...
     BEGIN  
      ...
      INSERT INTO #StocklineDraftForQtyToReceive_Asset ([AssetInventoryDraftId])  
      SELECT [AssetInventoryDraftId] FROM DBO.AssetInventoryDraft WITH (NOLOCK) WHERE PurchaseOrderId = @PurchaseOrderId AND PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId  
      AND IsParent = 0 AND isSerialized = 0 AND IsSameDetailsForAllParts = 1 AND AssetInventoryId IS NULL  
      ORDER BY AssetInventoryDraftId DESC;  
  
      SELECT @LoopID_QtyToReceive_Asset = MAX(ID) FROM #StocklineDraftForQtyToReceive_Asset;  
      ...
      UPDATE DBO.AssetInventoryDraft SET AssetInventoryId = 0   
      WHERE AssetInventoryDraftId = @SelectedStockLineDraftId_Asset AND isSerialized = 0 AND IsSameDetailsForAllParts = 1 AND IsParent = 1;  
     END  
     ELSE  
     BEGIN  
      UPDATE dstl  
   ...
   BEGIN  
	...
	SELECT @IsSerializedPart = IM.isSerialized FROM DBO.ItemMasterNonStock IM WITH (NOLOCK) WHERE IM.MasterPartId = @ItemMasterId_Part;  
    ...
    INSERT INTO #tmpNonStockInventoryDraft SELECT * FROM DBO.NonStockInventoryDraft NonStockDraft WITH(NOLOCK) WHERE NonStockDraft.PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId AND IsParent = 1 AND ISNULL(NonStockDraft.NonStockInventoryId
,0) = 0 ORDER BY NonStockInventoryDraftId;  
    ...
    BEGIN  
     ...
     SELECT TOP 1 @IsSameDetailsForAllParts_NS = StkDraft.IsSameDetailsForAllParts FROM DBO.NonStockInventoryDraft StkDraft WITH(NOLOCK) WHERE IsParent = 1 AND StkDraft.PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId;  
	
	--print @IsSameDetailsForAllParts_NS
 ...
 -- Print @NonStockTotalRec
     ...
     BEGIN  
      ...
      BEGIN  
       ...
       ;WITH tempNonStockInventory  ([PurchaseOrderId],[PurchaseOrderPartRecordId],[PurchaseOrderNumber],[RepairOrderId],[IsParent],[ParentId],[MasterPartId],[PartNumber],[PartDescription]  
        ...
        ,[NonStockInventoryNumber] ,[ControlNumber],[ControlID],[IdNumber],[ReceiverNumber],[ReceivedDate],[IsSerialized],[SerialNumber],CASE WHEN @IsSerializedPart = 1 THEN [Quantity] ELSE CASE WHEN IsSameDetailsForAllParts = 0 THEN [Quantity] ELSE @QtyToReceive END END,[QuantityRejected],CASE WHEN @IsSerializedPart = 1 THEN [Quantity] ELSE CASE WHEN IsSameDetailsForAllParts = 0 THEN [Quantity] ELSE @QtyToReceive END END  
        ,[CurrencyId] ,[Currency],[ConditionId],[Condition],[GLAccountId],[GLAccount],[UnitOfMeasureId],[UnitOfMeasure],[ManufacturerId],[Manufacturer],[MfgExpirationDate],[UnitCost]  
        ,[ExtendedCost],[Acquired],[IsHazardousMaterial],[ItemNonStockClassificationId],[NonStockClassification] ,[SiteId],[Site],[WarehouseId],[Warehouse],[LocationId],[Location]  
        ,[ShelfId],[Shelf],[BinId],[Bin],[ShippingViaId],[ShippingVia],[ShippingAccount],[ShippingReference],[IsSameDetailsForAllParts],[VendorId],[VendorName],[RequisitionerId]  
        ...
        ,[IsActive],[IsDeleted],CASE WHEN @IsSerializedPart = 1 THEN [Quantity] ELSE @QtyToReceive END FROM #tmpNonStockInventoryDraft WHERE NonStockInventoryDraftId = @TempNonStockId)  
        ...
        ,[NonStockInventoryNumber] ,[ControlNumber],[ControlID],[IdNumber],[ReceiverNumber],[ReceivedDate],[IsSerialized],[SerialNumber],[Quantity],[QuantityRejected],
		  [QuantityOnHand]
        ,[CurrencyId] ,[Currency],[ConditionId],[Condition],[GLAccountId],[GLAccount],[UnitOfMeasureId],[UnitOfMeasure],[ManufacturerId],[Manufacturer],[MfgExpirationDate],[UnitCost]  
        ,[ExtendedCost],[Acquired],[IsHazardousMaterial],[ItemNonStockClassificationId],[NonStockClassification] ,[SiteId],[Site],[WarehouseId],[Warehouse],[LocationId],[Location]  
        ,[ShelfId],[Shelf],[BinId],[Bin],[ShippingViaId],[ShippingVia],[ShippingAccount],[ShippingReference],[IsSameDetailsForAllParts],[VendorId],[VendorName],[RequisitionerId]  
  ...
  --IF EXISTS (SELECT TOP 1 1 FROM DBO.PurchaseOrderPart POP WITH (NOLOCK) WHERE POP.PurchaseOrderPartRecordId = @SelectedPurchaseOrderPartRecordId AND ISNULL(POP.WorkOrderMaterialsId, 0) > 0)  
  --BEGIN  
  -- DECLARE @SelectedPartsToReceive VARCHAR(50);  
  -- DECLARE @PurchaseOrderPartReferenceId BIGINT;  
  
  -- SELECT @PurchaseOrderPartReferenceId = PurchaseOrderPartReferenceId FROM DBO.PurchaseOrderPartReference POPR WITH (NOLOCK) WHERE POPR.PurchaseOrderId = @PurchaseOrderId AND POPR.PurchaseOrderPartId = @SelectedPurchaseOrderPartRecordId;  
  
  -- SET @SelectedPartsToReceive = CAST(@PurchaseOrderPartReferenceId AS VARCHAR(50));  
  
  -- EXEC dbo.USP_ReserveStocklineForReceivingPO @PurchaseOrderId = @PurchaseOrderId, @SelectedPartsToReserve = @SelectedPartsToReceive, @UpdatedBy = @UpdatedBy, @AllowAutoIssue = 1;  
  --END  
  
  DECLARE @IsAutoReserveReceivedStockline BIT = 0;  
  SELECT @IsAutoReserveReceivedStockline = ISNULL(POS.IsAutoReserveReceivedStockline, 0) FROM [dbo].[PurchaseOrderSettingMaster] POS WITH (NOLOCK) WHERE POS.MasterCompanyId = @MasterCompanyId;  
  
  IF (@IsAutoReserveReceivedStockline = 1)  
  BEGIN  
   DECLARE @SelectedPartsToReserve VARCHAR(500) = '';  
  
   SELECT @SelectedPartsToReserve = STUFF((  
    SELECT ',' + CAST(POPR.PurchaseOrderPartReferenceId AS VARCHAR(100))   
    FROM DBO.PurchaseOrderPartReference POPR WITH (NOLOCK)  
    WHERE POPR.PurchaseOrderId = @PurchaseOrderId  
    ORDER BY POPR.PurchaseOrderPartReferenceId FOR XML PATH('')  
   ),1, 1, '');  
