...
-- exec USP_GetCreditMemo_AccountingDetailsById 10079
************************************************************************/   
...
BEGIN    
 ...
 BEGIN TRY        
   ...
   FROM [dbo].[CommonBatchDetails] JBD WITH(NOLOCK)  
		...
		--LEFT JOIN [dbo].[Customer] C WITH(NOLOCK) ON CPD.CustomerId=C.CustomerId  
     ...
     WHERE CPD.ReferenceID = @ReferenceId     
  END    
  END TRY    
 BEGIN CATCH          
  IF @@trancount > 0    
