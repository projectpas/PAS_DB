 ...
 ** PR   Date         Author  			Change Description             
 1    21/07/2023   Hemant Saliya		Created 
 
**************************************************************       

EXEC [USP_GetIncomeStatementActualTrendReport] 1, 8, 1, 132,132
************************************************************************/
  
CREATE   PROCEDURE [dbo].[USP_GetIncomeStatementTrendBudgetReport]
(
...
BEGIN
  BEGIN TRY
        ...
        BEGIN
              INSERT INTO #AccTrendTable (LeafNodeId, NodeName, Amount, AccountingPeriodId, IsBlankHeader, IsTotlaLine)
							SELECT  LeafNodeId,'Total - ' + Name, TotalAmount,@AccountcalID, 0, 1
						      --SELECT  LeafNodeId, Name, TotalAmount,@AccountcalID, 0, 1
									FROM #TempTable
			 ...
			 IF(@IsFristRow = 1)
				...
				INSERT INTO ##AccTrendTablePivot(Name,IsBlankHeader,IsTotlaLine, LeafNodeId, ParentId)
						   ...
						   --SELECT TOP 1  Name,0, 1, LeafNodeId, LeafNodeId
										 FROM #TempTable
										     WHERE LeafNodeId = @CLID  AND ChildCount > 0  AND  AccountingPeriodId = @AccountcalID
	...
	--SELECT * FROM #AccTrendTable --where IsTotlaLine = 1
	--SELECT * INTO #TmpAccTrendTable
	--FROM #AccTrendTable where IsTotlaLine = 1

	--UPDATE #TmpAccTrendTable SET NodeName = REPLACE(NodeName,'Total -',' ') FROM #TmpAccTrendTable

	--SELECT ID, LeafNodeId,  UPPER(NodeName) NodeName, Amount, AccountingPeriod, AccountingPeriod, IsBlankHeader, IsTotlaLine FROM #TmpAccTrendTable
	...
	--Final Results
	  ...
	  UPDATE ##AccTrendTablePivot SET [name] = UPPER(REPLACE([name],'Total -','')) FROM ##AccTrendTablePivot
	  UPDATE ##AccTrendTablePivot SET [name] = LTRIM([name]) FROM ##AccTrendTablePivot

	SELECT * FROM ##AccTrendTablePivot WHERE IsBlankHeader != 1 AND IsTotlaLine = 1 --Order BY parentId 
