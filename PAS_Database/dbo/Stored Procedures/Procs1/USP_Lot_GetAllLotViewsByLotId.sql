 ...
 ** --   --------     -------		--------------------------------          
	...
	8    25-Aug-2026			 RAJESH GAMI						[PN-17745] Ported from PAS_DB - HowAcquired/AcquiredRef CASE expressions in the ViewAllPN branch now also recognize the new 'Turn In' type (in addition to 'Trans In(Lot)') so stocklines created via "Create Stockline from Lot" are still displayed correctly.
	...
	SET NOCOUNT ON;
		...
		BEGIN		
			...
			BEGIN
				...
				,(CASE WHEN ltCal.Type = @LOT_TransIn_LOT OR ltCal.Type = @LOT_TransOut_LOT OR ltCal.Type = @LOT_TurnIn THEN @LotTransIn
			...
			BEGIN
				...
				FROM DBO.LOT lot WITH(NOLOCK)
					 ...
					 LEFT JOIN DBO.BillingInvoicingItems sobii on sop.SalesOrderPartId = sobii.SubReferenceId AND sobi.BillingInvoicingId = sobii.BillingInvoicingId AND ISNULL(sobii.IsPerformaInvoice,0) = 0 AND sobi.[ModuleId] = @SOModuleId
					 LEFT JOIN DBO.ItemClassification ic WITH(NOLOCK) ON im.ItemClassificationId = ic.ItemClassificationId
					 LEFT JOIN DBO.ItemGroup ig WITH(NOLOCK) ON im.ItemGroupId = ig.ItemGroupId
					 LEFT JOIN DBO.Condition c WITH(NOLOCK) ON c.ConditionId = sl.ConditionId
					 LEFT JOIN DBO.PurchaseOrder po WITH(NOLOCK) ON sl.PurchaseOrderId = po.PurchaseOrderId 
