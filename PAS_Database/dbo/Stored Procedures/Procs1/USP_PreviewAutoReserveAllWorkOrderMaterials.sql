			...
			BEGIN TRANSACTION
				BEGIN
					...
					SELECT DENSE_RANK() OVER (ORDER BY ISNULL(wom.StockLineId,0) DESC) AS RowNumber , 
						...
						[UnReservedQty],
						[UnIssuedQty],
						[IssuedById],[ReservedById],[IsEquPart],[ItemMappingId],
						[TotalReserved],
						[TotalIssued],
						[TotalUnReserved],
						[TotalUnIssued],
						[ProvisionId],[MaterialMandatoriesId],[WOPartNoId],
						...
						wom.[QuantityTurnIn],
						UPPER([stkFigure]) AS [stkFigure],UPPER([stkItem]) AS [stkItem],UPPER(wom.[PartNumber]) AS [PartNumber],
