/*************************************************************               
 ** File:   [USP_SaveNonPOInvoiceMSDetails]               
 ** Author:   Devendra Shekh      
 ** Description: to add managementstructuredetails for nonpoinvoice
 ** Purpose:             
 ...
 ** --   --------         -------			--------------------------------              
    1    15-SEP-2023	Devendra Shekh			Created    
 **************************************************************/   

CREATE   PROCEDURE [dbo].[USP_SaveNonPOInvoiceMSDetails]  
@ModuleID INT,
@ReferenceID BIGINT,
@EntityMSID BIGINT, 
@MasterCompanyId INT,
		...
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					...
					INSERT INTO [dbo].[NonPOInvoiceManagementStructureDetails]
					...
					SELECT @MSDetailsId = IDENT_CURRENT('NonPOInvoiceManagementStructureDetails');

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[NonPOInvoiceManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

					EXECUTE(@Query)  

					UPDATE [dbo].[NonPOInvoiceManagementStructureDetails] 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_SaveNonPOInvoiceMSDetails' 
