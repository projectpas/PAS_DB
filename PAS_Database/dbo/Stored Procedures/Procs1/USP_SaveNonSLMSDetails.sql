...
/*************************************************************           
 ** File:   [USP_SaveNonSLMSDetails]           
 ** Author:   Hemant Saliya
 ** Description: This SP is Used to Save Managment Structure Details
 ** Purpose:         
 ** Date:   02/17/2022        
 ...
 ** --   --------     -------		--------------------------------          
 ...
 EXECUTE USP_SaveNonSLMSDetails 2

**************************************************************/ 
    
CREATE PROCEDURE [dbo].[USP_SaveNonSLMSDetails]    
(    
@ModuleID INT,
@ReferenceID BIGINT,
@EntityMSID BIGINT, 
		...
		BEGIN TRY
			BEGIN TRANSACTION
				BEGIN
					...
					DECLARE @MSDetailsId BIGINT; -- For Non Stockline
					...
					INSERT INTO [dbo].[NonStocklineManagementStructureDetails]
					...
					SELECT @MSDetailsId = IDENT_CURRENT('NonStocklineManagementStructureDetails');

					SELECT @MSLevel = MC.ManagementStructureLevel
					FROM [dbo].[MasterCompany] MC WITH(NOLOCK) 
					WHERE MC.MasterCompanyId = @MasterCompanyId

					SET @Query = N'INSERT INTO #TempTable (LastMSName) SELECT DISTINCT TOP 1 CAST ( Level' + CAST( + @MSLevel AS VARCHAR(20)) + 'Name AS VARCHAR(MAX)) FROM [dbo].[NonStocklineManagementStructureDetails] MSD WITH(NOLOCK) 
					WHERE MSD.MSDetailsId = CAST (' + CAST(@MSDetailsId AS VARCHAR(20)) + ' AS INT)'

					EXECUTE(@Query)  

					UPDATE [dbo].[NonStocklineManagementStructureDetails] 
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
              , @AdhocComments     VARCHAR(150)    = 'USP_SaveNonSLMSDetails' 
