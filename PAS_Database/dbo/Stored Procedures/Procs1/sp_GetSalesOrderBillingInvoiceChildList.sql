		...
		BEGIN 
			...
			BEGIN 
				INSERT INTO #SalesOrderBillingInvoiceChildList(
					...
					TotalSales ,InvoiceStatus ,	SmentNo ,VersionNo ,IsVersionIncrease ,	IsNewInvoice,IsProforma )
				(
					SELECT 0 AS SalesOrderShippingId,   
					sobi.SOBillingInvoicingId,
					sobi.InvoiceDate,
					sobi.InvoiceNo AS InvoiceNo,
					'' AS SOShippingNum, 
					(SELECT ISNULL(SUM(SORR.QtyToReserve), 0) FROM DBO.SalesOrderReserveParts SORR WITH (NOLOCK) WHERE SORR.SalesOrderPartId = sopfinal.SalesOrderPartId AND SORR.StockLineId = sopfinal.StockLineId) as QtyToBill,   
					so.SalesOrderNumber, imt.partnumber, imt.ItemMasterId,sopfinal.ConditionId, imt.PartDescription, sl.StockLineNumber,  
					sl.SerialNumber, cr.[Name] as CustomerName,   
					sopfinal.StockLineId,  
					(SELECT b.NoofPieces FROM dbo.SalesOrderBillingInvoicing a WITH (NOLOCK) 
						INNER JOIN dbo.SalesOrderBillingInvoicingItem b WITH (NOLOCK) ON a.SOBillingInvoicingId = b.SOBillingInvoicingId 
						WHERE b.SOBillingInvoicingItemId = SOBII.SOBillingInvoicingItemId
						AND ISNULL(a.IsProforma,0) = 0 AND ISNULL(b.IsProforma,0) = 0) AS QtyBilled,  
					sopfinal.ItemNo,  
					sopfinal.SalesOrderId, sopfinal.SalesOrderPartId, cond.Description as 'Condition',   
					curr.Code as 'CurrencyCode',  
					CASE WHEN ISNULL(sobi.SOBillingInvoicingId, 0) = 0 THEN ((ISNULL(sopfinal.UnitSalesPricePerUnit, 0) * ISNULL(SOR.QtyToReserve, 0)) +   
					((((ISNULL(sopfinal.UnitSalesPricePerUnit, 0) * ISNULL(SOR.QtyToReserve, 0)) +
					(SELECT ISNULL(SUM(BillingAmount), 0) FROM dbo.SalesOrderFreight sof WITH (NOLOCK) WHERE sof.SalesOrderId = @SalesOrderId AND sof.ItemMasterId = sopfinal.ItemMasterId AND sof.ConditionId = @ConditionId AND sof.IsActive = 1 AND sof.IsDeleted = 0) +   
					(SELECT ISNULL(SUM(BillingAmount), 0) FROM dbo.SalesOrderCharges socg WITH (NOLOCK) WHERE socg.SalesOrderId = @SalesOrderId AND socg.ItemMasterId = sopfinal.ItemMasterId AND socg.ConditionId = @ConditionId AND socg.IsActive = 1 AND socg.IsDeleted = 0)
					) * ISNULL(sopfinal.TaxPercentage, 0)) / 100) +   
					(SELECT ISNULL(SUM(BillingAmount), 0) FROM dbo.SalesOrderFreight sof WITH (NOLOCK) WHERE sof.SalesOrderId = @SalesOrderId AND sof.ItemMasterId = sopfinal.ItemMasterId AND sof.ConditionId = @ConditionId AND sof.IsActive = 1 AND sof.IsDeleted = 0) +   
					(SELECT ISNULL(SUM(BillingAmount), 0) FROM dbo.SalesOrderCharges socg WITH (NOLOCK) WHERE socg.SalesOrderId = @SalesOrderId AND socg.ItemMasterId = sopfinal.ItemMasterId AND socg.ConditionId = @ConditionId AND socg.IsActive = 1 AND socg.IsDeleted = 0))
					ELSE sobi.GrandTotal END as 'TotalSales',  
					(SELECT a.InvoiceStatus FROM dbo.SalesOrderBillingInvoicing a WITH (NOLOCK) 
					...
					LEFT JOIN DBO.SalesOrderPart SOPR WITH (NOLOCK) on SOPR.SalesOrderId = sop.SalesOrderId AND SOPR.SalesOrderPartId = soapr.SalesOrderPartId
					LEFT JOIN DBO.SalesOrderPart ApprPart WITH (NOLOCK) on ApprPart.SalesOrderId = sop.SalesOrderId AND ApprPart.ItemMasterId = sop.ItemMasterId AND ApprPart.ConditionId = sop.ConditionId
					LEFT JOIN DBO.SalesOrderPart sopfinal WITH (NOLOCK) on sopfinal.SalesOrderPartId = ApprPart.SalesOrderPartId
					LEFT JOIN DBO.SalesOrderBillingInvoicingItem sobii WITH (NOLOCK) on sobii.SalesOrderPartId = sopfinal.SalesOrderPartId AND ISNULL(sobii.IsProforma,0) = 0
					LEFT JOIN DBO.SalesOrderBillingInvoicing sobi WITH (NOLOCK) on sobi.SOBillingInvoicingId = sobii.SOBillingInvoicingId AND ISNULL(sobi.IsProforma,0) = 0 
					LEFT JOIN DBO.Stockline sl WITH (NOLOCK) on sl.StockLineId = sopfinal.StockLineId  
					WHERE sop.SalesOrderId = @SalesOrderId AND sop.ItemMasterId = @SalesOrderPartId AND sop.ConditionId = @ConditionId 
					--AND ((ISNULL(soapr.SalesOrderApprovalId, 0) > 0   
					--AND (ISNULL(SOR.SalesOrderReservePartId, 0) > 0) AND (ISNULL(SOR.TotalReserved, 0) > 0)))
					AND ISNULL(soapr.SalesOrderPartId, 0) > 0  
					AND ((ISNULL(SOR.SalesOrderReservePartId, 0) > 0) AND (ISNULL(SOR.TotalReserved, 0) > 0))
					)
				--ORDER BY sobi.SOBillingInvoicingId DESC
			END
