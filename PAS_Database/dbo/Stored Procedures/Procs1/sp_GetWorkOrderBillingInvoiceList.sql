			...
			BEGIN
				...
				BEGIN
					...
					SELECT 
						...
						CASE WHEN ISNULL((Select SUM(ISNULL(WOBI.QtyBilled,0)) FROM BillingInvoicing WOB inner join  BillingInvoicingItems WOBI on WOB.BillingInvoicingId = WOBI.BillingInvoicingId where ISNULL(WOB.IsVersionIncrease,0) = 0 and WOB.ReferenceId = wo.WorkOrderId AND WOBI.SubReferenceId = wop.ID AND ISNULL(WOBI.IsPerformaInvoice, 0) = 0),0) <0 THEN 0 ELSE
										ISNULL((Select SUM(ISNULL(WOBI.QtyBilled,0)) FROM BillingInvoicing WOB inner join  BillingInvoicingItems WOBI on WOB.BillingInvoicingId = WOBI.BillingInvoicingId where ISNULL(WOB.IsVersionIncrease,0) = 0 and WOB.ReferenceId = wo.WorkOrderId AND WOBI.SubReferenceId = wop.ID AND ISNULL(WOBI.IsPerformaInvoice, 0) = 0),0) END AS QtyBilled,
						...
						CASE WHEN SUM(ISNULL(wosi.QtyShipped, 0)) = (SELECT SUM(ISNULL(QtyBilled, 0)) FROM BillingInvoicingItems wobII Where wobII.ItemMasterId = imt.ItemMasterId AND ISNULL(wobII.IsPerformaInvoice, 0) = 0) THEN 'Fullfilled'
						END as [NewStatus],
				...
				BEGIN
					...
					SELECT 
						...
						CASE WHEN ISNULL(wop.RevisedItemmasterid, 0) > 0 THEN wop.RevisedItemmasterid ELSE imt.ItemMasterId END As ItemMasterId,
						wop.ID as WorkOrderPartId ,
						CASE WHEN ((SELECT SUM(ISNULL(WP.Quantity, 0)) FROM dbo.WorkOrderPartNumber  WP WHERE  WP.WorkOrderId = wo.WorkOrderId AND WP.ID = wop.ID)) - ISNULL((Select SUM(ISNULL(WOBI.QtyBilled,0)) FROM BillingInvoicing WOB inner join  BillingInvoicingItems WOBI on WOB.BillingInvoicingId = WOBI.BillingInvoicingId where ISNULL(WOB.IsVersionIncrease,0) = 0 and WOB.ReferenceId = wo.WorkOrderId AND WOBI.SubReferenceId = wop.ID AND ISNULL(WOBI.IsPerformaInvoice, 0) = 0),0) < 0 THEN 0 ELSE ((SELECT SUM(ISNULL(WP.Quantity, 0)) FROM dbo.WorkOrderPartNumber  WP WHERE  WP.WorkOrderId = wo.WorkOrderId AND WP.ID = wop.ID)) - ISNULL((Select SUM(ISNULL(WOBI.QtyBilled,0)) FROM BillingInvoicing WOB inner join  BillingInvoicingItems WOBI on WOB.BillingInvoicingId = WOBI.BillingInvoicingId where ISNULL(WOB.IsVersionIncrease,0) = 0 and WOB.ReferenceId = wo.WorkOrderId AND WOBI.SubReferenceId = wop.ID AND ISNULL(WOBI.IsPerformaInvoice, 0) = 0),0) END as QtyRemaining,
						CASE WHEN 
						...
						--CASE WHEN SUM(ISNULL(wopick.QtyToShip, 0)) = (SELECT SUM(ISNULL(NoofPieces, 0)) FROM BillingInvoicingItems wobII Where wobII.ItemMasterId = imt.ItemMasterId) THEN 'Fullfilled'
						--END as [Status],
						CASE WHEN SUM(ISNULL(wop.Quantity, 0)) = (SELECT SUM(ISNULL(QtyBilled, 0)) FROM BillingInvoicingItems wobII Where wobII.ItemMasterId = imt.ItemMasterId AND ISNULL(wobII.IsPerformaInvoice, 0) = 0) THEN 'Fullfilled'
						END as [NewStatus],
