CREATE Procedure [dbo].[sp_UpdateExchangeSOBillingInvoiceDetail]
...
BEGIN
	...
	BEGIN
		...
		IF OBJECT_ID(N'tempdb..#ExchangeSOBillingInvoicingMSDATA') IS NOT NULL
		BEGIN
		DROP TABLE #ExchangeSOBillingInvoicingMSDATA
		END
		CREATE TABLE #ExchangeSOBillingInvoicingMSDATA
		(
		 MSID bigint,
		...
		INSERT INTO #ExchangeSOBillingInvoicingMSDATA
		...
		FROM dbo.ExchangeSalesOrderBillingInvoicing SB WITH (NOLOCK)
		LEFT JOIN #ExchangeSOBillingInvoicingMSDATA PMS ON PMS.MSID = @ManagementStructureId
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
            , @AdhocComments     VARCHAR(150)    = 'sp_UpdateExchangeSOBillingInvoiceDetail' 
