...
BEGIN
	...
	BEGIN TRAN
		...
		DECLARE @MainLoopId int = 0, @MainKitLoopId int = 0, @MaterialTotalCount int = 0, @MaterialKitTotalCount int = 0
		DECLARE @WorkOrderMaterialsId bigint, @WorkOrderMaterialsKitId bigint
			IF OBJECT_ID(N'tempdb..#tempTableBulk') IS NOT NULL
			BEGIN
				DROP TABLE #tempTableBulk
			END
			CREATE TABLE #tempTableBulk(
			   [ID] [bigint] IDENTITY(1,1) NOT NULL,
			   [WorkOrderMaterialsId] [bigint] NULL,
			)
			IF OBJECT_ID(N'tempdb..#tempTableBulkKit') IS NOT NULL
			BEGIN
				DROP TABLE #tempTableBulkKit
			END
			CREATE TABLE #tempTableBulkKit(
			   [ID] [bigint] IDENTITY(1,1) NOT NULL,
			   [WorkOrderMaterialsKitId] [bigint] NULL,
			)
			
		INSERT INTO #tempTableBulk (WorkOrderMaterialsId)  SELECT value FROM STRING_SPLIT(@WorkOrderMaterialsIds,',')
		INSERT INTO #tempTableBulkKit (WorkOrderMaterialsKitId)  SELECT value FROM STRING_SPLIT(@WorkOrderMaterialsKitIds,',')

		Select @MaterialTotalCount = Count(*),@MainLoopId =MIN(ID) from #tempTableBulk
		WHILE  @MaterialTotalCount >0
		BEGIN
			
			SET @WorkOrderMaterialsId = (SELECT TOP 1 WorkOrderMaterialsId FROM #tempTableBulk WHERE ID = @MainLoopId)
			...
			where POP.PurchaseOrderId = @PurchaseOrderId  AND POP.isParent = 1 AND POP.WorkOrderId > 0 and ISNULL(POP.SubWorkOrderId,0)  = 0 AND WOM.WorkOrderMaterialsId = @WorkOrderMaterialsId

			set @MaterialTotalCount = @MaterialTotalCount - 1;
			set @MainLoopId = @MainLoopId +1;
		END		


		Select @MaterialKitTotalCount = Count(*),@MainKitLoopId =MIN(ID) from #tempTableBulk
		WHILE  @MaterialKitTotalCount >0
		BEGIN
			SET @WorkOrderMaterialsKitId = (SELECT TOP 1 WorkOrderMaterialsKitId FROM #tempTableBulkKit WHERE ID = @MainKitLoopId)
			...
			JOIN dbo.PurchaseOrder P ON P.PurchaseOrderId = POP.PurchaseOrderId
			where POP.PurchaseOrderId = @PurchaseOrderId  AND POP.isParent = 1 AND POP.WorkOrderId > 0 and ISNULL(POP.SubWorkOrderId,0)  = 0 AND WOM.WorkOrderMaterialsKitId = @WorkOrderMaterialsKitId

			set @MaterialKitTotalCount = @MaterialKitTotalCount - 1;
			set @MainKitLoopId = @MainKitLoopId +1;
		END
