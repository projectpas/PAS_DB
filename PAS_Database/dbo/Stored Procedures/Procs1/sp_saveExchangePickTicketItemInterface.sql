...
EXEC [sp_saveExchangePickTicketItemInterface] 
******** 
** Change History 
********   
...
BEGIN     
 ...
 BEGIN  
   ...
   VALUES(@SOPickTicketNumber, @ExchangeSalesOrderId, @CreatedBy, @UpdatedBy, GETUTCDATE(), GETUTCDATE(), @IsActive, @IsDeleted, @ExchangeSalesOrderPartId,  
  ...
  BEGIN  
   UPDATE [dbo].[ExchangeSOPickTicket] SET ConfirmedById = @ConfirmedById, IsConfirmed = @IsConfirmed, ConfirmedDate = GETUTCDATE() WHERE SOPickTicketId = @SOPickTicketId;  
  END  
 END  
 COMMIT  TRANSACTION  
