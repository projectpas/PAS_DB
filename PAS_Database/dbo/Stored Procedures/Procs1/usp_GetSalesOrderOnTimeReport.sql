 ...
 **************************************************************           
	...
	1	 01/02/2024	  AMIT GHEDIYA	     added isperforma Flage for SO
...
**************************************************************/
CREATE OR ALTER PROCEDURE [dbo].[usp_GetSalesOrderOnTimeReport] @customername varchar(40) = NULL,
@Fromdate datetime,
@Todate datetime,
@mastercompanyid int,
    ...
    BEGIN TRANSACTION
      ...
      LEFT OUTER JOIN dbo.customer WITH (NOLOCK) ON SO.customerid = customer.customerid
        ...
        LEFT OUTER JOIN dbo.itemmaster WITH (NOLOCK)
         ...
         LEFT OUTER JOIN dbo.billinginvoicing AS SOBI WITH (NOLOCK)
