	...
	BEGIN TRY
		BEGIN TRANSACTION
			BEGIN
				...
				--CASE WHEN INVOICE IS GENERATED THEN TAKE IT FROM INVOICE  OLD TABLE
				...
				IF((SELECT COUNT(1) FROM [dbo].[BillingInvoicingItems] WOB WITH(NOLOCK) 
					LEFT JOIN [dbo].[WorkOrderWorkFlow] WOWF WITH(NOLOCK) ON WOB.[SubReferenceId] = WOWF.[WorkOrderPartNoId]
				...
				BEGIN					
					...
					SET Revenue = (ISNULL(WOB.[GrandTotal],0) - (ISNULL(WOB.[SalesTax], 0) + ISNULL(WOB.[OtherTax],0))),
						...
						LaborRevePer = dbo.udfCalcPercentage(ISNULL(WOCD.TotalLaborCost,0), (ISNULL(WOB.[GrandTotal],0) - (ISNULL(WOB.[SalesTax], 0) + ISNULL(WOB.[OtherTax],0)))),
