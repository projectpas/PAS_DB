  ...
  SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
    ...
    BEGIN TRANSACTION
		...
		BEGIN 
			...
			;WITH rptCTE (TotalRecordsCount, workorderPartId, customername, pn, partNos, pndescription, serialnum, workscope, workOrderId, wonum, workOrderNum, fromstage, tostage, employee, currentstage, recdate, opendate, quotedate, quotenum, quoteamount, quoteapprovalamount, level1, level2, level3, level4, level5, level6, level7, level8, level9, level10,Sequence,CurrentStageId,MasterCompanyId) 
				AS (
					SELECT 
						...
						(SELECT UPPER(WOS_From.Code + '-' + WOS_From.Stage) 
						 FROM DBO.WorkOrderStage WOS_From WITH (NOLOCK) 
						 WHERE WOS_From.WorkOrderStageId = WTT.OldStageId) AS fromstage,
						(SELECT UPPER(WOS_To.Code + '-' + WOS_To.Stage) 
						 FROM DBO.WorkOrderStage WOS_To WITH (NOLOCK) 
						 WHERE WOS_To.WorkOrderStageId = WTT.CurrentStageId) AS tostage,
						UPPER(MAX(WTT.ChangedBy)) AS employee, 
						UPPER(MAX(WOSG_Curr.Code + '-' + WOSG_Curr.Stage)) AS currentstage,
						CASE 
							WHEN ISNULL(@IsDownload, 0) = 0 
								THEN FORMAT(MAX(WPN.ReceivedDate), 'MM/dd/yyyy') 
								ELSE CONVERT(VARCHAR(50), MAX(WPN.ReceivedDate), 107) 
						END AS recdate, 
						CASE 
							WHEN ISNULL(@IsDownload, 0) = 0 
								THEN FORMAT((SELECT [dbo].[ConvertUTCtoLocal](MAX(WO.OpenDate), MAX(TZ.Description))), 'MM/dd/yyyy') 
								ELSE CONVERT(VARCHAR(50), (SELECT [dbo].[ConvertUTCtoLocal](MAX(WO.OpenDate), MAX(TZ.Description))), 107) 
						END AS opendate, 
						CASE 
							WHEN ISNULL(@IsDownload, 0) = 0 
								THEN FORMAT(MAX(workOrderQ.OpenDate), 'MM/dd/yyyy') 
								ELSE CONVERT(VARCHAR(50), MAX(workOrderQ.OpenDate), 107) 
						END AS quotedate, 
						MAX(workOrderQ.QuoteNumber) AS quotenum, 
						MAX(CASE 
							WHEN woq.QuoteMethod = 1 
								THEN ISNULL(CommonFlatRate, 0) 
								ELSE ISNULL(woq.FreightFlatBillingAmount + woq.ChargesFlatBillingAmount + woq.LaborFlatBillingAmount + woq.MaterialFlatBillingAmount, 0) 
						END) AS quoteamount,
						MAX(CASE 
							WHEN workOrderQ.QuoteStatusId = @ApprovedQuoteStatusId 
								THEN CASE 
									WHEN woq.QuoteMethod = 1 
										THEN ISNULL(CommonFlatRate, 0) 
										ELSE ISNULL(woq.FreightFlatBillingAmount + woq.ChargesFlatBillingAmount + woq.LaborFlatBillingAmount + woq.MaterialFlatBillingAmount, 0) 
						...
						MAX(WOSG.Sequence) AS Sequence,
						MAX(WTT.CurrentStageId) AS CurrentStageId,
						MAX(WO.MasterCompanyId) AS MasterCompanyId
					...
					WHERE CAST(WTT.StatusChangedDate AS DATE) BETWEEN CAST(@Fromdate AS DATE) AND CAST(@Todate AS DATE) 
					  ...
					  AND WO.IsActive = 1
					  AND (WTT.ChangedBy = ISNULL(@Employee, WTT.ChangedBy)) 
					  AND ((ISNULL(@FromStage, '') = '' OR WOSG_Old.WorkOrderStageId = @FromStage) 
						   AND (ISNULL(@ToStage, '') = '' OR WOSG.WorkOrderStageId = @ToStage))
				 ...
				 ,FinalCTE(TotalRecordsCount, workorderPartId, customername, pn, partNos, pndescription, serialnum, workscope, workOrderId, wonum, workOrderNum, fromstage, tostage, employee,
				 ...
				 ,Sequence,CurrentStageId,MasterCompanyId) 
				 AS (SELECT DISTINCT TotalRecordsCount, workorderPartId, customername, pn, partNos, pndescription, serialnum, workscope, workOrderId, wonum, workOrderNum, fromstage,
				 tostage, employee, currentstage, recdate, opendate, quotedate, quotenum, quoteamount, quoteapprovalamount, level1, level2, level3, level4, level5, level6, level7, level8, level9, 
				 level10,Sequence,CurrentStageId,MasterCompanyId FROM rptCTE)

				

				  ,WithTotal (MasterCompanyId, TotalQuoteAmount,TotalQuoteApprovalAmount)   
					 AS (SELECT MasterCompanyId,   
					FORMAT(SUM(quoteamount), 'N', 'en-us') TotalQuoteAmount,  
					FORMAT(SUM(quoteapprovalamount), 'N', 'en-us') TotalQuoteApprovalAmount  
					FROM FinalCTE  
					...
					ORDER BY (Sequence), CurrentStageId DESC  
					OFFSET((@PageNumber - 1) * @pageSize) ROWS FETCH NEXT @pageSize ROWS ONLY;   

				-- SELECT * FROM rptCTE
				--ORDER BY MAX(Sequence), CurrentStageId DESC
				--OFFSET((@PageNumber - 1) * @pageSize) ROWS FETCH NEXT @pageSize ROWS ONLY
