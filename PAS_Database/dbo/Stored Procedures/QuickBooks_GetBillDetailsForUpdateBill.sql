/*************************************************************           
 ** File:   [QuickBooks_GetBillDetailsForUpdateBill]           
 ** Author:   Abhishek Jirawla
 ** Description: Get Bill Details to Update Bill in QuickBooks    
 ** Purpose:         
 ** Date:   06-Feb-2025       
 ...
 ** --   --------		-------			--------------------------------          
    1    06-Feb-2025   Abhishek Jirawla	Created
     
 EXECUTE [QuickBooks_GetNewBillListForCreateBill] 1
**************************************************************/ 
CREATE     PROCEDURE [dbo].[QuickBooks_GetBillDetailsForUpdateBill]
...
BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT NPO.NonPOInvoiceId AS InvoiceId,
					...
					NPO.MasterCompanyId,
					NPO.UpdatedBy,
					NPO.QuickBooksReferenceId AS BillQuickBooksReferenceId,
					NPO.SyncToken,
				...
				WHERE ISNULL(NPO.QuickBooksReferenceId, '') != '' AND NPO.NonPOInvoiceId = @ReferenceId AND NPO.MasterCompanyId = @MasterCompanyId
	...
	BEGIN CATCH      
			...
			,@AdhocComments VARCHAR(150) = 'QuickBooks_GetBillDetailsForUpdateBill'
