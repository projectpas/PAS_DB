 ...
 ** --   --------		-------					--------------------------------          
	...
	8    20/July/2026			 RAJESH GAMI						[PN-17350] - Removed IsNonStock=0 filter from ItemMaster LEFT JOIN so Non-Stock invoice lines are included in the QuickBooks Exchange SO invoice sync payload.
...
BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT	SOBI.SOBillingInvoicingId,
						...
						P.TaxRateRef,
						P.TxnTaxCodeRef,
						ISNULL(SSBI.PeriodicBillingAmount, 0),
	...
	BEGIN CATCH      
			...
			,@AdhocComments VARCHAR(150) = 'QuickBooks_GetSyncPendingExChangeInvoiceList'
