 ...
 ** --   --------     -------		  --------------------------------          
	...
	7    27/Apr/2026	RAJESH GAMI		 Bank name lower if CompanyCode is 'a2z'
  ...
  SET NOCOUNT ON
    BEGIN TRY
      BEGIN
			...
			DECLARE @MasterCompanyId INT,@NEO_MasterComapnyId BIGINT = 20,@NEO_MasterCompanyIdCode  VARCHAR(100)='NEO' , @CompanyCode VARCHAR(100)='',@a2z_MasterCompanyIdCode  VARCHAR(100)='a2z';;
			...
			ELSE
			BEGIN
					SELECT  '<label> ' + CASE WHEN  @CompanyCode =@a2z_MasterCompanyIdCode THEN ISNULL(BankName, '') ELSE UPPER(ISNULL(BankName, '')) END + ' </label><br/>' +
