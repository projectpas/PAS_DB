...
BEGIN
  ...
  BEGIN TRANSACTION
	...
	BEGIN
		...
		UPDATE TEMP_TABLE
		SET
		QuantityQuote    = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(QuantityQuote, 0)    ELSE [dbo].[fn_ConvertUOM](ISNULL(QuantityQuote, 0),   IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 0, TEMP_TABLE.MasterCompanyId) END),
		QtyRequested     = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(QtyRequested, 0)     ELSE [dbo].[fn_ConvertUOM](ISNULL(QtyRequested, 0),    IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 0, TEMP_TABLE.MasterCompanyId) END),
		QtyQuoted        = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(QtyQuoted, 0)        ELSE [dbo].[fn_ConvertUOM](ISNULL(QtyQuoted, 0),       IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 0, TEMP_TABLE.MasterCompanyId) END),
		QtyAvailable     = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(QtyAvailable, 0)     ELSE [dbo].[fn_ConvertUOM](ISNULL(QtyAvailable, 0),    IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 0, TEMP_TABLE.MasterCompanyId) END),
		QtyOH            = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(QtyOH, 0)            ELSE [dbo].[fn_ConvertUOM](ISNULL(QtyOH, 0),           IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 0, TEMP_TABLE.MasterCompanyId) END),
		GrossSaleAmount  = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(GrossSaleAmount, 0)  ELSE [dbo].[fn_ConvertUOM](ISNULL(GrossSaleAmount, 0), IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		...
		NetSaleAmount    = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(NetSaleAmount, 0)    ELSE [dbo].[fn_ConvertUOM](ISNULL(NetSaleAmount, 0),   IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		TaxAmount        = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(TaxAmount, 0)        ELSE [dbo].[fn_ConvertUOM](ISNULL(TaxAmount, 0),       IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		UnitCostExtended = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(UnitCostExtended, 0) ELSE [dbo].[fn_ConvertUOM](ISNULL(UnitCostExtended, 0),IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		MarginAmount     = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(MarginAmount, 0)     ELSE [dbo].[fn_ConvertUOM](ISNULL(MarginAmount, 0),    IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		UnitSalesPrice   = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(UnitSalesPrice, 0)   ELSE [dbo].[fn_ConvertUOM](ISNULL(UnitSalesPrice, 0),  IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		...
		SalesPriceExtended = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(SalesPriceExtended, 0) ELSE [dbo].[fn_ConvertUOM](ISNULL(SalesPriceExtended, 0), IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END),
		TEMP_TABLE.UnitCost = (CASE WHEN IM.[ConsumeUnitOfMeasure] = IM.[StockUnitOfMeasure] THEN ISNULL(TEMP_TABLE.UnitCost, 0) ELSE [dbo].[fn_ConvertUOM](ISNULL(TEMP_TABLE.UnitCost, 0), IM.[ConsumeUnitOfMeasure], IM.[StockUnitOfMeasure], 1, TEMP_TABLE.MasterCompanyId) END)
