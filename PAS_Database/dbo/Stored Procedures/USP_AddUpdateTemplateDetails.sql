...
BEGIN
	...
	BEGIN TRANSACTION
		...
		BEGIN
			...
			USING @tbl_WorkflowChargesListType AS [Source]
			ON (TARGET.WorkflowId = SOURCE.WorkflowId AND TARGET.WorkflowChargesListId = SOURCE.WorkflowChargesListId) 
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[VendorName], [Source].[Order], [Source].[IsDeleted], [Source].[Memo], [Source].[WFParentId], @IsVersionIncrease
				);			
		END
		ELSE
		BEGIN
		...
		BEGIN
			...
			USING @tbl_WorkflowDirectionType AS [Source]
			ON (TARGET.WorkflowId = SOURCE.WorkflowId AND TARGET.WorkflowDirectionId = SOURCE.WorkflowDirectionId) 
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					GETUTCDATE(), GETUTCDATE(), [Source].[IsActive], [Source].[IsDeleted], [Source].[Order], [Source].[WFParentId], @IsVersionIncrease, [Source].[TaskName],
		...
		BEGIN
			...
			USING @tbl_WorkflowEquipmentListType AS [Source]
			ON ([Target].[WorkflowId] = [Source].[WorkflowId] AND [Target].[WorkflowEquipmentListId] = [Source].[WorkflowEquipmentListId])
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					GETUTCDATE(), GETUTCDATE(), [Source].[IsActive], [Source].[IsDeleted], [Source].[PartNumber], [Source].[Order], [Source].[Memo], [Source].[WFParentId], [Source].[IsVersionIncrease],
		...
		BEGIN
			...
			USING @tbl_WorkFlowExclusionListType AS [Source]
			ON ([Target].[WorkflowId] = [Source].[WorkflowId] AND [Target].[WorkflowExclusionId] = [Source].[WorkflowExclusionId])
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[Order], [Source].[ConditionId], [Source].[ItemClassificationId], [Source].[WFParentId], [Source].[IsVersionIncrease]
				);			
		END
		ELSE
		BEGIN
		...
		BEGIN
			...
			USING @tbl_WorkflowExpertiseListType AS [Source]
			ON ([Target].[WorkflowId] = [Source].[WorkflowId] AND [Target].[WorkflowExpertiseListId] = [Source].[WorkflowExpertiseListId])
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[Order], [Source].[Memo], [Source].[WFParentId], [Source].[IsVersionIncrease], [Source].[OverheadburdenPercentId]
				);			
		END
		ELSE
		BEGIN
		...
		BEGIN
			...
			USING @tbl_WorkflowMaterialType AS [Source]
			ON ([Target].[WorkflowId] = [Source].[WorkflowId] AND [Target].[WorkflowMaterialListId] = [Source].[WorkflowMaterialListId])
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[MaterialMandatoriesId], [Source].[WFParentId], [Source].[IsVersionIncrease], [Source].[Figure], [Source].[Item]
				);
		END
		ELSE
		BEGIN
		...
		BEGIN
			...
			USING @tbl_WorkflowMeasurementListType AS [Source]
			ON ([Target].[WorkflowId] = [Source].[WorkflowId] AND [Target].[WorkflowMeasurementId] = [Source].[WorkflowMeasurementId])
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[Order], [Source].[PartDescription], [Source].[WFParentId], [Source].[IsVersionIncrease]
				);
		END
		ELSE
		BEGIN
		...
		BEGIN
			...
			WHEN NOT MATCHED THEN
				...
				VALUES (
					...
					[Source].[Status], [Source].[Image], [Source].[TaskId], @workFlowMainId, @MasterCompanyId, [Source].[Order], [Source].[IsActive], [Source].[Memo], [Source].[WFParentId], [Source].[IsVersionIncrease]
				);
	...
	BEGIN
		...
		SELECT	WFT.[WorkFlowTaskId],  @workFlowMainId,  @WorkFlowNumber,  WFT.[TaskId],  WFT.[TaskDescription],  WFT.[SequenceNumber],  WFT.[Descrepancy], @IsVersionIncrease,  WFT.[Resolution],  @MasterCompanyId,
