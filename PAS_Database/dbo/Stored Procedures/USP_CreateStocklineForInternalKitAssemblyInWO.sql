/*************************************************************           
 ** File:   [USP_CreateStocklineForInternalKitAssemblyInWO]           
 ** Author:   Moin Bloch
 ** Description: This stored procedure is used to create stockline for InternalKitAssembly
 ** Purpose:         
 ** Date: 21-08-2026
 ...
 ** --   --------     -------		--------------------------------          
    1    21-08-2026	  Moin Bloch	Created 
	
************************************************************************/
CREATE PROCEDURE [dbo].[USP_CreateStocklineForInternalKitAssemblyInWO]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @PartNumber VARCHAR(50),@ManufacturerId BIGINT,@QtyOrder INT,@SerialNumber VARCHAR(30) = ''
		...
		DECLARE @Memo NVARCHAR(MAX) ='Stockline Created For Internal Kit Assembly'
		...
		DECLARE @StkManagementStructureModuleId BIGINT=0,@StockLineModuleID INT=0,@IsNonStock BIT = 0,@IsService BIT = 0
		...
		SELECT @PartNumber = ITM.[PartNumber], 
			   ...
			   @QuantityOnHand = @KitsToPrepare,
			   @QuantityAvailable = @KitsToPrepare,			  
			   @UnitCost = 0,
	...
	BEGIN CATCH
		...
		DECLARE @ErrorLogID INT,
				...
				,@AdhocComments VARCHAR(150) = 'USP_CreateStocklineForInternalKitAssemblyInWO'
				,@ProcedureParameters VARCHAR(3000) = '@WorkOrderId = ''' + ISNULL(CAST(@WorkOrderId AS VARCHAR(20)), '') + ''', @ConditionId = ''' + ISNULL(CAST(@ConditionId AS VARCHAR(20)), '') + ''', @ItemMasterId = ''' + ISNULL(CAST(@ItemMasterId AS VARCHAR(20)), '') + ''''
