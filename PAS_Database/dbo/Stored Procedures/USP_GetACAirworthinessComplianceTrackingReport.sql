/************************************************************
** File:        [USP_GetACAirworthinessComplianceTrackingReport]
** Author:      Amit Ghediya
** Description: AC Airworthiness Compliance Tracking report - reads AircraftPublication
**              with the same joins/columns as USP_GetAircraftPublicationList, but as a
**              dedicated report procedure (From/To Entry Date range + multi-select
**              Publication Type) decoupled from the live CRUD listing procedure.
**
** Change History
************************************************************
...
CREATE   PROCEDURE [dbo].[USP_GetACAirworthinessComplianceTrackingReport]
    ...
    @PublicationType      VARCHAR(MAX)    = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
			...
			LEFT JOIN (
				...
				INNER JOIN [dbo].[AircraftRegistryHeader] ARH WITH (NOLOCK) ON ARH.AircraftRegistryId = AMP.AircraftRegistryId
            ...
            OUTER APPLY (
                ...
                FROM [dbo].[AircraftMaintenanceProgram] AMP WITH (NOLOCK)
				LEFT JOIN [dbo].[WorksheetMapping] WSM WITH (NOLOCK) ON WSM.ProgramId = AMP.ProgramId
                LEFT JOIN [dbo].[WorksheetHeader] WSH WITH (NOLOCK) ON WSH.ProgramId = WSM.ProgramId AND WSH.WorksheetHeaderId = WSM.WorksheetHeaderId
            ...
            WHERE
				...
				AND ((@FromEntryDate IS NULL AND @ToEntryDate IS NULL)
						OR CAST(AP.EntryDate AS DATE) BETWEEN
						CAST(@FromEntryDate AS DATE)
						AND CAST(@ToEntryDate AS DATE)
					)
                AND (
				...
				AND (NULLIF(@PublicationType, '') IS NULL OR EXISTS (
						SELECT 1 FROM STRING_SPLIT(@PublicationType, ',') PTS
						WHERE PT.[Name] LIKE '%' + PTS.value + '%'
					))
    ...
    BEGIN CATCH
        ...
        DECLARE
            ...
            @AdhocComments       VARCHAR(150)  = 'USP_GetACAirworthinessComplianceTrackingReport',
