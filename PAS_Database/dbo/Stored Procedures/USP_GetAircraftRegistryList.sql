 ...
 ** Description:  To Get Aircraft Registry Data from Aricraft header table
 ** Date:        
...
--exec [dbo].[USP_GetAircraftRegistryList] 105
************************************************************************/
...
BEGIN TRY
    ;WITH CTE AS
    (
        SELECT AR.AircraftRegistryId, AR.MakeType, AR.AircraftModel, AR.AircraftSubModel,AR.NumOfEngines, AR.TailNum, AR.SerialNum,AR.ManufacturedDate,AR.PlaceInServiceDate,AR.TotalTSN, AR.TotalCSN,AR.Hobbs, AR.AircraftLocation, AR.NextScheduled,AR.MEL, AR.AircraftStatus, AR.IsActive, AR.CreatedDate, COUNT(1) OVER() AS TotalRecords
        FROM [dbo].[AircraftRegistryHeader] AR WITH (NOLOCK)
        WHERE  AR.MasterCompanyId = @MasterCompanyId  AND (@IsDeleted IS NULL OR AR.IsDeleted = @IsDeleted)
            AND (
                @GlobalFilter IS NULL OR
                AR.MakeType LIKE '%' + @GlobalFilter + '%' OR
                AR.AircraftModel LIKE '%' + @GlobalFilter + '%' OR
                AR.TailNum LIKE '%' + @GlobalFilter + '%' OR
                AR.SerialNum LIKE '%' + @GlobalFilter + '%' OR
                AR.AircraftStatus LIKE '%' + @GlobalFilter + '%'
            )
