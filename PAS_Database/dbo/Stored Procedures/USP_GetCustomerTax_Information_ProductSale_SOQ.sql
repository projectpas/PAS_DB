 ...
 ** --   --------     -------			--------------------------------          
	...
	4    03/19/2025   RAJESH GAMI		Modified for SaleTax and OtherTax Multyply by part count
  ...
  BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE #tmprShipDetails SET [SalesTax] = @FreighSalesTax + @ChargeSalesTax, 
											[OtherTax] = @FreighOtherTax + @ChargeOtherTax									
									WHERE [ID] = @MinId
			END
		END
