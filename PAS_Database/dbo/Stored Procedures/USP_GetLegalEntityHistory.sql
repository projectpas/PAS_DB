	...
	DECLARE @CurrntEmpTimeZoneDesc VARCHAR(100) = '';
		...
		WHERE 
		 ...
		 SELECT
			...
			ISNULL(lea.[IsBalancingEntity], 0) AS IsBalancingEntity,
			ISNULL(lea.[IsAddressForShipping], 0) AS IsAddressForShipping,
			ISNULL(lea.[IsAddressForBilling], 0) AS IsAddressForBilling,
			ISNULL(lea.[LastLevel], 0) AS LastLevel,
			...
			ISNULL(lea.[IsActive], 0) AS IsActive,
			ISNULL(lea.[IsDeleted], 0) AS IsDeleted,
			ISNULL(lea.[CompanyName], '') AS CompanyName,
			ISNULL(lea.[TagName], '') AS TagName,
			ISNULL(TZ.[TimeZoneName], '') AS TimeZoneName,
			ISNULL(lea.[IsPrintCheckNumber], 0) AS IsPrintCheckNumber
