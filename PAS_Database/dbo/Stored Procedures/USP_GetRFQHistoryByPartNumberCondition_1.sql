...
BEGIN
  ...
  --      [MarkUpPercentValue] DECIMAL(18,2) NULL,
	...
	BEGIN
		...
		--PRINT 3
		INSERT INTO #tmpResult1(PartNumber, Condition, Code, UnitPrice, [Sequence], QuoteSendReviewId, QuoteSendReview)
	...
	IF((SELECT MAX(UnitPrice) FROM #tmpResult1) > 0)
	BEGIN
		SELECT TOP 1 * FROM #tmpResult1 WHERE ISNULL(UnitPrice, 0) > 0 Order by Sequence
	END
	ELSE 
	BEGIN
		SELECT TOP 1 * FROM #tmpResult1 Order by Sequence
	END
