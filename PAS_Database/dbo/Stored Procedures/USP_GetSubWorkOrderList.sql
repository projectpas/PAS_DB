...
BEGIN	
		...
		BEGIN
			...
			SET @IsLabor = CASE WHEN (SELECT COUNT(SubWorkOrderLaborHeaderId) FROM [dbo].[SubWorkOrderLaborHeader] WITH(NOLOCK) WHERE SubWorkOrderId = @TmpSubWOId) > 0 THEN
						   ...
						   WHERE SUBH.SubWorkOrderId = @TmpSubWOId) > 1 THEN 1 ELSE 0 END ELSE 0 END
			SET @IsCharges = CASE WHEN (SELECT COUNT(SubWorkOrderChargesId) FROM [dbo].[SubWorkOrderCharges] WITH(NOLOCK) WHERE SubWorkOrderId = @TmpSubWOId) > 0 THEN 1 ELSE 0 END
			SET @IsFreight = CASE WHEN (SELECT COUNT(SubWorkOrderFreightId) FROM [dbo].[SubWorkOrderFreight] WITH(NOLOCK) WHERE SubWorkOrderId = @TmpSubWOId) > 0 THEN 1 ELSE 0 END
		...
		;WITH Result AS(
				SELECT DISTINCT
						...
						ISNULL(WOS.WorkScopeCode, '') AS 'WorkScope',
						SWO.OpenDate,
						SWO.WorkOrderId,
	...
	BEGIN CATCH      
			...
			,@ProcedureParameters VARCHAR(3000) = '@Parameter1 = ''' + CAST(ISNULL(@PageNumber, '') AS VARCHAR(100))
			   ...
			   + '@Parameter17 = ''' + CAST(ISNULL(@OpenDate  , '') AS VARCHAR(100))
