/*************************************************************           
 ** File:   [USP_GetWOQuoteDetails_Labor]           
 ** Author:   Devendra Shekh
 ** Description: This is used to get Work Order Quote Details after computing Amount for Labor
 ** Date:   27-May-2025        
 ** RETURN VALUE:           
 **************************************************************           
 ** Change History           
...
**************************************************************/
CREATE   PROCEDURE [dbo].[USP_GetWOQuoteDetails_Labor]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE	@WorkOrderQuoteDetailsId BIGINT, @MaterialFlatBillingAmount DECIMAL(18, 2), @LaborFlatBillingAmount DECIMAL(18, 2), @ChargesFlatBillingAmount DECIMAL(18, 2),
				@QuoteLaborCost DECIMAL(18, 2), @QuoteOverHeadCost DECIMAL(18, 2),  @totalRevenue DECIMAL(18, 2), @totalMargin DECIMAL(18, 2),
				@MaterialMargin DECIMAL(18, 2), @LaborMargin DECIMAL(18, 2), @ChargesMargin DECIMAL(18, 2),
				@InitialRowId INT = 1, @LaborBuildMethod INT;
		...
		SELECT	@LaborBuildMethod = [LaborBuildMethod], @LaborFlatBillingAmount = [LaborFlatBillingAmount], @WorkOrderQuoteDetailsId = [WorkOrderQuoteDetailsId]
		FROM #tmpWorkOrderQuoteDetailsResult WHERE [RowId] = @InitialRowId;

		IF EXISTS(SELECT 1 FROM @tbl_WorkOrderQuoteLaborType)
		BEGIN
			DECLARE @LaborBilling DECIMAL(18, 2), @LaborCost DECIMAL(18, 2), @OverHeadCost DECIMAL(18, 2);;

			SELECT @LaborBilling = SUM(ISNULL(BillingAmount, 0)), @LaborCost = SUM(ISNULL(TotalCost, 0)), @OverHeadCost = SUM(ISNULL(DirectLaborOHCost, 0)) FROM @tbl_WorkOrderQuoteLaborType GROUP BY WorkOrderQuoteLaborHeaderId;

			IF(@LaborBilling IS NOT NULL)
			BEGIN
				UPDATE TMP
				SET	TMP.LaborBilling = ISNULL(@LaborBilling, 0),
					TMP.LaborCost = ISNULL(@LaborCost, 0),
					TMP.OverHeadCost = ISNULL(@OverHeadCost, 0),
					TMP.LaborRevenue = ISNULL(@LaborBilling, 0),
					TMP.LaborMargin = ISNULL(@LaborBilling, 0) - ISNULL(@LaborCost, 0),
					TMP.LaborFlatBillingAmount = CASE WHEN ISNULL(@LaborBuildMethod, 0) = @FlateRateBillMethod THEN @LaborFlatBillingAmount ELSE ISNULL(@LaborBilling, 0) END
		...
		SELECT	@MaterialFlatBillingAmount = [MaterialFlatBillingAmount], @LaborFlatBillingAmount = [LaborFlatBillingAmount], @ChargesFlatBillingAmount = [ChargesFlatBillingAmount],
				@QuoteLaborCost = [LaborCost], @QuoteOverHeadCost = [OverHeadCost], @MaterialMargin = [MaterialMargin], @LaborMargin = [LaborMargin], @ChargesMargin = [ChargesMargin], @WorkOrderQuoteDetailsId = [WorkOrderQuoteDetailsId]
		FROM #tmpWorkOrderQuoteDetailsResult WHERE [RowId] = @InitialRowId;
		...
		UPDATE TMP
		SET	TMP.LaborMarginPer = dbo.fn_GetMarginPercentage(@totalMargin, @totalRevenue),
			TMP.LaborRevenuePercentage = dbo.fn_GetRevenuePercentage(@QuoteLaborCost, @totalRevenue),
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------
        , @AdhocComments     VARCHAR(150)    = 'USP_GetWOQuoteDetails_Labor' 
