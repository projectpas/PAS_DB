/*************************************************************           
 ** File:   [USP_ReallocateSOInvoiceAccountingPeriod]           
 ** Author:   Vishal Suthar
 ** Description: Update Batch entry date for SO Invoice
 ** Purpose:         
 ** Date:   11/09/2026
...
**************************************************************/
CREATE   PROCEDURE [dbo].[USP_ReallocateSOInvoiceAccountingPeriod]
...
BEGIN
	...
	BEGIN TRY
		...
		SET @NewInvoiceDate = DATEADD(HOUR, 9, CAST(CAST(@NewInvoiceDate AS DATE) AS DATETIME2(7)));
		...
		DECLARE @SOIJournalTypeId BIGINT = NULL;
		...
		SELECT @SOIJournalTypeId = ID FROM [dbo].[JournalType] WITH(NOLOCK) WHERE UPPER([JournalTypeCode]) = UPPER('SOI');
		...
		-- ASSUMPTION TO CONFIRM: SalesOrderBatchDetails.DocumentId is reused generically per
		-- JournalType - a SalesOrderShippingId for 'SOS' entries, a BillingInvoicingId for
		-- 'SOI' entries (mirroring how WorkOrderBatchDetails carries both shipping- and
		-- invoice-oriented columns on one row, keyed by journal type).
		INSERT INTO #tmpExistingEntries (JournalBatchDetailId, JournalBatchHeaderId, CommonJournalBatchDetailId, CurrentStatusId)
		SELECT DISTINCT SOD.[JournalBatchDetailId], BD.[JournalBatchHeaderId], SOD.[CommonJournalBatchDetailId], BH.[StatusId]
		...
		WHERE SOD.[DocumentId] = @BillingInvoicingId
		  AND BD.[JournalTypeId] = @SOIJournalTypeId
	...
	BEGIN CATCH
		...
		DECLARE @ErrorLogID INT, @DatabaseName VARCHAR(100) = DB_NAME(),
				@AdhocComments VARCHAR(150) = 'USP_ReallocateSOInvoiceAccountingPeriod',
