...
BEGIN
	...
	BEGIN
		...
		-- [PN-17635] Reversal entries belong in the SAME batch as the original invoice they're reversing - reuse
		-- the original JournalBatchHeaderId (already captured on #OriginalEntries) instead of inserting a new
		...
		-- [PN-17635] The header now represents both the original entries AND this reversal, so add to whatever
		-- totals it already has instead of overwriting them (overwriting would erase the original invoice's totals).
