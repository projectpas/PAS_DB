		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					---------Create SOQ With part---------------------------------------------
					SELECT @PartNumber = [LinePartNumber], @BuyerCompanyName = [BuyerCompanyName] FROM [dbo].[CustomerRfq] WITH(NOLOCK) WHERE [CustomerRfqId] = @GetCustomerRfqId;
					...
					IF(ISNULL(@ItemMasterId,0) > 0 AND  ISNULL(@CustomerId,0) > 0 AND ISNULL(@IsAutoInternalQuote,0) > 0)
					BEGIN
						EXEC [dbo].[USP_CreateSalesOrderQuoteFromAI] @EmailRfqQuoteDetails,@CustomerId,@MasterCompanyId,@CreatedBy,2,@GetCustomerRfqId,@ItemMasterId,0
					END
