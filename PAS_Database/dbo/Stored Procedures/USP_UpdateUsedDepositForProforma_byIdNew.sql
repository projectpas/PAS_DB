/*************************************************************             
 ** File:   [USP_UpdateUsedDepositForProforma_byIdNew]             
 ** Author:   Moin Bloch
 ...
 ** --   --------     -------				-------------------------------            
	...
	EXEC  [dbo].[USP_UpdateUsedDepositForProforma_byIdNew] 508,15
**************************************************************/ 

create    PROCEDURE [dbo].[USP_UpdateUsedDepositForProforma_byIdNew]      
	...
	BEGIN TRANSACTION
		...
		BEGIN
			SELECT @ReferenceId = [ReferenceId], 
			       @TotalWorkOrderCostPlus = ISNULL([SubTotal],0),
				   @NewGrandTotal = ISNULL([GrandTotal], 0)
			...
			SELECT @DepositAmt = SUM(ISNULL([DepositAmount], 0)), 
			       @OldUsedDepositAmount = SUM(ISNULL([UsedDeposit], 0)) 
			 FROM [dbo].[BillingInvoicing] WITH(NOLOCK) 
			WHERE [ReferenceId] = @ReferenceId 
			AND [ModuleId] = @ModuleId
			AND ISNULL([IsVersionIncrease],0) = 0 
			AND ISNULL([IsPerformaInvoice],0) = 1 
			AND UPPER([InvoiceStatus]) = 'INVOICED'
			...
			SELECT TOP 1 @WOProFormaBillingInvoicingId = [BillingInvoicingId] 
			 FROM [dbo].[BillingInvoicing] WITH(NOLOCK) 
			WHERE [ReferenceId] = @ReferenceId 
			  AND [ModuleId] = @ModuleId
			  AND ISNULL([IsVersionIncrease],0) = 0  
			...
			BEGIN
				UPDATE [dbo].[BillingInvoicing] 
				   SET [RemainingAmount] = @RemainingAmount,
				       [ProformaDeposit] = @UsedDepositAmt
...
-----------------------------------PLEASE CHANGE THE VALUES FROM HERE TILL THE NEXT LINE----------------------------------------      
        , @AdhocComments     VARCHAR(150)    = 'USP_UpdateUsedDepositForProforma_byIdNew'       
