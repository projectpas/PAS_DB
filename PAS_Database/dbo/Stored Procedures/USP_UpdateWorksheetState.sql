/************************************************************
** File:        [USP_UpdateWorksheetState]
** Author:      Amit Ghediya
** Description: Update IsDeleted / IsActive state on an
**              WorksheetHeader row by Id.
**
** Change History
************************************************************
** PR   Date         Author          Description
** --   ----------   -------------   -------------------------
** 1    16/06/2026	  Amit Ghediya  Created [PN-16874]
************************************************************/
CREATE     PROCEDURE [dbo].[USP_UpdateWorksheetState]
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN TRANSACTION;
            ...
            SET @WorksheetRows = @@ROWCOUNT;

        COMMIT TRANSACTION;

        -- Return the Id on success, -1 when no row matched the WHERE clause.
        SELECT CASE WHEN @WorksheetRows > 0 THEN @WorksheetHeaderId ELSE -1 END AS WorksheetHeaderId;
    ...
    BEGIN CATCH
        ...
        DECLARE
            ...
            @AdhocComments       VARCHAR(150)  = 'USP_UpdateWorksheetState',
