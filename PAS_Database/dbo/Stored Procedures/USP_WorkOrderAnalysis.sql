...
<<<<<<< HEAD
		IF EXISTS (SELECT WOBillingInvoicingItemId FROM [dbo].[WorkOrderBillingInvoicingItem] WITH(NOLOCK) WHERE WorkOrderPartId = @WorkOrderPartNoId AND IsVersionIncrease = 0 AND ISNULL(IsPerformaInvoice, 0) != 1)
=======
...
<<<<<<< HEAD
				...
				CASE 
					...
					THEN ROUND((woc.PartsCost + woc.LaborCost + woc.ChargesCost) * 100.0 / woc.Revenue, 2) 
					ELSE 0 
				END AS DirectCostRevenuePercentage,
				(ISNULL(woc.Revenue, 0) - (woc.PartsCost + woc.LaborCost + woc.ChargesCost)) AS Margin,
				CASE 
					WHEN ISNULL(woc.Revenue, 0) > 0 
					THEN ROUND((ISNULL(woc.Revenue, 0) - (woc.PartsCost + woc.LaborCost + woc.ChargesCost)) * 100.0 / woc.Revenue, 2) 
=======
...
<<<<<<< HEAD
			...
			LEFT JOIN [dbo].[WorkOrderBillingInvoicingItem] wbi WITH(NOLOCK) ON wop.ID = wbi.WorkOrderPartId AND wbi.IsVersionIncrease = 0 AND ISNULL(wbi.IsPerformaInvoice, 0) != 1
			LEFT JOIN [dbo].[WorkOrderBillingInvoicing] wb WITH(NOLOCK) ON wbi.BillingInvoicingId = wb.BillingInvoicingId AND wb.IsVersionIncrease = 0 AND ISNULL(wb.IsPerformaInvoice, 0) != 1
...
<<<<<<< HEAD
				...
				CASE 
					WHEN q.Revenue > 0 
					THEN ROUND(woc.DirectCost * 100.0 / q.Revenue, 2) 
					ELSE 0 
				END AS DirectCostRevenuePercentage,
				(q.Revenue - woc.DirectCost) AS Margin,
				CASE 
					WHEN q.Revenue > 0 
					THEN ROUND((q.Revenue - woc.DirectCost) * 100.0 / q.Revenue, 2) 
=======
