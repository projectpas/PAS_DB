    ...
    BEGIN TRY
        BEGIN TRANSACTION
            ...
            DECLARE
				...
				@ManagementStructureModuleId  BIGINT,
				@ManagementStructureModuleIdPart  BIGINT,
				@CodeTypeId					  BIGINT,
                @VendorRFQRepairOrderNumber   NVARCHAR(100),
				...
				@Countries_Name               VARCHAR(50)    = 'UNITED STATES',
				@CodeType					  VARCHAR(50)    = 'Vendor',
				@FirstName					  VARCHAR(50)    = 'ADMIN';

			SELECT @ManagementStructureModuleId = [ManagementStructureModuleId] FROM dbo.ManagementStructureModule WITH (NOLOCK) WHERE ModuleName = 'VendorRFQROHeader';
			SELECT @ManagementStructureModuleIdPart = [ManagementStructureModuleId] FROM dbo.ManagementStructureModule WITH (NOLOCK) WHERE ModuleName = 'VendorRFQROPart';
			SELECT @CodeTypeId = CodeTypeId FROM dbo.CodeTypes WITH (NOLOCK) WHERE CodeType = 'VendorRFQRepairOrder';
            ...
            BEGIN
                ...
                BEGIN
                    SELECT TOP 1 @CountryId = countries_id
                    FROM dbo.Countries WITH (NOLOCK)
                    WHERE UPPER(countries_name) = @Countries_Name AND MasterCompanyId = @MasterCompanyId;
                END
            ...
            -- 3a. Get default status
            DECLARE @StatusId BIGINT, @StatusName NVARCHAR(100);

            SELECT TOP 1 @StatusId = VendorRFQStatusId, @StatusName = Description
            FROM dbo.VendorRFQStatus WITH (NOLOCK)
            WHERE IsActive = 1 AND IsDeleted = 0
            ORDER BY VendorRFQStatusId;
            ...
            SET @VendorRFQRepairOrderNumber =
                CASE
            ...
            -- 6. Create VendorRFQRepairOrder header
            INSERT INTO dbo.VendorRFQRepairOrder
                (VendorRFQRepairOrderNumber, VendorId, VendorName, VendorCode,
                 VendorContactId, VendorContact, VendorContactPhone, PriorityId, Priority, RequisitionerId,
                 StatusId, Status, OpenDate, NeedByDate, CreditLimit,
                 ManagementStructureId, Notes, MasterCompanyId,
                 CreatedBy, UpdatedBy, CreatedDate, UpdatedDate,
                 IsActive, IsDeleted, Resale, DeferredReceiver,
                 FunctionalCurrencyId, ReportCurrencyId, ForeignExchangeRate)
            SELECT
                @VendorRFQRepairOrderNumber, @VendorId, v.VendorName, v.VendorCode,
                @VendorContactId, @VendorName, @VendorPhone, @PriorityId, @PriorityName,
                CASE WHEN ISNULL(@EmployeeId, 0) = 0 THEN @DefaultEmployeeId ELSE @EmployeeId END,
                @StatusId, @StatusName, @Now, @NeedByDate, ISNULL(v.CreditLimit, 0),
            ...
            SET @VendorRFQRepairOrderId = SCOPE_IDENTITY();

            -- 6a. Save header management structure
            DECLARE @MSHeaderId BIGINT;
            EXEC dbo.[PROCAddROMSData]
                @VendorRFQRepairOrderId, @ManagementStructureId, @MasterCompanyId,
                @CreatedBy, @CreatedBy, @ManagementStructureModuleId, 1, @MSHeaderId OUTPUT;
			...
			DECLARE @DefaultUnitOfMeasureName NVARCHAR(250);

            SELECT TOP 1 @DefaultUnitOfMeasureId = UnitOfMeasureId, @DefaultUnitOfMeasureName = ShortName
            FROM dbo.UnitOfMeasure WITH (NOLOCK)
			WHERE MasterCompanyId = @MasterCompanyId AND UPPER([Description]) = 'EA'
			AND IsActive = 1 AND IsDeleted = 0;

            ------------------------------------------------------------
            -- GET DEFAULT CONDITION (RO Part.ConditionId is NOT NULL,
            -- unlike PO Part where a missing match can fall back to 0)
            ------------------------------------------------------------
            DECLARE @DefaultConditionId BIGINT;
            DECLARE @DefaultConditionName NVARCHAR(256);

            SELECT TOP 1 @DefaultConditionId = ConditionId, @DefaultConditionName = Description
            FROM dbo.Condition WITH (NOLOCK)
            WHERE MasterCompanyId = @MasterCompanyId AND IsActive = 1 AND IsDeleted = 0
            ORDER BY ConditionId;

            IF @DefaultConditionId IS NULL RETURN;
            ...
            ------------------------------------------------------------
            -- 7. INSERT VendorRFQRepairOrderPart
            -- Uses per-part ManufacturerId from @PartManufacturers
            ------------------------------------------------------------
            INSERT INTO dbo.VendorRFQRepairOrderPart
                (VendorRFQRepairOrderId, ItemMasterId, PartNumber, PartDescription,
                 ...
                 MasterCompanyId, CreatedBy, UpdatedBy, CreatedDate, UpdatedDate,
                 IsActive, IsDeleted, IsNoQuote, UOMId, UnitOfMeasure)
            SELECT
                @VendorRFQRepairOrderId,
                im.ItemMasterId,
                LTRIM(RTRIM(p.PartNumber)),
                ISNULL(LTRIM(RTRIM(p.Description)), 'N/A'),
                ISNULL(c.ConditionId, @DefaultConditionId),
                ISNULL(c.Description, @DefaultConditionName),
                pm.ManufacturerId,              
                pm.ManufacturerName,
                ...
                @CreatedBy, @CreatedBy, @Now, @Now,
                1, 0, 0,
				im.PurchaseUnitOfMeasureId, @DefaultUnitOfMeasureName
            FROM @tbl_EmailParts p
            ...
            BEGIN
                EXEC dbo.[PROCAddROMSData]
                    ...
                    @CreatedBy, @CreatedBy, @ManagementStructureModuleIdPart, 1, @MSPartId OUTPUT;
            ...
            SET ThirdPartyRFQId = @VendorRFQRepairOrderId
            WHERE IntegrationEmailID = @IntegrationEmailID;

            -- 9. Refresh RO detail aggregates
            EXEC dbo.UpdateVendorRFQRepairOrderDetail @VendorRFQRepairOrderId;
     ...
     BEGIN CATCH
	   ...
	   ,@AdhocComments VARCHAR(150) = 'usp_CreateVendorRFQROFromEmail'
