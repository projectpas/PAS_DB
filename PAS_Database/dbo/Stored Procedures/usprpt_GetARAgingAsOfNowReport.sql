	 ...
	 BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO #TEMPInvoiceRecords(BillingInvoicingId,CustomerId,CustomerName,CustomerCode,
											CurrencyCode,BalanceAmount,CurrentAmount,PaymentAmount,
											Amountlessthan0days,Amountlessthan30days,Amountlessthan60days,
											Amountlessthan90days,Amountlessthan120days,Amountmoerthan120days,
			...
			SELECT DISTINCT WOBII.BillingInvoicingId,	
							C.CustomerId,
							UPPER(ISNULL(C.[Name],'')),      
							UPPER(ISNULL(C.CustomerCode,'')),
							UPPER(CR.Code), 
							WOBII.GrandTotal, -- BalanceAmount      
							((ISNULL(WOBII.GrandTotal, 0) - ISNULL(WOBII.RemainingAmount, 0)) + ISNULL(WOBII.CreditMemoUsed, 0)),  -- CurrentlAmount     
							ISNULL(WOBII.RemainingAmount, 0) + ISNULL(WOBII.CreditMemoUsed, 0), --PaymentAmount  		               				
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) <= 0 THEN WOBII.RemainingAmount ELSE 0 END),
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) > 0 AND DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + ISNULL(WO.NetDays,0)  AS DATE), GETUTCDATE())<= 30 THEN WOBII.RemainingAmount ELSE 0 END),
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) > 30 AND DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + ISNULL(WO.NetDays,0)  AS DATE), GETUTCDATE())<= 60 THEN WOBII.RemainingAmount ELSE 0 END),
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) > 60 AND DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + ISNULL(WO.NetDays,0)  AS DATE), GETUTCDATE()) <= 90 THEN WOBII.RemainingAmount ELSE 0 END),
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) > 90 AND DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + ISNULL(WO.NetDays,0)  AS DATE), GETUTCDATE()) <= 120 THEN WOBII.RemainingAmount ELSE 0 END),
							(CASE WHEN DATEDIFF(DAY, CAST(CAST(WOBII.InvoiceDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1
																WHEN CTM.Code='CIA' THEN -1
																WHEN CTM.Code='CreditCard' THEN -1
																WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(WO.NetDays,0) END) AS DATE), GETUTCDATE()) > 120 THEN WOBII.RemainingAmount	ELSE 0 END),
							WOBII.GrandTotal,  -- InvoiceAmount      
							0,
							0,
							ISNULL(WOBII.CreditMemoUsed,0) AS CreditMemoUsed,						
							...
							STUFF((SELECT DISTINCT ',' + LE.[Name]  
								...
								FOR XML PATH('')), 1, 1, ''))
			FROM [dbo].[WorkOrderBillingInvoicing] WOBII WITH (NOLOCK) 
				INNER JOIN [dbo].[WorkOrder] WO WITH (NOLOCK) ON WO.WorkOrderId = WOBII.WorkOrderId      
				...
				INNER JOIN [dbo].[Currency] CR WITH(NOLOCK) ON CR.CurrencyId = WOBII.CurrencyId      
			...
			WHERE WO.CustomerId = ISNULL(@CustomerId, WO.CustomerId) AND WOBII.IsVersionIncrease = 0   
				AND WOBII.RemainingAmount > 0 AND WOBII.InvoiceStatus = 'Invoiced' 
				AND CAST(WOBII.InvoiceDate AS DATE) <= CAST(@AsOfDate AS DATE) 
			...
			--INSERT INTO #TEMPInvoiceRecords(BillingInvoicingId, CustomerId ,CustomerName ,CustomerCode ,CurrencyCode ,BalanceAmount, CurrentAmount, PaymentAmount ,
			--Amountlessthan0days ,Amountlessthan30days ,Amountlessthan60days ,Amountlessthan90days ,Amountlessthan120days,
			--Amountmoerthan120days,InvoiceAmount , CMAmount ,CreditMemoAmount, CreditMemoUsed ,level1 ,level2 ,level3 ,level4 ,level5 ,level6 ,level7 ,level8 ,level9 ,level10 ,MasterCompanyId ,StatusId ,
			...
			--			    UPPER(ISNULL(C.CustomerCode,'')),     					      
			--			    UPPER(CR.Code), 
			--			    ISNULL(CM.Amount,0), -- ISNULL(WOBI.GrandTotal,0), 
			--			    0,--(ISNULL(WOBI.GrandTotal,0) - ISNULL(WOBII.RemainingAmount,0)  + ISNULL(WOBII.CreditMemoUsed,0)),      
			--			    ISNULL(CM.Amount,0),--ISNULL(WOBII.RemainingAmount,0) + ISNULL(WOBII.CreditMemoUsed,0),
			...
			--					 WHEN CTM.Code='CIA' THEN -1      
			--					 WHEN CTM.Code='CreditCard' THEN -1      
			--					 WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(CTM.NetDays,0) END) AS DATE), GETUTCDATE()) > 0 AND DATEDIFF(DAY, CAST(CAST(CM.CreatedDate AS DATETIME) + ISNULL(CTM.NetDays,0)  AS DATE), GETUTCDATE())<= 30 THEN CM.Amount ELSE 0 END) AS Amountpaidby30days,      
			--			    (CASE WHEN DATEDIFF(DAY, CAST(CAST(CM.CreatedDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1      
			--					 WHEN CTM.Code='CIA' THEN -1      
			--					 WHEN CTM.Code='CreditCard' THEN -1      
			--					 WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(CTM.NetDays,0) END) AS DATE), GETUTCDATE()) > 30 AND DATEDIFF(DAY, CAST(CAST(CM.CreatedDate AS DATETIME) + ISNULL(CTM.NetDays,0)  AS DATE), GETUTCDATE())<= 60 THEN CM.Amount ELSE 0 END) AS Amountpaidby60days,      
			--			    (CASE WHEN DATEDIFF(DAY, CAST(CAST(CM.CreatedDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1      
			--					 WHEN CTM.Code='CIA' THEN -1      
			--					 WHEN CTM.Code='CreditCard' THEN -1      
			--					 WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(CTM.NetDays,0) END) AS DATE), GETUTCDATE()) > 60 AND DATEDIFF(DAY, CAST(CAST(CM.CreatedDate AS DATETIME) + ISNULL(CTM.NetDays,0)  AS DATE), GETUTCDATE()) <= 90 THEN CM.Amount ELSE 0 END) AS Amountpaidby90days,      
			--			    (CASE WHEN DATEDIFF(DAY, CASt(CAST(CM.CreatedDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1      
			--					 WHEN CTM.Code='CIA' THEN -1      
			--					 WHEN CTM.Code='CreditCard' THEN -1      
			--					 WHEN CTM.Code='PREPAID' THEN -1 ELSE ISNULL(CTM.NetDays,0) END) AS DATE), GETUTCDATE()) > 90 AND DATEDIFF(DAY, CASt(CAST(CM.CreatedDate AS DATETIME) + ISNULL(CTM.NetDays,0)  AS DATE), GETUTCDATE()) <= 120 THEN CM.Amount ELSE 0 END) AS Amountpaidby120days,      
			--			    (CASE WHEN DATEDIFF(DAY, CASt(CAST(CM.CreatedDate AS DATETIME) + (CASE WHEN CTM.Code = 'COD' THEN -1      
			--					 WHEN CTM.Code='CIA' THEN -1      
			--					 WHEN CTM.Code='CreditCard' THEN -1      
			...
			--			INNER JOIN [dbo].[WorkOrderBillingInvoicingItem] WOBI WITH (NOLOCK) ON CM.InvoiceId = WOBI.BillingInvoicingId
			--			INNER JOIN [dbo].[WorkOrderBillingInvoicing] WOBII WITH (NOLOCK) ON WOBII.BillingInvoicingId = WOBI.BillingInvoicingId      
			--			INNER JOIN [dbo].[WorkOrder] WO WITH (NOLOCK) ON WO.WorkOrderId = WOBII.WorkOrderId      
			--			INNER JOIN [dbo].[Customer] c  WITH (NOLOCK) ON C.CustomerId = WO.CustomerId   
			...
			--			AND CAST(WOBII.InvoiceDate AS DATE) <= CAST(@AsOfDate AS DATE) AND WO.mastercompanyid = @mastercompanyid      
			--			AND (ISNULL(@Level1,'') ='' OR MSD.[Level1Id] IN (SELECT Item FROM DBO.SPLITSTRING(@Level1,',')))      
			--			AND (ISNULL(@Level2,'') ='' OR MSD.[Level2Id] IN (SELECT Item FROM DBO.SPLITSTRING(@Level2,',')))      
			--			AND (ISNULL(@Level3,'') ='' OR MSD.[Level3Id] IN (SELECT Item FROM DBO.SPLITSTRING(@Level3,',')))      
			--			AND (ISNULL(@Level4,'') ='' OR MSD.[Level4Id] IN (SELECT Item FROM DBO.SPLITSTRING(@Level4,',')))      
			...
			SELECT  [CustomerId],[CustomerName],[CustomerCode],[CurrencyCode],
					...
					ISNULL(SUM([Amountmoerthan120days]),0) Amountmoerthan120days,
			...
			GROUP BY [CustomerId],[CustomerName],[CustomerCode],[CurrencyCode],[IsCreditMemo],[StatusId],
					 [level1],[level2],[level3],[level4],[level5],[level6],[level7],[level8],[level9],[level10]	
		...
		BEGIN
			...
			SELECT DISTINCT WOBI.[BillingInvoicingId],			               
			                ...
			                CASE WHEN (DATEDIFF(DAY, WOBI.[InvoiceDate], GETUTCDATE()) - ISNULL(WO.NetDays,0)) > 0 
							     ...
							     ELSE 0 END,  -- 'DSO' 		
							DATEDIFF(DAY, WOBI.[ShipDate], GETUTCDATE()),  --  'DSS'
			...
			INSERT INTO #TEMPInvoiceRecordsDetailsView([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],
				...
				SELECT DISTINCT SOBI.[SOBillingInvoicingId],
			                    ...
			                    CASE WHEN (DATEDIFF(DAY, SOBI.[InvoiceDate], GETUTCDATE()) - ISNULL(SO.[NetDays],0)) > 0 
									 ...
									 ELSE 0 END, -- 'DSO', 		
								DATEDIFF(DAY, SOBI.[ShipDate], GETUTCDATE()),  --  'DSS' 
			...
			INSERT INTO #TEMPInvoiceRecordsDetailsView([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],
				...
				SELECT DISTINCT ESOBI.SOBillingInvoicingId,
								...
								CASE WHEN (DATEDIFF(DAY, ESOBI.[InvoiceDate], GETUTCDATE()) - ISNULL(ESO.NetDays,0)) > 0 
							     ...
							     ELSE 0 END,  -- 'DSO' 		
								DATEDIFF(DAY, ESOBI.[ShipDate], GETUTCDATE()),  --  'DSS' 
		  ...
		  INNER JOIN [dbo].[AccountingBatchManagementStructureDetails] MSD WITH (NOLOCK) ON MSD.[ModuleID] = @MSModuleId AND MSD.[ReferenceID] = MJD.[ManualJournalDetailsId]    
		   ...
		   WHERE MJD.[ReferenceId] = ISNULL(@CustomerId,MJD.ReferenceId)  
			...
			GROUP BY MJH.[ManualJournalHeaderId],MJD.[ReferenceId],CST.[Name],CST.[CustomerCode],CR.[Code],MJH.[JournalNumber], 
   		    ...
   		    SELECT * INTO #TempResult1 FROM #TEMPInvoiceRecordsDetailsView
			WHERE (
