 ...
 **************************************************************           
	...
	10    23/July/2026			 RAJESH GAMI						[PN-17350] - Removed 2 leftover IsNonStock=0 exclusion filters on the SalesOrder-linked branches (SOP.SalesOrderId=SO.SalesOrderId). The ExchangeSalesOrder-only branches were left untouched, out of scope.
...
BEGIN
  ...
  SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
    ...
    BEGIN TRY
		...
		BEGIN
			IF OBJECT_ID(N'tempdb..#TEMPInvoiceRecordsSSRS') IS NOT NULL    
			BEGIN    
				DROP TABLE #TEMPInvoiceRecordsSSRS
			END

			CREATE TABLE #TEMPInvoiceRecordsSSRS(        
			...
			INSERT INTO #TEMPInvoiceRecordsSSRS([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
			...
			WHERE WO.[CustomerId] = ISNULL(@CustomerId, WO.CustomerId) AND ISNULL(WOBI.[IsVersionIncrease],0) = 0 AND ISNULL(WOBI.[IsPerformaInvoice],0) = 0
				...
				AND @IsInvoice = 1  AND WOBI.ModuleId = @WOModuleId
			...
			UPDATE #TEMPInvoiceRecordsSSRS SET InvoicePaidAmount = ISNULL(tmpcash.InvoicePaidAmount,0)
				FROM( SELECT 
				 ...
				 FROM [dbo].[InvoicePayments] IPS WITH (NOLOCK)   
					JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = IPS.SOBillingInvoicingId AND TmpInv.ModuleTypeId = @WOModuleTypeId
				...
				) tmpcash WHERE tmpcash.BillingInvoicingId = #TEMPInvoiceRecordsSSRS.BillingInvoicingId

			UPDATE #TEMPInvoiceRecordsSSRS SET CreditMemoAmount = ISNULL(tmpcm.CMAmount, 0)
					FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
						FROM [dbo].[CreditMemoDetails] CMD WITH (NOLOCK)   
							...
							JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = WOBII.BillingInvoicingId AND TmpInv.ModuleTypeId = @WOModuleTypeId
				...
				) tmpcm WHERE tmpcm.BillingInvoicingId = #TEMPInvoiceRecordsSSRS.BillingInvoicingId	
			...
			SELECT DISTINCT SOBI.[BillingInvoicingId],
				...
				WHERE SO.CustomerId = ISNULL(@customerid, SO.[CustomerId])
					...
					AND @IsInvoice = 1	AND SOBI.ModuleId = @SOModuleId
			...
			UPDATE #TEMPInvoiceRecordsSSRS SET [InvoicePaidAmount] = ISNULL(tmpcash.[InvoicePaidAmount],0)
				FROM(SELECT ISNULL(SUM(IPS.PaymentAmount),0) AS 'InvoicePaidAmount',
					   IPS.SOBillingInvoicingId AS BillingInvoicingId
				 FROM [dbo].[InvoicePayments] IPS WITH (NOLOCK)   
					JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = IPS.SOBillingInvoicingId AND TmpInv.ModuleTypeId = @SOModuleTypeId
					LEFT JOIN [dbo].[CustomerPayments] CP WITH (NOLOCK) ON CP.ReceiptId = IPS.ReceiptId  
			...
			UPDATE #TEMPInvoiceRecordsSSRS SET [CreditMemoAmount] = ISNULL(tmpcm.[CMAmount], 0)
				FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
					FROM [dbo].[CreditMemoDetails] CMD WITH (NOLOCK)   
						...
						JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = SOBII.BillingInvoicingId AND TmpInv.ModuleTypeId = @SOModuleTypeId
			...
			UPDATE #TEMPInvoiceRecordsSSRS SET [InvoicePaidAmount] = ISNULL(tmpcash.[InvoicePaidAmount],0)
				FROM(SELECT ISNULL(SUM(IPS.PaymentAmount),0) AS 'InvoicePaidAmount',				  
					   IPS.SOBillingInvoicingId AS BillingInvoicingId
				 FROM [dbo].[InvoicePayments] IPS WITH (NOLOCK)   
					JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = IPS.SOBillingInvoicingId AND TmpInv.ModuleTypeId = @EXSOModuleTypeId
					LEFT JOIN [dbo].[CustomerPayments] CP WITH (NOLOCK) ON CP.ReceiptId = IPS.ReceiptId  
			...
			UPDATE #TEMPInvoiceRecordsSSRS SET [CreditMemoAmount] = ISNULL(tmpcm.[CMAmount], 0)
				FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
					FROM [dbo].[CreditMemoDetails] CMD WITH (NOLOCK)   
						...
						JOIN #TEMPInvoiceRecordsSSRS TmpInv ON TmpInv.BillingInvoicingId = SOBII.SOBillingInvoicingId AND TmpInv.ModuleTypeId = @EXSOModuleTypeId
		  ...
		  INNER JOIN [dbo].[AccountingBatchManagementStructureDetails] MSD WITH (NOLOCK) ON MSD.[ModuleID] = @MSModuleId AND MSD.[ReferenceID] = MJD.[ManualJournalDetailsId]    
		   ...
		   WHERE MJD.[ReferenceId] = ISNULL(@CustomerId,MJD.ReferenceId)  
			...
			SELECT  [CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
			 ...
			 INTO #TempResultSSRS1 FROM #TEMPInvoiceRecordsSSRS		
			...
			SELECT @Count = COUNT(CustomerId) FROM #TempResultSSRS1

			SELECT *, @Count AS NumberOfItems FROM #TempResultSSRS1 ORDER BY  
		...
		BEGIN			
			...
			INSERT INTO #TEMPInvoiceRecordsDetailsViewSSRS([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
			...
			WHERE WO.[CustomerId] = ISNULL(@CustomerId, WO.CustomerId) AND ISNULL(WOBI.[IsVersionIncrease],0) = 0 AND ISNULL(WOBI.[IsPerformaInvoice],0) = 0
				AND ISNULL(WOBI.[RemainingAmount],0) > 0 AND WOBI.[InvoiceStatus] = @InvoiceStatus
				AND CAST(WOBI.[InvoiceDate] AS DATE) <= CASE WHEN ISNULL(@id5, 0) = 1 THEN CAST(@AsOfDate AS DATE) ELSE CAST(@AsOfDate-1 AS DATE) END
				AND WO.[MasterCompanyId] = @masterCompanyid AND WOBI.ModuleId = @WOModuleId
				AND @IsInvoice = 1
				AND (CASE WHEN @IsDeposit = 1 THEN ((ISNULL(WOBI.[GrandTotal], 0) - ISNULL(WOBI.[RemainingAmount], 0)) + ISNULL(WOBI.[CreditMemoUsed], 0)) END > 0 OR CASE WHEN @IsDeposit = 0 THEN 1 END = 1) 
			...
			INSERT INTO #TEMPInvoiceRecordsDetailsViewSSRS([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
				...
				WHERE SO.CustomerId = ISNULL(@customerid, SO.[CustomerId])
					AND ISNULL(SOBI.[RemainingAmount],0) > 0 AND SOBI.[InvoiceStatus] = @InvoiceStatus AND ISNULL(SOBI.[IsPerformaInvoice],0) = 0 
					AND CAST(SOBI.[InvoiceDate] AS DATE) <= CASE WHEN ISNULL(@id5, 0) = 1 THEN CAST(@AsOfDate AS DATE) ELSE CAST(@AsOfDate-1 AS DATE) END
					AND SO.[MasterCompanyId] = @Mastercompanyid AND SOBI.ModuleId = @SOModuleId
					AND @IsInvoice = 1
					AND (CASE WHEN @IsDeposit = 1 THEN ((ISNULL(SOBI.[GrandTotal], 0) - ISNULL(SOBI.[RemainingAmount], 0)) + ISNULL(SOBI.[CreditMemoUsed], 0)) END > 0 OR CASE WHEN @IsDeposit = 0 THEN 1 END = 1) 
			...
			UPDATE #TEMPInvoiceRecordsDetailsViewSSRS SET [CreditMemoAmount] = ISNULL(tmpcm.[CMAmount], 0)
				FROM( SELECT ISNULL(SUM(CMD.Amount),0) AS 'CMAmount', TmpInv.BillingInvoicingId, CMD.BillingInvoicingItemId      
					FROM [dbo].[CreditMemoDetails] CMD WITH (NOLOCK)   
						INNER JOIN [dbo].[CreditMemo] CM WITH (NOLOCK) ON CM.CreditMemoHeaderId = CMD.CreditMemoHeaderId 
						INNER JOIN [dbo].[BillingInvoicingItems] SOBII WITH(NOLOCK) ON SOBII.BillingInvoicingItemId = CMD.BillingInvoicingItemId 
						JOIN #TEMPInvoiceRecordsDetailsViewSSRS TmpInv ON TmpInv.BillingInvoicingId = SOBII.BillingInvoicingId AND TmpInv.ModuleTypeId = @SOModuleTypeId
					WHERE CMD.[IsWorkOrder] = 0 AND CM.[CustomerId] = TmpInv.CustomerId AND CM.[StatusId] = @CMPostedStatusId
					GROUP BY CMD.BillingInvoicingItemId, TmpInv.BillingInvoicingId  
			) tmpcm WHERE tmpcm.BillingInvoicingId = #TEMPInvoiceRecordsDetailsViewSSRS.BillingInvoicingId
			...
			INSERT INTO #TEMPInvoiceRecordsDetailsViewSSRS([BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
				...
				SELECT DISTINCT ESOBI.SOBillingInvoicingId,
                                ...
                                UPPER(ISNULL(C.[CustomerCode],'')),
								CASE 
									WHEN C.CustomerAffiliationId = @exte_cusId THEN UPPER(@exte_cus)
									WHEN C.CustomerAffiliationId = @inter_cusId THEN UPPER(@inter_cus)
									ELSE UPPER(@affi_cus)
								END AS CustomerAffiliation,
								UPPER(CR.[Code]), 
		  ...
		  INNER JOIN [dbo].[AccountingBatchManagementStructureDetails] MSD WITH (NOLOCK) ON MSD.[ModuleID] = @MSModuleId AND MSD.[ReferenceID] = MJD.[ManualJournalDetailsId]    
		   ...
		   WHERE MJD.[ReferenceId] = ISNULL(@CustomerId,MJD.ReferenceId)  
			...
			SELECT [ID],[BillingInvoicingId],[CustomerId],[CustomerName],[CustomerCode],[CustomerAffiliation],
				   [CurrencyCode],[DocType],[InvoiceNo],InvoiceNumber,[InvoiceDate],[DSI],[DSO],[DSS],[DueDate],
				   [CustomerRef],[Salesperson],[CreditTerms],											
			...
			INTO #TempResultSSRS FROM #TEMPInvoiceRecordsDetailsViewSSRS
			WHERE ((ISNULL(@CustomerName,'') ='' OR [CustomerName] LIKE '%' + @CustomerName+'%') AND
				  (ISNULL(@CustomerCode,'') ='' OR [CustomerCode] LIKE '%' + @CustomerCode + '%') AND					
