...
BEGIN  
  ...
  SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED  
   ...
   ;WITH InvoicesSOWO AS (
		...
		LEFT JOIN dbo.CreditMemo CM WITH (NOLOCK) ON CM.InvoiceId = BI.BillingInvoicingId
	...
	InvoiceWithSalesperson AS (
		...
		AND ((SA.ActivityTypeId = 1 AND FI.ModuleId = @WorkOrderModuleId) OR (SA.ActivityTypeId = 2 AND FI.ModuleId = @SalesOrderModuleId))
	  ), rptCTE (TotalRecordsCount, MasterCompanyId, EmployeeId, Salesperson, Customer, RevenueAmount, RevenueRate, RevenueCommission,
      ...
      SELECT COUNT(1) OVER () AS TotalRecordsCount,
		...
		LEFT JOIN dbo.EntityStructureSetup ES WITH (NOLOCK) ON ES.EntityStructureId = MSD.EntityMSID
		JOIN DBO.[Percent] RP WITH (NOLOCK) ON BI.RevenuePercentageId = RP.PercentId
		JOIN DBO.[Percent] MP WITH (NOLOCK) ON BI.MarginPercentageId = MP.PercentId
		WHERE 1 = 1
			AND  ((ISNULL(@Employee, '') = '' OR BI.EmployeeId = @Employee) 
			AND  (ISNULL(@Customer, '') = '' OR BI.CustomerId = @Customer)
			)
			...
			GROUP BY E.MasterCompanyId, E.EmployeeId, E.FirstName, E.LastName, C.[Name], RP.PercentValue, MP.PercentValue, 
			...
			,FinalCTE(TotalRecordsCount, MasterCompanyId, EmployeeId, Salesperson, Customer, RevenueAmount, RevenueRate, RevenueCommission,
				 MarginAmount, MarginRate, MarginCommission, TotalCommission,
				 level1, level2, level3, level4, level5, level6, level7, level8,level9, level10) 

			  AS (SELECT DISTINCT TotalRecordsCount, MasterCompanyId, EmployeeId, Salesperson, Customer, RevenueAmount, RevenueRate, RevenueCommission,
				 MarginAmount, MarginRate, MarginCommission, TotalCommission,
		    ...
		    SELECT COUNT(2) OVER () AS TotalRecordsCount, FC.EmployeeId, Salesperson, Customer, FORMAT(ISNULL(RevenueAmount,0) , 'N', 'en-us') Revenueamount, 
